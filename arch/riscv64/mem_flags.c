...
{
    return ((size_t)((uintptr_t)kernel_virt_end - (uintptr_t)kernel_virt_start))
        +  ((size_t)((uintptr_t)kernel_boot_end - (uintptr_t)kernel_boot_start));
}
...
#else
#error "One of CONFIG_RISCV64_SV39, CONFIG_RISCV64_SV48 or CONFIG_RISCV64_SV57 must be set!"
#endif
    ...
    printk("Reserving the Kernel Identity Map [%p - %p)",
            ...
            __riscv64_identity_map_offset + (1ULL<<CONFIG_RISCV64_IDENTITY_MAP_ORDER));
...
declare_init_desc(mem_flags, riscv64_virt_flags_static_init, "Setting RISC-V Virtual Memory Types");
