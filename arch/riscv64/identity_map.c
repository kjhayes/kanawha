...
#define VIRTUAL_BASE_ROOT_INDEX RISCV64_SV_LEVEL_4_INDEX_OF_ADDR(CONFIG_RISCV64_KERNEL_VIRTUAL_BASE)
#else
...
#define VIRTUAL_BASE_ROOT_INDEX RISCV64_SV_LEVEL_3_INDEX_OF_ADDR(CONFIG_RISCV64_KERNEL_VIRTUAL_BASE)
#else
...
#define VIRTUAL_BASE_ROOT_INDEX RISCV64_SV_LEVEL_2_INDEX_OF_ADDR(CONFIG_RISCV64_KERNEL_VIRTUAL_BASE)
#endif
#endif
#endif

#if ((CONFIG_RISCV64_KERNEL_VIRTUAL_BASE % ROOT_PAGE_SIZE)!=0)
#error "Could not map kernel to provided virtual base! (it is not a multiple of the root page table page size!"
#endif
...
riscv64_boot_alloc_page_table(void)
{
    ...
    struct riscv64_sv_page_table *table =
        &riscv64_boot_page_table_cache[riscv64_boot_page_table_cache_allocated_count];
        ...
        )
{
    ...
    if((((uintptr_t)mapping_to) & ((1ULL<<RISCV64_SV_PAGE_ORDER_LEVEL_0)-1)) != 0) {
        PANIC("Cannot drill mapping to a physical region which is not aligned to the minimum page size!");
    }
    if((((uintptr_t)mapping_from) & ((1ULL<<RISCV64_SV_PAGE_ORDER_LEVEL_0)-1)) != 0) {
        PANIC("Cannot drill mapping from a virtual region which is not aligned to the minimum page size!");
    }
    if(!may_overmap && ((((uintptr_t)*mapping_size) & ((1ULL<<RISCV64_SV_PAGE_ORDER_LEVEL_0)-1)) != 0)) {
        PANIC("Cannot drill mapping of size which is not a multiple of the minimum page size if overmapping is disabled!");
    }

    // NOTE: Do not make this if-else chain into a switch statement,
    //       if so, clang tends to generate a jump table which will be placed in the
    //       regular .text/data sections instead of in the .boot.text or .boot.data sections
    if(pt_level == 0){
    ...
    int virt_aligned = (((uintptr_t)mapping_from & ((1ULL<<pt_page_order)-1)) == 0);
    int phys_aligned = (((uintptr_t)mapping_to   & ((1ULL<<pt_page_order)-1)) == 0);
      ...
      && !(*entry & RISCV64_SV_VALID))
    {
        ...
        *entry = (((uintptr_t)mapping_to)>>2)
            |RISCV64_SV_VALID|RISCV64_SV_READ|RISCV64_SV_WRITE|RISCV64_SV_EXEC;
        ...
        *mapping_size -= pt_page_size;
        ...
        if(pt_index == RISCV64_SV_ENTRIES_PER_LEVEL-1) {
            ...
            // Tail recurse
            return riscv64_boot_drill_mapping(
                pt,
                pt_level,
                mapping_to,
                mapping_from,
    ...
    if((*entry & RISCV64_SV_VALID) == 0) {
        ...
        *entry = (((uintptr_t)subtable)>>2) | RISCV64_SV_VALID;
        ...
        subtable = (void*)(uintptr_t)(((*entry)<<2) & ~((1ULL<<RISCV64_SV_TABLE_ALIGN)-1));
    }
    ...
    if(*mapping_size == 0 || (pt_index == (RISCV64_SV_ENTRIES_PER_LEVEL-1))) {
        // Do not recurse (either we are done entirely, or done with this level
        return;
    }
...
riscv64_boot_setup_paging(void __phys *kernel_phys_base)
{
    ...
    for(size_t i = 0; i < RISCV64_SV_ENTRIES_PER_LEVEL * RISCV64_BOOT_PAGE_TABLE_CACHE_SIZE; i++) {
        cache[i] = 0;
    }
    }

    struct riscv64_sv_page_table *root_pt =
        riscv64_boot_alloc_page_table();
    ...
    // Now that we have paging, we can access the the standard text and data sections in highmem
    // (must use indirection carefully though due to relocations)
...
riscv64_map_identity_map_region(void)
{
    ...
    if(identity_map_region == NULL) {
        eprintk("OOM Error when initializing kernel identity map vmem_region!\n");
        return -ENOMEM;
    }
...
declare_init_desc(vmem, riscv64_map_identity_map_region, "Creating Identity Map Virtual Memory Region");
