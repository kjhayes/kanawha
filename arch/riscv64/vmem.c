...
sv_level_page_size(int level)
{
    ...
    {
    case 0:
        return RISCV64_SV_PAGE_SIZE_LEVEL_0;
    case 1:
        return RISCV64_SV_PAGE_SIZE_LEVEL_1;
    case 2:
        return RISCV64_SV_PAGE_SIZE_LEVEL_2;
#ifdef CONFIG_RISCV64_SV48
    case 3:
        return RISCV64_SV_PAGE_SIZE_LEVEL_3;
#ifdef CONFIG_RISCV64_SV57
    case 4:
        return RISCV64_SV_PAGE_SIZE_LEVEL_4;
#endif
#endif
    default:
...
sv_level_page_order(int level)
{
    ...
    {
    case 0:
        return RISCV64_SV_PAGE_ORDER_LEVEL_0;
    case 1:
        return RISCV64_SV_PAGE_ORDER_LEVEL_1;
    case 2:
        return RISCV64_SV_PAGE_ORDER_LEVEL_2;
#ifdef CONFIG_RISCV64_SV48
    case 3:
        return RISCV64_SV_PAGE_ORDER_LEVEL_3;
#ifdef CONFIG_RISCV64_SV57
    case 4:
        return RISCV64_SV_PAGE_ORDER_LEVEL_4;
#endif
#endif
    default:
...
static void
riscv64_verify_page_table(struct riscv64_sv_page_table __phys *phys_table,
                          int level)
{
    ...
    for(size_t i = 0; i < RISCV64_SV_ENTRIES_PER_LEVEL; i++)
    {
        ...
        if(*entry & RISCV64_SV_VALID)
        {
            ...
            if(RISCV64_SV_ENTRY_IS_LEAF(*entry))
            {
                ...
                {
                    panic("Unaligned Physical Page in Page "
                          "Table Level %d\n",
                          level);
                }
            }
            ...
            {
                riscv64_verify_page_table(addr, level - 1);
            }
        }
        else
        {
            // We should only have fully NULL entries which are not
            // valid
            if(*entry)
            {
...
static void
riscv64_dump_page_table(printk_f *printer,
...
#define DUMP_PENDING_LEAF()                                                    \
    ...
    {                                                                          \
        ...
        {                                                                      \
            pending_vaddr = (void *)((uintptr_t)pending_vaddr |                \
                                     ((~(uintptr_t)0) << VADDR_BITS));         \
        }                                                                      \
...
#define VADDR_BITS 39
    ...
    for(size_t i = 0; i < RISCV64_SV_ENTRIES_PER_LEVEL; i++)
    {
        ...
        if(!is_leaf)
        {
            riscv64_dump_page_table(printer, cur_paddr, vbase, level - 1);
        }
...
int
arch_vmem_map_init(struct vmem_map *generic_map)
{
    int res;
    ...
    res =
        page_alloc(RISCV64_SV_TABLE_ORDER, (void __phys **)&map->root_table, 0);
    if(res)
    {
...
static int
free_sv_page_tables(struct riscv64_sv_page_table __phys *phys_table, int level)
{
    int res;
    if(level > 0)
    {
        ...
        for(size_t i = 0; i < RISCV64_SV_ENTRIES_PER_LEVEL; i++)
        {
            ...
            if(!RISCV64_SV_ENTRY_IS_LEAF(*entry))
            {
                ...
                *entry = 0;
                ...
                if(res)
                {
                    wprintk("Failed to free subtree of page "
                            "table (%s) Leaking "
                            "Memory!\n",
                            errnostr(res));
                }
...
arch_vmem_map_deinit(struct vmem_map *map)
{
    ...
    res = free_sv_page_tables(map->arch_state.root_table,
...
{
    int res;
    res = page_alloc(RISCV64_SV_TABLE_ORDER, (void __phys **)table_ptr, 0);
    if(res)
    {
        return res;
    }
...
static int
create_sv_table_entry(uint64_t *entry,
                      int level,
                      struct riscv64_sv_page_table __phys *subtable)
{
...
static int
create_sv_leaf_entry(uint64_t *entry,
                     int level,
...
static int
create_direct_sv_table(struct riscv64_sv_page_table __phys **table_ptr,
                       int level,
                       void __phys *base,
                       size_t size,
                       unsigned long flags)
{
    ...
    for(size_t i = 0; i < RISCV64_SV_ENTRIES_PER_LEVEL; i++)
    {
        ...
        if(size >= page_size)
        {
            res = create_sv_leaf_entry(entry, level, base, flags);
            if(res)
            {
        ...
        else
        {
            ...
            {
                struct riscv64_sv_page_table __phys *subtable;
                res = create_direct_sv_table(&subtable,
                                             level - 1,
                                             base,
                                             size,
                                             flags);
                ...
                res = create_sv_table_entry(entry, level, subtable);
                if(res)
                {
                    // TODO (We currently leak any children of
                    // the subtable
                    wprintk("Hit Possible Memory Leak "
                            "Condition\n");
...
static int
create_paged_sv_table(struct riscv64_sv_page_table __phys **table_ptr,
                      int level,
                      size_t size)
{
    int res;
    ...
    {
        struct riscv64_sv_page_table __phys *subtable;
        res = create_paged_sv_table(&subtable, level - 1, remaining_size);
        if(res)
        {
            return res;
        }
...
arch_vmem_region_init_direct(struct vmem_region *region)
{
    ...
    uintptr_t phys_end =
        (uintptr_t)region->direct.phys_base + (uintptr_t)region->size;

    if(phys_base % RISCV64_SV_PAGE_SIZE_LEVEL_0)
    {
        eprintk("Tried to initialize a direct region with physical base "
                "unaligned from the smallest page size!\n");
        return -EINVAL;
    }
    if(region->size % RISCV64_SV_PAGE_SIZE_LEVEL_0)
    {
        eprintk("Tried to initialize direct region with size that isn't a "
                "multiple of the minimum page size!\n");
        return -EINVAL;
    }
    ...
    if(region->size < RISCV64_SV_PAGE_SIZE_LEVEL_1)
    {
        region->arch_state.root_level =
            0; // We can fit inside a single level 0 table
    }
    else if(region->size < RISCV64_SV_PAGE_SIZE_LEVEL_2)
    {
        region->arch_state.root_level =
            1; // We can fit inside a single level 1 table
    }
#ifdef CONFIG_RISCV64_SV48
    else if(region->size < RISCV64_SV_PAGE_SIZE_LEVEL_3)
    {
        region->arch_state.root_level =
            2; // We can fit inside a single level 2 table
    }
#ifdef CONFIG_RISCV64_SV57
    else if(region->size < RISCV64_SV_PAGE_SIZE_LEVEL_4)
    {
        region->arch_state.root_level =
            3; // We can fit inside a single level 3 table
    }
#endif
#endif
    else
    {
        eprintk("arch_vmem_region_init_direct: region of size = 0x%lx is too "
                "large!\n",
                (uintptr_t)region->size);
        return -EINVAL;
    }

    res = create_direct_sv_table(&region->arch_state.root_table,
                                 region->arch_state.root_level,
...
arch_vmem_region_init_paged(struct vmem_region *region)
{
    ...
    res = create_paged_sv_table(&region->arch_state.root_table,
                                region->arch_state.root_level,
                                region->size);
    if(res)
    {
...
{
    int res;
    res = free_sv_page_tables(region->arch_state.root_table,
                              region->arch_state.root_level);
    if(res)
    {
        return res;
...
static int
sv_map_region_tables(struct riscv64_sv_page_table __phys *phys_map_table,
                     ...
                     struct vmem_region *region)
{
    ...
    size_t max_num_entries_in_map =
        RISCV64_SV_ENTRIES_PER_LEVEL - virtual_index;
    ...
    for(size_t vi = virtual_index; vi < RISCV64_SV_ENTRIES_PER_LEVEL; vi++)
    {
        ...
        if(!(*region_entry & RISCV64_SV_VALID))
        {
            ...
            dprintk("sv_map_region_tables: Hit end of region at "
                    "region index = "
                    "0x%lx\n",
                    vi - virtual_index);
            return 0;
        ...
        if(!(*map_entry & RISCV64_SV_VALID))
        {
            ...
            *map_entry = *region_entry;
            dprintk("sv_map_region_tables: mapping directly at index "
                    "= 0x%lx\n",
                    vi);
            continue;
        }
           ...
           RISCV64_SV_ENTRY_IS_LEAF(*region_entry))
        {
            ...
            panic("Unexpected vmem region overlap when mapping "
                  "region tables!\n");
            // This return should not be hit
            return -EINVAL;
        }

        struct riscv64_sv_page_table __phys *region_next_table =
            sv_pointer_from_entry(*region_entry);
        struct riscv64_sv_page_table __phys *map_next_table =
        ...
        if(!(*map_entry & RISCV64_SV_VMEM_SHARED_MAP))
        {
            ...
            struct riscv64_sv_page_table *shared_table =
                __va(phys_shared_table);

            // Copy the other region's table
            memcpy(shared_table,
                   __va(map_next_table),
                   sizeof(struct riscv64_sv_page_table));

            res = create_sv_shared_table_entry(map_entry,
                                               level,
                                               phys_shared_table);
            if(res)
            {
...
static int
sv_unmap_region_tables(struct riscv64_sv_page_table __phys *phys_map_table,
                       struct riscv64_sv_page_table __phys *phys_region_table,
                       int level,
                       void *vbase,
                       struct vmem_map *map,
                       struct vmem_region *region)
{
    ...
    for(size_t vi = virtual_index; vi < RISCV64_SV_ENTRIES_PER_LEVEL; vi++)
    {
        ...
        {
            // Something is wrong
            // (region was mapped incorrectly or this region is not
            // mapped at all)
            return -EINVAL;
        }
        ...
        else if(*map_entry & RISCV64_SV_VMEM_SHARED_MAP)
        {
            ...
            if(shared_table_empty)
            {
                ...
                if(res)
                {
                    wprintk("Failed to free shared intermediate "
                            "page table "
                            "(Possibly Leaking Memory) (err=%s)\n",
                            errnostr(res));
                }
                else
                {
                    // Only clear the map entry if we freed the
                    // shared table (Hopefully won't leak
                    // memory if we fail to free)
                    *map_entry = 0x0;
                }
            }
        }
        ...
        {
            // Either they should be exactly the same or it should be a
            // shared table
            return -EINVAL;
        }
    }
...
arch_vmem_map_map_region(struct vmem_map *map, struct vmem_region_ref *ref)
{
    ...
    {
        eprintk("arch_vmem_map_map_region: invalid map->level < region->level "
                "(%d < %d)\n",
                (int)level,
                (int)ref->region->arch_state.root_level);
        return -EINVAL;
    }

    size_t region_root_page_size =
        sv_level_page_size(ref->region->arch_state.root_level);
    ...
    while(level > ref->region->arch_state.root_level)
    {
        ...
        else
        {
            ...
            res = create_sv_shared_table_entry(map_entry, level, shared_table);
            if(res)
            {
                return res;
            }
    ...
    res = sv_map_region_tables(phys_map_table,
...
arch_vmem_map_unmap_region(struct vmem_map *map, struct vmem_region_ref *ref)
{
    ...
    if(level < ref->region->arch_state.root_level)
    {
        eprintk("arch_vmem_map_unmap_region: invalid map->level < "
                "region->level (%d < %d)\n",
                (int)level,
                (int)ref->region->arch_state.root_level);
        return -EINVAL;
    ...
    res = sv_unmap_region_tables(phys_map_table,
                                 ref->region->arch_state.root_table,
                                 level,
                                 ref->virt_addr,
                                 map,
                           ...
                           unsigned long flags)
{
    ...
    dprintk("arch_vmem_paged_region_map: region=%p, offset=%p, phys_addr=%p, "
            "size=%p\n",
            region,
            offset,
            phys_addr,
    ...
    struct riscv64_sv_page_table __phys *phys_root =
    ...
    while(size > 0)
    {
        ...
        dprintk("arch_vmem_paged_region_map(offset=0x%lx, phys_addr=%p, "
                "size=0x%lx)\n",
                offset,
                phys_addr,
                size);
        ...
        do
        {
            ...
            {
                eprintk("arch_vmem_paged_region_map(offset=0x%lx)"
                        " offset is "
                        "too large for page table!\n",
                        offset);
                return -EINVAL;
            }
            ...
            else
            {
                ...
                res =
                    create_sv_table_entry(cur_entry, cur_level, phys_subtable);
                if(res)
                {
                    return res;
                }
        ...
        struct riscv64_sv_page_table *cur_table = __va(cur_phys_table);
        size_t cur_index = (cur_offset % sv_level_page_size(cur_level + 1)) /
                           sv_level_page_size(cur_level);
        DEBUG_ASSERT(cur_index < RISCV64_SV_ENTRIES_PER_LEVEL);
        uint64_t *entry = &cur_table->entries[cur_index];

        res = create_sv_leaf_entry(entry, cur_level, phys_addr, flags);
        if(res)
        {
            return res;
        }

        dprintk("Created Leaf Entry 0x%lx at level=%d, offset=0x%lx, "
                "index=0x%lx, phys_addr=0x%lx\n",
                *entry,
                cur_level,
                offset,
                cur_index,
                             ...
                             size_t size)
{
    ...
    while(size > 0)
    {
        ...
        do
        {
            struct riscv64_sv_page_table *cur_table = __va(cur_phys_table);
            size_t cur_index =
                sv_level_index_of_addr(cur_level, (void *)offset);
            uint64_t *cur_entry = &cur_table->entries[cur_index];
            if(*cur_entry & RISCV64_SV_VALID)
            {
                DEBUG_ASSERT(!RISCV64_SV_ENTRY_IS_LEAF(*cur_entry));
            ...
            {
                // We are unmapping, if it doesn't exist, then our
                // job is already done
                break;
            }
            cur_level--;
        ...
        DEBUG_ASSERT(!(*entry & RISCV64_SV_VALID) ||
...
{
    riscv64_dump_page_table(printer,
                            map->arch_state.root_table,
                            0x0,
                            map->arch_state.root_level);
}
