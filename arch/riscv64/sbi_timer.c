        ...
        struct irq_action *action)
{
    ...
    if(timer->periodic) {
        ...
        cycles_t dur_cycles = cycles_from_duration(timer->period, current_timebase);
...
sbi_timer_apply_delta_xcall(void *__delta)
{
    ...
    cycles_t dur_cycles = cycles_from_duration(delta->duration, current_timebase);
    cycles_t alarm_time = current_time + dur_cycles;
        ...
        alarm_f *func)
{
    ...
    return xcall_run(timer->cpu_id, sbi_timer_apply_delta_xcall, (void*)&delta);
}

static int
sbi_timer_set_alarm_periodic(
        struct timer_dev *dev,
        size_t alarm,
        duration_t period,
        alarm_f *func)
...
declare_init_desc(post_topo, sbi_timer_init, "Initializing SBI Timer Extension");
