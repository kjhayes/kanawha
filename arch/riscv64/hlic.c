...
static irq_t
riscv64_dt_hlic_xlate_irq(struct dt_driver *driver,
                          struct dt_node *node,
                          const fdt32_t *cells,
                          size_t cell_count)
{
...
static struct dt_driver_ops riscv64_dt_hlic_driver_ops = {
...
static struct dt_driver riscv64_dt_hlic_driver = {
...
static int
riscv64_hlic_irq_dev_mask_irq(struct irq_dev *dev, hwirq_t hwirq)
{
...
static int
riscv64_hlic_irq_dev_unmask_irq(struct irq_dev *dev, hwirq_t hwirq)
{
...
riscv64_hlic_irq_dev_irq_status(struct irq_dev *dev, hwirq_t hwirq)
{
    switch(hwirq)
    {
    case 1:
...
static int
riscv64_hlic_irq_dev_trigger_irq(struct irq_dev *dev, hwirq_t hwirq)
{
...
int
riscv64_setup_cpu_hlic(struct riscv64_cpu *cpu, struct dt_node *hlic_node)
{
    int res;
    ...
    *(struct irq_domain **)percpu_ptr_specific(
        percpu_addr(riscv64_hlic_irq_domain),
        cpu->cpu.id) = hlic_domain;
    ...
    for(size_t i = 0; i < RISCV64_INTERRUPT_IRQ_DOMAIN_SIZE; i++)
    {
        ...
        if(riscv64_percpu_hlic_actions[i] == NULL)
        {
            ...
            if(action == NULL)
            {
                eprintk("Failed to install percpu link from "
                        "RISCV IRQ %lu to "
                        "HLIC IRQ!\n",
                        i);
                continue;
        ...
        res = irq_action_set_percpu_link(action, hlic_desc, cpu->cpu.id);
        if(res)
        {
            eprintk("Failed to create percpu link from RISCV IRQ to "
                    "HLIC IRQ "
                    "on CPU %lu\n",
                    (ul_t)cpu->cpu.id);
...
riscv64_hlic_provide_xcall_ipis(void)
{
    ...
    for(cpu_id_t id = 0; id < total_num_cpus(); id++)
    {
        ...
        res = xcall_provide_ipi_irq(
            id,
            irq_domain_revmap(hlic_domain, HLIC_SSI_HWIRQ));
        if(res)
        {
            eprintk("Failed to provide X-CALL IPI from HLIC on "
                    "CPU(%lu) (err=%s)\n",
                    (ul_t)id,
                    errnostr(res));
            return res;
...
declare_init_desc(smp,
                  riscv64_hlic_provide_xcall_ipis,
                  "Providing HLIC IPI(s) to X-CALL Framework");

struct irq_desc *
riscv64_hlic_irq_desc(hwirq_t hwirq, cpu_id_t cpu)
{
    struct irq_domain *domain = *(struct irq_domain **)percpu_ptr_specific(
        percpu_addr(riscv64_hlic_irq_domain),
        cpu);
