...
__attribute__((aligned(4096))) static uint8_t
    ap_bringup_virtual_stack[1ULL << AP_BRINGUP_STACK_ORDER];

_Static_assert(sizeof(ap_bringup_virtual_stack) ==
                   1ULL << AP_BRINGUP_STACK_ORDER,
               "ap_bringup_stack has incorrect size!");

void *
riscv64_boot_ap_init(hartid_t hartid)
{
    int res;
...
extern void
riscv64_ap_entry(void *, void *);
static void __phys *volatile riscv64_ap_entry_ptr =
    (void __phys *)&riscv64_ap_entry;
...
riscv64_smp_bringup_aps(void)
{
    ...
    struct vmem_region *lowmem_identity_map = vmem_region_create_direct(
        0,
    ...
    {
        eprintk("Failed to create lowmem identity map region for bringing up "
                "AP(s)!\n");
    ...
    if(res)
    {
        eprintk("Failed to map lowmem identity region into vmem_map for "
                "bringing up AP(s)!\n");
        vmem_map_destroy(map);
    ...
    struct vmem_region *highmem_map = vmem_region_create_direct(
    ...
    res = vmem_map_map_region(map,
                              highmem_map,
                              (void *)CONFIG_RISCV64_KERNEL_VIRTUAL_BASE);
    if(res)
    {
        eprintk("Failed to map highmem region into vmem_map for bringing up "
                "AP(s)!\n");
        vmem_map_destroy(map);
        vmem_region_destroy(lowmem_identity_map);
    ...
    for(cpu_id_t id = 0; id < total_num_cpus(); id++)
    {
        ...
        {
            res = sbi_hart_start(hartid, riscv64_ap_entry_ptr, satp);
            if(res)
            {
    ...
    if(res)
    {
        wprintk("Failed to free AP bringup lowmem vmem_region! (Leaking "
                "Memory)\n");
    }
    res = vmem_region_destroy(highmem_map);
    if(res)
    {
        wprintk("Faield to free AP bringup highmem vmem_region! (Leaking "
                "Memory)\n");
    }
    return 0;
}
