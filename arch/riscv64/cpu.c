        ...
        struct dt_node *node)
{
    ...
    if(reg_count != 1) {
        eprintk("Failed to get Device Tree \"cpu\" node reg property count!\n");
        return -EINVAL;
    }
    ...
    {
        struct dt_node *child = container_of(list_node, struct dt_node, child_node);
    ...
    freq_t *percpu_timebase = percpu_ptr_specific(percpu_addr(riscv64_timebase_freq), cpu->cpu.id);
...
declare_init_desc(topo, register_riscv64_dt_cpu_driver, "Registering Device Tree RISC-V CPU Node Driver");
