...
struct x64_percpu_data __x64_percpu_data[CONFIG_MAX_CPUS] = {0};
...
{
    __x64_percpu_data[remote_id].percpu_offset =
        percpu_area - (uintptr_t)__builtin_kpercpu_start;
    return 0;
}
...
{
    __x64_percpu_data[cur_cpu_id].percpu_offset =
        percpu_area - (uintptr_t)__builtin_kpercpu_start;
