...
x64_bsp_bringup_aps(void)
{
    ...
    struct x64_cpu *bsp = container_of(gen_cpu, struct x64_cpu, cpu);

    size_t num_brought_up = 0;

    void __phys *trampoline_paddr;

    res = page_alloc(AP_TRAMPOLINE_PAGE_ORDER,
                     &trampoline_paddr,
                     PAGE_ALLOC_16BIT);
    if(res)
    {
        eprintk("Failed to allocate 16-bit trampoline page for AP bringup! "
                "(err=%s)\n",
                errnostr(res));
        return res;
    }
    ...
    res = page_alloc(AP_BOOT_STACK_ORDER,
                     &stack_paddr,
                     0x0); // the AP won't use this before long mode,
    ...
    volatile uint8_t *ap_launched_byte =
        trampoline + ((uintptr_t)x64_ap_trampoline_ap_launched_byte -
                      (uintptr_t)x64_ap_trampoline_start);

    volatile uint16_t *trampoline_self_ptr =
        trampoline + ((uintptr_t)x64_ap_trampoline_self_ptr -
                      (uintptr_t)x64_ap_trampoline_start);
    *trampoline_self_ptr = (uint16_t)(uintptr_t)trampoline_paddr;

    volatile void **trampoline_virtual_stack_base =
        trampoline + ((uintptr_t)x64_ap_trampoline_virtual_stack_base -
                      (uintptr_t)x64_ap_trampoline_start);
    *trampoline_virtual_stack_base = virtual_stack;
    ...
    for(cpu_id_t ap_id = 0; ap_id < total_num_cpus(); ap_id++)
    {
        ...
        if(gen_ap == NULL)
        {
            wprintk("total_num_cpus() = %ld, but AP %ld cannot be "
                    "found!\n",
                    (sl_t)total_num_cpus(),
                    (sl_t)ap_id);
        }
        struct x64_cpu *ap = container_of(gen_ap, struct x64_cpu, cpu);
