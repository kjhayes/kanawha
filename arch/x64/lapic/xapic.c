...
xapic_provide_mmio_base(void __phys *base)
{
    if(xapic_phys_base_valid)
    {
        ...
        {
            eprintk("xAPIC provided conflicting values for MMIO Base! "
                    "(first=%p, second=%p)\n",
                    xapic_phys_base,
                    base);
            return -EEXIST;
...
xapic_setup_mmio(void)
{
    if(!xapic_phys_base_valid)
    {
        eprintk("Reached xAPIC MMIO initialization without a valid xAPIC MMIO "
                "address!\n");
        return -EINVAL;
    }
...
static uint64_t
xapic_read_reg(struct lapic *apic, size_t reg)
{
...
static int
xapic_write_reg(struct lapic *apic, size_t reg, uint64_t val)
{
...
static apic_id_t
xapic_read_id(struct lapic *apic)
{
    return (uint32_t)mmio_readl(xapic_mmio_base + LAPIC_REG_ID) >> 24;
}

static int
xapic_send_ipi(struct lapic *apic,
               apic_id_t target,
               uint8_t vector,
               int message_type,
               int logical,
...
{
    uint32_t icr_low =
        vector | ((uint32_t)message_type << 8) | ((uint32_t)!!logical << 11) |
        ((uint32_t)!!assert << 14) | ((uint32_t)!!trigger_mode << 15);
