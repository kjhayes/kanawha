...
x64_register_volatility_thread(void)
{
    ...
    struct thread_state *volatile_thread = kmalloc(sizeof(struct thread_state));
    ...
    res = thread_init(volatile_thread, x64_volatility_thread, NULL, 0x0);
    if(res)
    {
        return res;
    }
...
declare_init_desc(late,
                  x64_register_volatility_thread,
                  "Starting x64 Volatility Debug Thread");

void
x64_volatility_thread_fail(const char *reg_str,
                           uint64_t corrupt,
                           uint64_t checksum)
{
    panic("x64 Thread Volatility Thread Detected a Corrupted Register! reg=%s, "
          "corrupt=%p, original=%p",
          reg_str,
          corrupt,
          checksum);
}
