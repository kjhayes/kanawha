...
x64_fpu_init_any(void)
{
    ...
    write_mxcsr(MXCSR_INVALID_OP_MASK | MXCSR_DENORMAL_OP_MASK |
                MXCSR_DIV_BY_ZERO_MASK | MXCSR_OVERFLOW_MASK |
                MXCSR_UNDERFLOW_MASK | MXCSR_PRECISION_MASK);

    return 0;
}

int
x64_fpu_init_bsp(void)
{
    return x64_fpu_init_any();
}

int
x64_fpu_init_ap(void)
{
    return x64_fpu_init_any();
}

int
x64_fpu_per_process_init(void)
{
    write_mxcsr(MXCSR_INVALID_OP_MASK | MXCSR_DENORMAL_OP_MASK |
                MXCSR_DIV_BY_ZERO_MASK | MXCSR_OVERFLOW_MASK |
                MXCSR_UNDERFLOW_MASK | MXCSR_PRECISION_MASK);
...
static int
x64_simd_fault_handler(struct excp_state *gen_excp_state,
                       struct irq_action *action)
{
    ...
    if(res)
    {
        eprintk("Failed to deliver PROTFAULT signal to process on SIMD fault "
                "(user_ip=%p) (err=%s)!\n",
                process->user_ip,
                errnostr(res));
...
x64_install_simd_fault_handler(void)
{
    ...
    x64_simd_fault_action = irq_install_handler(x64_vector_irq_desc(19),
                                                NULL, // priv_state
...
declare_init_desc(dynamic,
                  x64_install_simd_fault_handler,
                  "Installing x64 SIMD Fault Handler");
