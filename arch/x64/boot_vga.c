...
#define X64_BOOT_VGA_PRINTK_EARLY_ATTR (X64_BOOT_VGA_ATTR_ORANGE | (X64_BOOT_VGA_ATTR_BLACK << 4))
...
x64_boot_vga_setchar(uint8_t c, uint8_t attr, unsigned x, unsigned y) 
{
    ...
    uint16_t *framebuffer = (void*)__va((void __phys *)X64_BOOT_VGA_FRAMEBUFFER_BASE);
