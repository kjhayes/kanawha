...
#define X64_BOOT_VGA_ATTR_WHITE 0xF
#define X64_BOOT_VGA_ATTR_BLACK 0x0
#define X64_BOOT_VGA_ATTR_GREEN 0x2
#define X64_BOOT_VGA_ATTR_LIME 0xA
#define X64_BOOT_VGA_ATTR_ORANGE 0x6
#define X64_BOOT_VGA_ATTR_YELLOW 0xE

#define X64_BOOT_VGA_PRINTK_EARLY_ATTR                                         \
    (X64_BOOT_VGA_ATTR_ORANGE | (X64_BOOT_VGA_ATTR_BLACK << 4))
...
x64_boot_vga_setchar(uint8_t c, uint8_t attr, unsigned x, unsigned y)
{
    ...
    uint16_t *framebuffer =
        (void *)__va((void __phys *)X64_BOOT_VGA_FRAMEBUFFER_BASE);
...
x64_boot_vga_shift_up(uint8_t c, uint8_t attr)
{
    ...
    for(unsigned y = 1; y < X64_BOOT_VGA_HEIGHT; y++)
    {
        for(unsigned x = 0; x < X64_BOOT_VGA_WIDTH; x++)
        {
            framebuffer[x + ((y - 1) * X64_BOOT_VGA_WIDTH)] =
                framebuffer[x + (y * X64_BOOT_VGA_WIDTH)];
        }
    }
...
static void
x64_boot_vga_newline(uint8_t c, uint8_t attr)
{
