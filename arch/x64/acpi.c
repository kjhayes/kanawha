...
// Devices
static int
parse_madt_lapic(struct acpi_madt *madt, struct acpi_madt_entry_hdr *hdr)
{
    ...
    int res = x64_bsp_register_smp_cpu(cpu, entry->apic_id, is_bsp);
...
static int
parse_madt_x2apic(struct acpi_madt *madt, struct acpi_madt_entry_hdr *hdr)
{
...
static int
parse_madt_ioapic(struct acpi_madt *madt, struct acpi_madt_entry_hdr *hdr)
{
    struct acpi_madt_entry_ioapic *entry = (void *)hdr;
    printk("MADT IOAPIC: id=0x%lx, gsi_base=0x%x\n",
           (apic_id_t)entry->ioapic_id,
           entry->gsi_base);
    int res = x64_register_ioapic(entry->ioapic_id,
...
static int
parse_madt_ioapic_override(struct acpi_madt *madt,
                           struct acpi_madt_entry_hdr *hdr,
                           uint16_t *overriden_isa_irqs)
{
    int res;
    struct acpi_madt_entry_ioapic_override *entry = (void *)hdr;
    printk("MADT IOAPIC Override: bus=0x%x, ioapic-irq=0x%x, pic-irq=0x%x "
           "flags=0x%x\n",
           entry->bus_source,
           entry->gsi,
           entry->irq_source,
    ...
    if(entry->bus_source == ACPI_MADT_IOAPIC_OVERRIDE_BUS_ISA)
    {
        ...
        {
            eprintk("ACPI MADT: IOAPIC Override (Failed to get "
                    "IO/APIC IRQ 0x%x)\n",
                    entry->gsi);
            return -EINVAL;
        }
        ...
        if(ioapic_desc == NULL)
        {
            eprintk("ACPI MADT: IOAPIC Override (Failed to get "
                    "IO/APIC IRQ "
                    "0x%x Descriptor (global IRQ = 0x%x)\n",
                    entry->gsi,
                    ioapic_irq);
            return -EINVAL;
        }
        ...
        if(pic_irq == NULL_IRQ)
        {
            eprintk("ACPI MADT: IOAPIC Override (Failed to get PIC "
                    "IRQ 0x%x)\n",
                    entry->gsi);
            return -EINVAL;
        }
        ...
        if(link == NULL)
        {
            eprintk("ACPI MADT: IOAPIC Override Failed to link "
                    "IO/APIC IRQ "
                    "0x%x to PIC IRQ 0x%x\n",
                    entry->gsi,
                    entry->irq_source);
            return -EINVAL;
        }
    ...
    case 0b11:
        ...
        break;
    default:
        eprintk("Reserved polarity 0b10 in MPS INT Flags of ACPI MADT IRQ "
                "Override!\n");
        return -EINVAL;
    }
    ...
    case 0b11:
        ...
        break;
    default:
        eprintk("Reserved trigger_mode 0b10 in MPS INT Flags of ACPI MADT IRQ "
                "Override!\n");
        return -EINVAL;
    }
...
static int
parse_madt_ioapic_nmi(struct acpi_madt *madt, struct acpi_madt_entry_hdr *hdr)
{
...
static int
parse_madt_lapic_nmi(struct acpi_madt *madt, struct acpi_madt_entry_hdr *hdr)
{
    struct acpi_madt_entry_lapic_nmi *entry = (void *)hdr;
    wprintk("MADT LAPIC NMI: lint=0x%x (Unsupported: Requires Handling ACPI "
            "Processor ID)\n",
            entry->lint);
    return 0;
}
...
x64_parse_acpi_madt(void)
{
    ...
    while(offset < madt->hdr.length)
    {
        ...
        case ACPI_MADT_ENTRY_LAPIC_ADDR_OVERRIDE:
            lapic_address = ((struct acpi_madt_entry_lapic_addr_override *)hdr)
                                ->lapic_address;
            break;
        }
        if(res)
    ...
    if(res)
    {
        eprintk("MADT Failed to provide xAPIC physical base address! "
                "(continuing...)\n");
    }
    ...
    for(size_t i = 0; i < 16; i++)
    {
        ...
        if(pic_desc == NULL)
        {
            wprintk("Failed to identity map PIC IRQ 0x%x to the IO/APIC "
                    "(failed to get PIC IRQ)!\n",
                    (u_t)i);
            continue;
        }
        ...
        if(link == NULL)
        {
            wprintk("Failed to created link from PIC IRQ 0x%x to "
                    "IO/APIC IRQ 0x%x\n",
                    (u_t)i,
                    (u_t)i);
            continue;
...
current_apic_id_from_cpuid(apic_id_t *id)
{
    ...
    case X64_VENDOR_AMD:
        ...
        if(result.ecx & (1UL << 22))
        { // Checking for extended topology support in ECX
            x64_cpuid(0x8000001E,
                      &result); // Getting the extended APIC ID
            *id = result.eax;
            res = 0;
            break;
        }
    ...
    case X64_VENDOR_INTEL:
        ...
        *id = result.edx; // EDX contains the extended APIC ID
                          // (lower 8-bits equivalent to xAPIC id if x2apic
                          // isn't supported)
        res = 0;
        break;
    default:
        eprintk("Do not know how to get APICID using CPUID with vendor "
                "\"%s\"\n",
...
declare_init_desc(static,
                  x64_cache_bsp_apic_id,
                  "Reading BSP APIC ID From CPUID");
