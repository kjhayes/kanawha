...
const static order_t x64_level_num_entries_order[NUM_LEVELS] = {
    9,
    9,
    9,
    9,
...
const static order_t x64_level_entry_order[NUM_LEVELS] = {
    3,
    3,
    3,
    3,
...
const static order_t x64_level_entry_region_order[NUM_LEVELS] = {
    12,
    21,
    30,
    39,
...
const static unsigned long x64_level_flags[NUM_LEVELS] = {
    // PT
...
static size_t
x64_paging_level_addr_table_index(int level, void *addr)
{
    switch(level)
    {
    case 0:
...
static int
x64_paging_get_entry_flags(int level,
                           void *entry_data,
...
static inline int
x64_paging_modify_entry_flags(int level,
                              void *entry_data,
                              unsigned long flags,
                              int clear)
{
    ...
    {
        wprintk("x64_paging_modify_entry_flags: cannot mark entry as "
                "non-readable (ignoring)!\n");
    }
    if(clear && (flags & PAGING_ENTRY_KERNEL_ACCESS))
    {
        wprintk("x64_paging_modify_entry_flags: cannot mark entry as "
                "non-kernel accessible (ignoring)!\n");
    }
...
static int
x64_paging_set_entry_flags(int level, void *entry_data, unsigned long flags)
{
    return x64_paging_modify_entry_flags(level, entry_data, flags, 0);
}

static int
x64_paging_clear_entry_flags(int level, void *entry_data, unsigned long flags)
{
    return x64_paging_modify_entry_flags(level, entry_data, flags, 1);
}

static int
x64_paging_entry_clear(int level, void *entry_data)
{
...
static int
x64_paging_read_entry_addr(int level, void *entry_data, void __phys **addr)
{
...
static int
x64_paging_write_entry_addr(int level, void *entry_data, void __phys *addr)
{
    uint64_t mask = x64_addr_mask(level);
    uint64_t *entry = (uint64_t *)entry_data;
    *entry &= ~mask;
...
static struct paging_mode x64_paging_mode = {
