...
static void
play_that_funky_music_white_boy(
        struct pit_dev *pit)
{
    struct {
        freq_t freq;
        ...
        {
            .freq = FREQ_NOTE_F3,
            .duration = msec_to_duration(500),
        },
        {.freq = 0, .duration=msec_to_duration(100),},
        {
            .freq = FREQ_NOTE_F3,
            .duration = msec_to_duration(500),
        },
        ...
        {.freq = 0, .duration=msec_to_duration(800),},
    ...
    size_t num_notes = sizeof(tune)/sizeof(tune[0]);
    for(size_t i = 0; i < num_notes; i++) {
        pc_speaker_set_tone(pit, tune[i].freq);
        clk_delay(tune[i].duration);
        pc_speaker_set_tone(pit, 0);
...
pit_init(void)
{
    ...
    play_that_funky_music_white_boy(pit);
