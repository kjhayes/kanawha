...
pt_level_num_table_entries(int level)
{
    ...
    {
    case 1:
        return X64_PT_ENTRIES;
    case 2:
        return X64_PD_ENTRIES;
    case 3:
        return X64_PDPT_ENTRIES;
    case 4:
        return X64_PML4_ENTRIES;
    case 5:
        return X64_PML5_ENTRIES;
    default:
...
pt_level_table_size(int level)
{
    ...
    case 2:
        return X64_PD_SIZE;
    case 3:
        return X64_PDPT_SIZE;
    case 4:
        return X64_PML4_SIZE;
    case 5:
        return X64_PML5_SIZE;
    default:
...
pt_level_entry_region_size(int level)
{
    ...
    {
    case 1:
        return X64_PT_ENTRY_REGION_SIZE;
    case 2:
        return X64_PD_ENTRY_REGION_SIZE;
    case 3:
        return X64_PDPT_ENTRY_REGION_SIZE;
    case 4:
        return X64_PML4_ENTRY_REGION_SIZE;
    case 5:
        return X64_PML5_ENTRY_REGION_SIZE;
    default:
...
pt_level_present_mask(int level)
{
    ...
    {
    case 1:
        return X64_PT_LEAF_PRESENT;
    case 2:
        return X64_PD_ENTRY_PRESENT;
    case 3:
        return X64_PDPT_ENTRY_PRESENT;
    case 4:
        return X64_PML4_ENTRY_PRESENT;
    case 5:
        return X64_PML5_ENTRY_PRESENT;
    default:
...
pt_level_shared_mask(int level, uint64_t *out)
{
    ...
    {
    case 2:
        *out = X64_PD_ENTRY_VMEM_SHARED_MAP;
        break;
    case 3:
        *out = X64_PDPT_ENTRY_VMEM_SHARED_MAP;
        break;
    case 4:
        *out = X64_PML4_ENTRY_VMEM_SHARED_MAP;
        break;
    case 5:
        *out = X64_PML5_ENTRY_VMEM_SHARED_MAP;
        break;
    default:
...
__x64_verify_page_table(void __phys *table, int level)
{
    int res;
    ...
    for(size_t i = 0; i < num_entries; i++)
    {
        ...
        if(!pt_level_entry_is_leaf(level, entry))
        {
            ...
            if(res)
            {
                eprintk("Kernel Page Subtable at Level %d is "
                        "Invalid!\n",
                        level - 1);
                return res;
            }
    ...
    {
        wprintk("__x64_verify_page_table: Found empty page table at "
                "level %d\n",
                level);
    }
    return 0;
...
static inline void
x64_verify_page_table(void __phys *table, int level)
{
    ...
    if(res)
    {
        panic("Failed to verify x64 Page Table paddr=%p vaddr=%p, "
              "level=%d\n",
              (uint64_t)table,
              (uint64_t)__va(table),
              level);
...
static int
create_pt_leaf_entry(uint64_t *entry,
                     int pt_level,
                     void __phys *base,
                     unsigned long flags)
{
    ...
    *entry |= (uintptr_t)base;
    ...
    case 1:
        *entry |= X64_PT_LEAF_PRESENT;
        //*entry |= flags & VMEM_REGION_EXEC ? 0 :
        // X64_PT_LEAF_EXEC_DISABLE;
...
static int
create_permissive_pt_table_entry(uint64_t *entry,
                                 int pt_level,
...
static int
create_shared_pt_table_entry(uint64_t *entry,
                             int pt_level,
                             void __phys *next_table)
{
    x64_verify_page_table(next_table, pt_level - 1);

    int res = create_permissive_pt_table_entry(entry, pt_level, next_table);
    if(res)
    {
        return res;
    }
...
static int
create_pt_table_entry(uint64_t *entry,
                      int pt_level,
                      void __phys *next_table,
                      unsigned long flags)
{
...
static int
create_empty_pt_table(void __phys **table_ptr, int table_level)
{
    int res;
...
static int
create_paged_pt_table(void __phys **table_ptr, int table_level, size_t size)
{
    int res;
    ...
    for(size_t i = 0; i < num_whole_entries; i++)
    {
        ...
        res = create_empty_pt_table(&subtable, table_level - 1);
        if(res)
        {
            return res;
        }
        res =
            create_permissive_pt_table_entry(((uint64_t *)__va(*table_ptr)) + i,
                                             table_level,
                                             subtable);
        if(res)
        {
    ...
    if(final_entry_size > 0)
    {
        void __phys *subtable;
        res =
            create_paged_pt_table(&subtable, table_level - 1, final_entry_size);
        if(res)
        {
            return res;
        }
        res = create_permissive_pt_table_entry(((uint64_t *)__va(*table_ptr)) +
                                                   num_whole_entries,
                                               table_level,
                                               subtable);
        if(res)
...
static int
create_direct_pt_table(void __phys **table_ptr,
                       int table_level,
                       void __phys *base,
                       size_t size,
                       unsigned long flags)
{
    ...
    for(size_t i = 0; i < num_whole_entries; i++)
    {
        ...
        {
            res = create_pt_leaf_entry(((uint64_t *)__va(*table_ptr)) + i,
                                       table_level,
        ...
        {
            void __phys *subtable;
            res = create_direct_pt_table(&subtable,
                                         table_level - 1,
                                         base + (entry_region_size * i),
                                         entry_region_size,
                                         flags);
            ...
            res = create_pt_table_entry(((uint64_t *)__va(*table_ptr)) + i,
                                        table_level,
                                        subtable,
                                        flags);
            if(res)
    ...
    if(final_entry_size > 0)
    {
        ...
        res = create_pt_table_entry(((uint64_t *)__va(*table_ptr)) +
                                        num_whole_entries,
                                    table_level,
                                    subtable,
                                    flags);
...
arch_vmem_region_init_direct(struct vmem_region *region)
{
    ...
    if((uintptr_t)region->direct.phys_base % X64_PT_ENTRY_REGION_SIZE != 0)
    {
        eprintk("Tried to initialize direct region with physical base "
                ...
                (uintptr_t)X64_PT_ENTRY_REGION_SIZE);
        return -EINVAL;
    }
    ...
    if(can_be_leaf && region->size == pt_level_entry_size)
    {
        ...
        res = create_pt_leaf_entry(&region->arch_state.pt_entry,
    ...
    else
    {
        ...
        res = create_direct_pt_table(&region->arch_state.pt_table,
                                     region->arch_state.pt_level,
                                     region->direct.phys_base,
                                     region->size,
                                     region->direct.flags);
        ...
        res = create_pt_table_entry(&region->arch_state.pt_entry,
                                    region->arch_state.pt_level + 1,
    ...
    {
        x64_verify_page_table(region->arch_state.pt_table,
                              region->arch_state.pt_level);
    }
    return 0;
}
...
arch_vmem_region_init_paged(struct vmem_region *region)
{
    ...
    if(region->size % X64_PD_ENTRY_REGION_SIZE != 0)
    {
        eprintk("Tried to initialize paged region with size that isn't a "
                ...
                region->size,
                (uintptr_t)X64_PD_ENTRY_REGION_SIZE);
        return -EINVAL;
    }
    ...
    region->arch_state.entry_only = 0;
    res = create_paged_pt_table(&region->arch_state.pt_table,
                                region->arch_state.pt_level,
                                region->size);
    if(res)
    {
    ...
    res = create_permissive_pt_table_entry(&region->arch_state.pt_entry,
                                           region->arch_state.pt_level + 1,
                                           region->arch_state.pt_table);
    if(res)
    {
...
static int
map_region_tables(void __phys *map_table,
                  ...
                  struct vmem_region *region)
{
    ...
    for(size_t vi = vindex; vi < num_possible_entries; vi++)
    {
        ...
        int map_is_leaf = entry_must_be_leaf  ? 1
                          : entry_can_be_leaf ? *map_entry & page_size_mask
                                              : 0;

        int region_is_leaf = entry_must_be_leaf ? 1
                             : entry_can_be_leaf
                                 ? *region_entry & page_size_mask
                                 : 0;
        ...
        void __phys *region_next_addr =
            (void __phys *)(*region_entry & next_table_addr_mask);
        void __phys *map_next_addr =
            (void __phys *)(*map_entry & next_table_addr_mask);
        ...
        if(!(*map_entry & shared_table_mask))
        {
            ...
            res = create_shared_pt_table_entry(map_entry,
                                               table_level,
                                               shared_table);
            if(res)
            {
        ...
        res = map_region_tables(map_next_addr,
                                region_next_addr,
                                table_level - 1,
                                vbase + (entry_region_size * (vi - vindex)),
                                map,
...
static int
free_page_tables(void __phys *table, int level)
{
    int res;
    ...
    for(size_t i = 0; i < num_entries; i++)
    {
        ...
        void __phys *subtable =
            (void __phys *)(pt_level_addr_mask(level) & entry);
...
arch_vmem_region_deinit(struct vmem_region *region)
{
    ...
    {
        return free_page_tables(region->arch_state.pt_table,
                                region->arch_state.pt_level);
    }
}
...
arch_vmem_map_map_region(struct vmem_map *map, struct vmem_region_ref *ref)
{
    ...
    if(pt_level < ref->region->arch_state.pt_level)
    {
        eprintk("arch_vmem_map_map_region: invalid map->pt_level < "
                "region->pt_level (%d < %d)\n",
                (int)pt_level,
                (int)ref->region->arch_state.pt_level);
        return -EINVAL;
    }
    ...
    while(pt_level > ref->region->arch_state.pt_level)
    {
        ...
        if(present_mask & *map_entry)
        {
            ...
            else
            {
                ...
                void __phys *shared_table;
                res =
                    page_alloc(ptr_orderof(next_table_size), &shared_table, 0);
                if(res)
                {
                    return -ENOMEM;
                }
        ...
        else
        {
            ...
            // Create a shared table entry
            res =
                create_shared_pt_table_entry(map_entry, pt_level, shared_table);
            if(res)
            {
                return res;
            }
    ...
    {
        res = map_region_tables(map_table,
                                region_table,
                                pt_level,
                                ref->virt_addr,
                                map,
...
static int
unmap_region_tables(void __phys *map_table,
                    void __phys *region_table,
                    int table_level,
                    void *vbase)
{
        ...
        else
        {
            ...
            else
            {
                ...
                for(size_t below = 0; below < level_below_num_possible_entries;
                    below++)
                {
                    uint64_t *below_entry =
                        ((uint64_t *)__va(map_next_addr)) + below;
                    if(*below_entry != 0)
                    {
                        can_free_table = 0;
                        break;
...
arch_vmem_map_unmap_region(struct vmem_map *map, struct vmem_region_ref *ref)
{
    ...
    if(pt_level < ref->region->arch_state.pt_level)
    {
        eprintk("arch_vmem_map_unmap_region: map->pt_level < region->pt_level "
                "(%d < %d)\n",
                pt_level,
                ref->region->arch_state.pt_level);
        return -EINVAL;
    }
    ...
    {
        size_t index =
            pt_level_table_index(map->arch_state.pt_level, ref->virt_addr);
        map_entry = ((uint64_t *)__va(map_table)) + index;
    ...
    if(ref->region->arch_state.entry_only)
    {
        ...
        case 2:
            ...
            {
                eprintk("Found not-present or non-leaf page when "
                        "unmapping "
                        "entry_only vmem_region!\n");
                return -EINVAL;
            }
            *entry = 0;
            break;
        case 3:
            ...
            *entry = 0;
            break;
        default:
            eprintk("Invalid pt_level for leaf page when unmapping "
                    "vmem_region! (level=%d)\n",
                    pt_level);
            return -EINVAL;
        }
    ...
    {
        res = unmap_region_tables(map_table,
                                  region_table,
                                  pt_level,
                                  ref->virt_addr);
        if(res)
                           ...
                           unsigned long flags)
{
    ...
    if(offset % X64_PT_ENTRY_REGION_SIZE || size % X64_PT_ENTRY_REGION_SIZE)
    {
    ...
    while(size > 0)
    {
        ...
        do
        {
            ...
            else
            {
                ...
                void __phys *subtable;
                res = create_empty_pt_table(&subtable, cur_level - 1);
                if(res)
                {
                    return res;
                }
                ...
                res = create_permissive_pt_table_entry(cur_entry,
                                                       cur_level,
                                                       subtable);
                if(res)
                {
                             ...
                             size_t size)
{
    ...
    while(size > 0)
    {
        ...
        int entry_level;
        if(size >= X64_PDPT_ENTRY_REGION_SIZE &&
           (offset % X64_PDPT_ENTRY_REGION_SIZE == 0) && max_entry_level >= 3)
        {
            page_size = X64_PDPT_ENTRY_REGION_SIZE;
            entries_per_table = X64_PDPT_ENTRIES;
            entry_level = 3;
        ...
        do
        {
            ...
            if(present_mask & *cur_entry)
            {
                // This must be a page table (TODO check this in an
                // assertion)
                uint64_t addr_mask = pt_level_addr_mask(cur_level);
                cur_table = (void __phys *)(*cur_entry & addr_mask);
                DEBUG_ASSERT(!pt_level_entry_is_leaf(cur_level, *cur_entry));
...
#define DUMP_PENDING_LEAF()                                                    \
    ...
    for(size_t entry_index = 0; entry_index < num_entries; entry_index++)
    {
        ...
        {
            uint64_t highmem_mask =
                ~((pt_level_entry_region_size(level) * num_entries) - 1);
...
arch_vmem_map_flush(struct vmem_map *map)
{
    ...
    int res =
        xcall_broadcast(tlb_shootdown_xcall, (void *)map->arch_state.pt_root);
    if(res)
    {
        return res;
    }
