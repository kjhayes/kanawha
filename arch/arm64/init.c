...
static int
arm64_reserve_dtb(struct fdt __phys *dtb)
{
    int res;
...
void
arm64_boot_bsp_init(struct fdt __phys *dtb)
{
    int res;

    arm64_init_current_fpu();
...
arm64_virtual_bsp_init(void)
{
    ...
    extern void arm64_init(void *);
    cpu_start_threading(arm64_init, NULL);
