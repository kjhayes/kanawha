...
#define PAGING_ENTRY_FLAGS_ALL ((1ULL << 8) - 1)
    int (*const get_entry_flags)(int level,
                                 void *entry_data,
                                 unsigned long *flags);
    int (*const set_entry_flags)(int level,
                                 void *entry_data,
                                 unsigned long flags);
    int (*const clear_entry_flags)(int level,
                                   void *entry_data,
                                   unsigned long flags);

    int (*const clear_entry)(int level, void *entry_data);

    int (*const read_entry_addr)(int level,
                                 void *entry_data,
                                 void __phys **addr);
    int (*const write_entry_addr)(int level,
                                  void *entry_data,
                                  void __phys *addr);
};

static inline int
dump_paging_entry_flags(printk_f *printer, unsigned long flags)
{
...
paging_mode_num_levels(const struct paging_mode *mode)
{
...
paging_level_num_entries(const struct paging_mode *mode, int level)
{
...
static inline size_t
paging_level_entry_size(const struct paging_mode *mode, int level)
{
    return 1ULL << paging_level_entry_order(mode, level);
}

static inline order_t
paging_level_table_order(const struct paging_mode *mode, int level)
{
...
static inline size_t
paging_level_table_size(const struct paging_mode *mode, int level)
{
...
static inline size_t
paging_level_entry_region_size(const struct paging_mode *mode, int level)
{
    return 1ULL << paging_level_entry_region_order(mode, level);
}

static inline order_t
paging_level_table_region_order(const struct paging_mode *mode, int level)
{
...
static inline size_t
paging_level_table_region_size(const struct paging_mode *mode, int level)
{
...
static inline size_t
paging_level_addr_table_index(const struct paging_mode *mode,
                              int level,
                              void *addr)
{
...
static inline int
paging_entry_get_flags(const struct paging_mode *mode,
                       int level,
                       void *entry,
                       unsigned long *flags)
{
    return (*mode->get_entry_flags)(level, entry, flags);
}

static inline int
paging_entry_set_flags(const struct paging_mode *mode,
                       int level,
                       void *entry,
                       unsigned long flags)
{
    return (*mode->set_entry_flags)(level, entry, flags);
}

static inline int
paging_entry_clear_flags(const struct paging_mode *mode,
                         int level,
                         void *entry,
                         unsigned long flags)
{
    return (*mode->clear_entry_flags)(level, entry, flags);
}

static inline int
paging_entry_read_addr(const struct paging_mode *mode,
                       int level,
                       void *entry,
                       void __phys **addr)
{
    return (*mode->read_entry_addr)(level, entry, addr);
}

static inline int
paging_entry_write_addr(const struct paging_mode *mode,
                        int level,
                        void *entry,
                        void __phys *addr)
{
    return (*mode->write_entry_addr)(level, entry, addr);
}

static inline int
paging_entry_clear(const struct paging_mode *mode, int level, void *entry_data)
{
