...
#define DECLARE_PERCPU_VAR(__TYPE, __NAME)                                     \
    ...
    __TYPE __percpu *const __percpu_ptr__##__NAME =                            \
        (__TYPE __percpu *const)&__percpu_var__##__NAME;

#define DECLARE_STATIC_PERCPU_VAR(__TYPE, __NAME)                              \
    static __TYPE __percpu_section __percpu_var__##__NAME;                     \
    static __TYPE __percpu *const __percpu_ptr__##__NAME =                     \
        (__TYPE __percpu *const)&__percpu_var__##__NAME;
...
static inline void *volatile percpu_ptr(void __percpu *ptr)
{
...
// Set the percpu area of a remote processor, should allow percpu_ptr_specific
// to work on all processors but not necessarily percpu_ptr on processor
// "remote_id".
int
...
//
// We pass in cur_cpu_id because we cannot use current_cpu_id() until the
// percpu area is initialized, NOTE: This does not mean you can call
// "arch_set_percpu_area" for a remote CPU safely!
//       Once it has been called once, all calls should be semantically the
//       same as "arch_set_percpu_area(current_cpu_id(), area);"
//
int
