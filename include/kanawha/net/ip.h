...
static inline void
ipv4_pkt_hdr_set_version(struct ipv4_pkt_hdr *hdr, uint8_t version)
{
    hdr->version_and_ihl &= 0x0F;
    hdr->version_and_ihl |= ((version << 4) & 0xF0);
}
static inline void
ipv4_pkt_hdr_set_ihl(struct ipv4_pkt_hdr *hdr, uint8_t ihl)
{
    hdr->version_and_ihl &= 0xF0;
    hdr->version_and_ihl |= (ihl & 0x0F);
}
static inline uint16_t
ipv4_pkt_hdr_get_ihl(struct ipv4_pkt_hdr *hdr)
{
    return hdr->version_and_ihl & 0xF;
}
static inline void
ipv4_pkt_hdr_set_header_length(struct ipv4_pkt_hdr *hdr, uint8_t length)
{
    uint8_t ihl = (length / 4) + !!(length % 4);
    ipv4_pkt_hdr_set_ihl(hdr, ihl);
}
static inline uint16_t
ipv4_pkt_hdr_get_header_length(struct ipv4_pkt_hdr *hdr)
{
    uint16_t ihl = ipv4_pkt_hdr_get_ihl(hdr);
    return ihl * 4;
}
static inline void
ipv4_pkt_hdr_set_ecn(struct ipv4_pkt_hdr *hdr, uint8_t ecn)
{
    hdr->ecn_and_dscp &= 0xFC;
    hdr->ecn_and_dscp |= (ecn & 0x3);
}
static inline void
ipv4_pkt_hdr_set_dscp(struct ipv4_pkt_hdr *hdr, uint8_t dscp)
{
    hdr->ecn_and_dscp &= 0x3;
    hdr->ecn_and_dscp |= ((dscp << 2) & 0xFC);
}
static inline void
ipv4_pkt_hdr_set_total_length(struct ipv4_pkt_hdr *hdr, uint16_t len)
{
    hdr->total_length = htobe16(len);
}
static inline void
ipv4_pkt_hdr_set_identification(struct ipv4_pkt_hdr *hdr, uint16_t id)
{
    hdr->identification = htobe16(id);
}
static inline void
ipv4_pkt_hdr_set_flags(struct ipv4_pkt_hdr *hdr, uint8_t flags)
{
    hdr->flags_and_fragment_high &= 0x1F;
    hdr->flags_and_fragment_high |= ((flags << 5) & 0xE0);
}
static inline void
ipv4_pkt_hdr_set_fragment_offset(struct ipv4_pkt_hdr *hdr,
...
static inline void
ipv4_pkt_hdr_set_ttl(struct ipv4_pkt_hdr *hdr, uint8_t ttl)
{
    hdr->ttl = ttl;
}
static inline void
ipv4_pkt_hdr_set_protocol(struct ipv4_pkt_hdr *hdr, uint8_t prot)
{
    hdr->protocol = prot;
}
static inline void
ipv4_pkt_hdr_set_source(struct ipv4_pkt_hdr *hdr, struct ipv4_addr addr)
{
    hdr->src_addr = addr.raw;
}
static inline void
ipv4_pkt_hdr_set_destination(struct ipv4_pkt_hdr *hdr, struct ipv4_addr addr)
{
    hdr->dst_addr = addr.raw;
}
static inline void
ipv4_pkt_hdr_compute_checksum(struct ipv4_pkt_hdr *hdr)
{
