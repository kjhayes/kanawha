...
#define GFX_FORMAT_BYTE_R3G3B2     (16) // Single byte with bit layout  | b1 | b0 | g2 | g1 | g0 | r2 | r1 | r0 |
...
#define GFX_FORMAT_BIT_MONO (19) // Monochrome (BITWISE) (most significant bit on left)
...
struct gfx_layout
{
    ...
    unsigned long offset; // Offset before the first pixel in this layer (byte or bit depending on format)
    unsigned long stride; // Stride between pixels (1 for single (byte or bit depending on format) pixels)
};
