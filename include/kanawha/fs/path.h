...
int
fs_path_create_anonymous(struct fs_node *node, struct fs_path **out);
...
static inline int
fs_path_get_inode_index(struct fs_path *path, size_t *index_out)
{
    if(path == NULL)
    {
        return -EINVAL;
...
static inline int
fs_path_get_inode_attr(struct fs_path *path, int attr, size_t *value_out)
{
    if(path == NULL)
    {
        return -EINVAL;
