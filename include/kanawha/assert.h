...
#define ASSERT_TYPE_SIZE(__type, __size)                                       \
    _Static_assert(sizeof(__type) == (__size),                                 \
                   "sizeof(" #__type ") is not exactly \"" #__size             \
                   "\" bytes!");

#define ASSERT_FIELD_OFFSET(__type, __field, __offset)                         \
    _Static_assert(offsetof(__type, __field) == (__offset),                    \
                   "offsetof(" #__type "." #__field                            \
                   ") is not exactly \"" #__offset "\" bytes!");
...
#define DEBUG_ASSERT_MSG(__COND, __FMT, ...)                                   \
    ...
    {                                                                          \
        if(!(__COND))                                                          \
        {                                                                      \
            panic("Failed Assertion: \"" #__COND "\" (%s:%ld) " __FMT,         \
                  __FILE__,                                                    \
                  (sl_t)__LINE__,                                              \
                  ##__VA_ARGS__);                                              \
        }                                                                      \
...
#define KERNEL_ADDR_HIGHER_HALF_CHECK(ptr)                                     \
    ((((uintptr_t)ptr & (0xF000000000000000ULL)) == (0xF000000000000000ULL)))
#else
...
#define KERNEL_ADDR(ptr)                                                       \
    (((uintptr_t)ptr != 0) && KERNEL_ADDR_HIGHER_HALF_CHECK(ptr) && 1)
#endif
