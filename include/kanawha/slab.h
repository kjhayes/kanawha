 ...
 * - An allocator only depends on large block allocations being available
 *underneath it, either statically allocated in .bss or from the "page_alloc"
 *framework
 *
 * This means we can "boot-strap" allocators such as "page_alloc" by providing
 *some statically allocated memory for it during boot. And that we can use slab
 *allocators to allocate smaller than page sized objects for internal use in
 *"kmalloc" safely. ynam Because of the constrained use-cases of the slab
 *allocator framework here, it should be used when needed. It is designed to be
 *useful even very early during boot, and as such can be fairly slow.
 */
