...
#define TERM_DEV_FLUSH_SIG(RET, ARG, ...) RET(int)
...
struct term_mode
{
    ...
    unsigned
        input_ign_cr : 1; // Ignore all \r which are inputted (after xlation)
    unsigned
        input_ign_nl : 1; // Ignore all \n which are inputted (after xlation)
...
ssize_t
term_driver_write_nonblocking(struct term_dev *dev,
                              void *buffer,
                              size_t buflen);
...
DEFINE_OP_LIST_WRAPPERS(TERM_DEV_OP_LIST,
                        static inline,
                        /* No Prefix */,
                        term_dev,
