...
virtio_pci_cap_ext_cfg_readb(struct virtio_pci_device *dev,
                             struct virtio_pci_cap *cap,
                             uint16_t offset)
{
    DEBUG_ASSERT(offset < cap->cap_len);
    return pci_cap_readb(dev->func, cap->cap, offset + 0x10);
}

static inline uint16_t
virtio_pci_cap_ext_cfg_readw(struct virtio_pci_device *dev,
                             struct virtio_pci_cap *cap,
                             uint16_t offset)
{
    DEBUG_ASSERT(offset < cap->cap_len);
    return pci_cap_readw(dev->func, cap->cap, offset + 0x10);
}

static inline uint32_t
virtio_pci_cap_ext_cfg_readl(struct virtio_pci_device *dev,
                             struct virtio_pci_cap *cap,
                             uint16_t offset)
{
    DEBUG_ASSERT(offset < cap->cap_len);
    return pci_cap_readl(dev->func, cap->cap, offset + 0x10);
}

static inline uint8_t
virtio_pci_cap_bar_readb(struct virtio_pci_device *dev,
                         struct virtio_pci_cap *cap,
                         uint32_t offset)
{
    ...
    dprintk("virtio_pci_cap_bar_readb(offset=0x%lx, cap->offset=0x%lx)\n",
            offset,
            cap->offset);
    return pci_bar_readb(cap->bar, offset + cap->offset);
}
...
{
    DEBUG_ASSERT(offset < cap->length);
    return pci_bar_writeb(cap->bar, offset + cap->offset, val);
}
...
{
    DEBUG_ASSERT(offset < cap->length);
    return pci_bar_writew(cap->bar, offset + cap->offset, val);
}
...
{
    DEBUG_ASSERT(offset < cap->length);
    return pci_bar_writel(cap->bar, offset + cap->offset, val);
}
...
{
    DEBUG_ASSERT(offset < cap->length);
    return pci_bar_writeq(cap->bar, offset + cap->offset, val);
}

// Get/Set Which Queue's Config is Active
static inline uint16_t
virtio_pci_device_get_queue_cfg(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
                                    VIRTIO_PCI_COMMON_CFG_QUEUE_SELECT);
}
static inline int
virtio_pci_device_set_queue_cfg(struct virtio_pci_device *dev,
                                uint16_t queue_no)
{
    virtio_pci_cap_bar_writew(dev,
                              dev->common_cfg_cap,
                              VIRTIO_PCI_COMMON_CFG_QUEUE_SELECT,
                              queue_no);
    return 0;
...
// Queue Size
static inline uint16_t
virtio_pci_device_queue_cfg_get_size(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
...
static inline void
virtio_pci_device_queue_cfg_set_size(struct virtio_pci_device *dev,
                                     uint16_t value)
{
    virtio_pci_cap_bar_writew(dev,
                              dev->common_cfg_cap,
...
// Queue MSI-X
static inline uint16_t
virtio_pci_device_queue_cfg_get_msix(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
...
static inline void
virtio_pci_device_queue_cfg_set_msix(struct virtio_pci_device *dev,
                                     uint16_t value)
{
    virtio_pci_cap_bar_writew(dev,
                              dev->common_cfg_cap,
...
// Queue MSI-X
static inline uint16_t
virtio_pci_device_queue_cfg_get_enabled(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
...
static inline void
virtio_pci_device_queue_cfg_set_enabled(struct virtio_pci_device *dev,
                                        uint16_t value)
{
    virtio_pci_cap_bar_writew(dev,
                              dev->common_cfg_cap,
...
// Queue Notify Offset
static inline uint16_t
virtio_pci_device_queue_cfg_get_notify_offset(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
...
// Queue Notify Data
static inline uint16_t
virtio_pci_device_queue_cfg_get_notify_data(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
...
// Queue Reset
static inline uint16_t
virtio_pci_device_queue_cfg_get_reset(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readw(dev,
                                    dev->common_cfg_cap,
...
static inline void
virtio_pci_device_queue_cfg_set_reset(struct virtio_pci_device *dev,
                                      uint16_t value)
{
    virtio_pci_cap_bar_writew(dev,
                              dev->common_cfg_cap,
...
static inline uint64_t
virtio_pci_device_queue_cfg_get_desc(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readq(dev,
                                    dev->common_cfg_cap,
                                    VIRTIO_PCI_COMMON_CFG_QUEUE_DESC);
}
static inline void
virtio_pci_device_queue_cfg_set_desc(struct virtio_pci_device *dev,
                                     uint64_t value)
{
    virtio_pci_cap_bar_writeq(dev,
                              dev->common_cfg_cap,
                              VIRTIO_PCI_COMMON_CFG_QUEUE_DESC,
                              value);
}
...
static inline uint64_t
virtio_pci_device_queue_cfg_get_device_area(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readq(dev,
                                    dev->common_cfg_cap,
...
static inline void
virtio_pci_device_queue_cfg_set_device_area(struct virtio_pci_device *dev,
                                            uint64_t value)
{
    virtio_pci_cap_bar_writeq(dev,
                              dev->common_cfg_cap,
...
static inline uint64_t
virtio_pci_device_queue_cfg_get_driver_area(struct virtio_pci_device *dev)
{
    return virtio_pci_cap_bar_readq(dev,
                                    dev->common_cfg_cap,
...
static inline void
virtio_pci_device_queue_cfg_set_driver_area(struct virtio_pci_device *dev,
                                            uint64_t value)
{
    virtio_pci_cap_bar_writeq(dev,
                              dev->common_cfg_cap,
...
struct virtio_pci_queue *
virtio_pci_create_queue(struct virtio_pci_device *device, uint16_t queue_no);
int
virtio_pci_destroy_queue(struct virtio_pci_device *device,
