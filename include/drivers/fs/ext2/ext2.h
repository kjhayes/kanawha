...
_Static_assert(sizeof(struct ext2_superblock) == 236,
               "struct ext2_superblock has incorrect size!");
...
#define EXT2_BLOCK_OFFSET(index, blksize) ({ ((index) * blksize); })
...
_Static_assert(sizeof(struct ext2_group_desc) == 0x20,
               "struct ext2_group_desc is the wrong size!");
