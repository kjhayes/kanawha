...
#define usb_xhci_read(xhci_ptr, __NAME)\
    (__usb_xhci_read_ ## __NAME((xhci_ptr)))
...
#define __USB_XHCI_REG_SET_OFFSET_RUNTIME(xhci_ptr) ((xhci_ptr)->runtime_reg_offset)

#define USB_XHCI_DEFINE_READ_REG(__NAME, __SIZE, __REG_SET, __OFFSET)\
static inline uint##__SIZE##_t \
__usb_xhci_read_ ## __NAME(\
        struct usb_xhci *xhci)\
{\
    size_t reg_set_offset = __USB_XHCI_REG_SET_OFFSET_ ## __REG_SET(xhci);\
    __USB_XHCI_LE_TYPE_##__SIZE le;\
    le = __USB_XHCI_PCI_READ_FUNC_ ## __SIZE (\
            &xhci->func->bars[0],\
...
static inline void \
__usb_xhci_write_ ## __NAME(\
        struct usb_xhci *xhci,\
        uint ## __SIZE ## _t __value)\
{\
    size_t reg_set_offset = __USB_XHCI_REG_SET_OFFSET_ ## __REG_SET(xhci);\
    __USB_XHCI_LE_TYPE_##__SIZE le = __USB_XHCI_HOST_TO_LE_##__SIZE(__value);\
    __USB_XHCI_PCI_WRITE_FUNC_ ## __SIZE (\
            &xhci->func->bars[0],\
            reg_set_offset + (__OFFSET),\
            le\
            );\
...
{\
    ...
    DEBUG_ASSERT_MSG((value & ~mask) == 0, \
            ...
            " register "#__REG" field "#__FIELD" (value=%p, width=%d)",\
            (uintptr_t)value, (int)bitwidth);\
    \
