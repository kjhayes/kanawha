...
#define IDT64_ENTRY_FLAG_GATE_TYPE_TRAP                                        \
    ((X64_SEGMENT_TYPE_TRAP_GATE & 0xF) << 8)
#define IDT64_ENTRY_FLAG_GATE_TYPE_INTERRUPT                                   \
    ((X64_SEGMENT_TYPE_INTERRUPT_GATE & 0xF) << 8)
...
_Static_assert(sizeof(struct idt64_entry) == 16,
               "idt64_entry is not exactly 16 bytes wide!");
...
_Static_assert(sizeof(struct gdt64_segment) == 8,
               "sizeof(struct gdt64_segment) is not exactly 8 bytes!");
...
_Static_assert(sizeof(struct gdt64_system_segment) == 16,
               "sizeof(struct gdt64_system_segment) is not exactly 16 bytes!");
...
_Static_assert(sizeof(struct gdt64_call_gate) == 16,
               "sizeof(struct gdt64_system_segment) is not exactly 16 bytes!");
