...
#define ahci_read_ghc(ahci_ptr, __REG)\
    ({\
     le32_t le;\
     le = pci_bar_readl(&(ahci_ptr)->func.bars[5], 0x0 + AHCI_GHC_OFFSET_ ## __REG)\
     letoh32(le);\
     })
...
#define ahci_write_ghc(ahci_ptr, __REG, __VAL)\
    ({\
     ...
     pci_bar_writel(&(ahci_ptr)->func.bars[5], 0x0 + AHCI_GHC_OFFSET_ ## __REG, le)\
     })
...
ahci_pci_ids[] = {
    {
    .class = 0x1, .subclass = 0x6,
    .flags = PCI_ID_CHECK_CLASS|PCI_ID_CHECK_SUBCLASS|PCI_ID_IGNORE_DEVICE|PCI_ID_IGNORE_VENDOR,
    },
};
