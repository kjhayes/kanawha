...
static int
virtio_blk_dev_write(struct blk_dev *dev,
                     void *data,
                     size_t base_sector,
                     size_t num_sectors)
{
    ...
    size_t input_sizes[2] = {sizeof(struct virtio_blk_req),
                             512ULL * num_sectors};

    void *output_datas[1] = {&status};
    size_t output_sizes[1] = {1};

    res = virtio_transact(blk->queue,
                          2,
                          input_datas,
                          input_sizes,
                          1,
...
static int
virtio_blk_dev_read(struct blk_dev *dev,
                    void *data,
                    size_t base_sector,
                    size_t num_sectors)
{
...
static ssize_t
virtio_blk_num_sectors(struct blk_dev *blk_dev)
{
    struct virtio_blk *blk = container_of(blk_dev, struct virtio_blk, blk_dev);
    return blk->num_sectors;
}

static order_t
virtio_blk_sector_order(struct blk_dev *blk_dev)
{
    return 9;
}

static struct blk_driver virtio_blk_driver = {
...
static int
virtio_blk_probe(struct virtio_driver *driver, struct virtio_device *device)
{
...
static int
virtio_blk_negotiate(struct virtio_driver *driver, struct virtio_device *device)
{
...
static int
virtio_blk_init_device(struct virtio_driver *driver,
...
#undef NAMEBUFLEN
    ...
    res = register_blk_dev(&blk->blk_dev, blk->name);
    if(res)
    {
        kfree(blk->name);
        kfree(blk);
...
static int
virtio_blk_deinit_device(struct virtio_driver *driver,
                         struct virtio_device *device)
{
    return -EUNIMPL;
}

static struct virtio_driver_ops virtio_blk_virtio_driver_ops = {
...
static uint16_t virtio_blk_virtio_ids[] = {
    2,
};

static struct virtio_driver virtio_blk_virtio_driver = {
...
declare_init_desc(device,
                  register_virtio_blk_driver,
                  "Registered Virtio Block Driver");
