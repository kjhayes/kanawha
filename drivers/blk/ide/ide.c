...
struct ide_command
{
    ...
    enum {
	IDE_COMMAND_PENDING,  // This is sitting in a queue waiting to be started
	IDE_COMMAND_STARTING, // Registers are being written (and the 400ns delay)
	IDE_COMMAND_RUNNING,  // The device is currently servicing this command (can poll for BSY)
	IDE_COMMAND_COMPLETE, // This command is completed (check error==0 for success)
    } state;
...
__maybe_unused
static inline void
ide_command_target_secondary(
	struct ide_command *cmd)
{
    cmd->drive_head_reg |= (1U<<4);
}
static inline void
ide_command_enable_lba(
	struct ide_command *cmd)
{
...
static inline int
ide_launch_current_command_lockless(
	struct ide_dev *dev)
{
    ...
    if(dev->flags & IDE_DEV_FLAG_LBA48) {
        outb(dev->io_base + IDE_FEATURES_REG_OFFSET,      (cmd->features_reg >> 8) & 0xFF);
        outb(dev->io_base + IDE_SECTOR_COUNT_REG_OFFSET,  (cmd->sector_count_reg >> 8) & 0xFF);
        outb(dev->io_base + IDE_SECTOR_NUMBER_REG_OFFSET, (cmd->sector_number_reg >> 8) & 0xFF);
        outb(dev->io_base + IDE_CYLINDER_LOW_REG_OFFSET,  (cmd->cylinder_low_reg >> 8) & 0xFF);
        outb(dev->io_base + IDE_CYLINDER_HIGH_REG_OFFSET, (cmd->cylinder_high_reg >> 8) & 0xFF);
    } else {
    ...
    outb(dev->io_base + IDE_SECTOR_COUNT_REG_OFFSET, cmd->sector_count_reg & 0xFF);
    outb(dev->io_base + IDE_SECTOR_NUMBER_REG_OFFSET, cmd->sector_number_reg & 0xFF);
    outb(dev->io_base + IDE_CYLINDER_LOW_REG_OFFSET, cmd->cylinder_low_reg & 0xFF);
    outb(dev->io_base + IDE_CYLINDER_HIGH_REG_OFFSET, cmd->cylinder_high_reg & 0xFF);
...
static int
ide_dev_timeout_all_pending_commands(
	struct ide_dev *dev)
{
    int res;
	...
	struct ide_command *cmd = container_of(iter, struct ide_command, list_node);
	cmd->error = 0xFFFF; // TODO
	mbarrier();
	cmd->state = IDE_COMMAND_COMPLETE;
    }
	...
	struct ide_command *cmd)
{
    ...
    if ((status & IDE_STATUS_ERR) || (status & IDE_STATUS_DF)) {
      ...
      if(cmd->buflen < 512) {
          wprintk("ide_handle_running_task: IDE device requested more data but command buffer has no more sectors (cmd->buflen=%d)!\n",
		  (int)cmd->buflen);
      ...
      if(cmd->buflen > 0) {
          wprintk("ide_handle_running_task: IDE command finished but buffer still has 0x%x bytes! (status=0x%x)\n",
    ...
    while(1) {
        ...
        if(cmd == NULL || cmd->state == IDE_COMMAND_COMPLETE) {
            ...
            while(1) {
                ...
                if(res) {
                    wprintk("ide_check_current_command_task: Failed to launch next command! (err=%s)\n",
            		errnostr(res));
                    cmd->error = 0xFFFF; // TODO this should be a more specific error condition
                    cmd->state = IDE_COMMAND_COMPLETE;
            	    continue; // Try again
                }
            }
            ...
            // so the BSY signal might not be asserted, but that does not
            // mean the command has actually finished
	...
	struct ide_dev *dev)
{
	...
	// Floating bus, neither drive exists
        dev->flags &= ~(IDE_DEV_FLAG_PRIMARY_EXISTS | IDE_DEV_FLAG_SECONDARY_EXISTS);
	return 0;
    }
...
#undef ATTEMPTS
	...
	if (buffer[83] & (1<<10)) {
		...
		wprintk("IDE mismatch: secondary drive supports LBA48 but primary does not!\n");
	    ...
	    if(dev->flags & IDE_DEV_FLAG_LBA48) {
		wprintk("IDE mismatch: primary drive supports LBA48 but secondary does not!\n");
	    }
	    dev->flags &= ~IDE_DEV_FLAG_LBA48;
	}
	...
	struct ide_dev **out)
{
    ...
    if(!(dev->flags &
	(IDE_DEV_FLAG_PRIMARY_EXISTS|
	 IDE_DEV_FLAG_SECONDARY_EXISTS)))
    {
	kfree(dev);
	return res;
    }
	...
	size_t num_sectors)
{
    ...
    struct ide_dev *dev = container_of(blk_dev, struct ide_dev, primary_blk_dev);

#define MAX_SECTORS_PER_WRITE 256
    _Static_assert(MAX_SECTORS_PER_WRITE <= 256, "IDE Device cannot read/write more than 256 sectors at once!");

    size_t num_writes = (num_sectors / MAX_SECTORS_PER_WRITE) + !!(num_sectors % MAX_SECTORS_PER_WRITE);
    ...
    { // Bounds Checking
      ...
      else if(!(dev->flags & IDE_DEV_FLAG_LBA48) && (max_sector_offset > ((1ULL<<28)-1))) {
	...
	cmd.sector_number_reg = ((sec >> 0) & 0xFF) | (((sec >> 24)  & 0xFF) << 8);
	cmd.cylinder_low_reg = ((sec >> 8) & 0xFF) | (((sec >> 32)  & 0xFF) << 8);
	cmd.cylinder_high_reg = ((sec >> 16) & 0xFF) | (((sec >> 40)  & 0xFF) << 8);
	cmd.features_reg = 0;
	cmd.drq_write = 1;
	cmd.ignore_rdy = 0;
...
#define MAX_SECTORS_PER_READ 256
    _Static_assert(MAX_SECTORS_PER_READ <= 256, "IDE Device cannot read/write more than 256 sectors at once!");

    size_t num_reads = (num_sectors / MAX_SECTORS_PER_READ) + !!(num_sectors % MAX_SECTORS_PER_READ);
...
static ssize_t
ide_primary_blk_dev_num_sectors(
	struct blk_dev *blk_dev)
{
    struct ide_dev *dev = container_of(blk_dev, struct ide_dev, primary_blk_dev);
...
static order_t
ide_primary_blk_dev_sector_order(
	struct blk_dev *blk_dev)
{
...
static int
ide_secondary_blk_dev_write(
	struct blk_dev *blk_dev,
	void *data,
	size_t base_sector,
	size_t num_sectors)
{
    struct ide_dev *dev = container_of(blk_dev, struct ide_dev, secondary_blk_dev);
    return -EUNIMPL;
}

static int
ide_secondary_blk_dev_read(
	struct blk_dev *blk_dev,
	void *data,
	size_t base_sector,
	size_t num_sectors)
...
static ssize_t
ide_secondary_blk_dev_num_sectors(
	struct blk_dev *blk_dev)
{
    struct ide_dev *dev = container_of(blk_dev, struct ide_dev, secondary_blk_dev);
...
static order_t
ide_secondary_blk_dev_sector_order(
	struct blk_dev *blk_dev)
{
    // All IDE Devices have 512 byte sectors
    return 9;
