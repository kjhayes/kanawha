...
static int
try_match_device(struct usb_device_driver *driver, struct usb_device *device)
{
    int res;
    ...
    res = usb_device_driver_configure_device(driver, device, &chosen_config);
    if(res)
    {
        return res;
    }
    ...
    {
        eprintk("USB device driver requested non-existant configuration %d! "
                "(num_configs=%d)\n",
                (int)chosen_config,
...
static int
try_match_interface(struct usb_interface_driver *driver,
...
register_usb_interface(struct usb_interface *interface)
{
    ...
    if(!matched)
    {
        ilist_push_tail(&unmatched_interfaces_list, &interface->match_node);
    }
...
static int
usb_unknown_device_probe(struct usb_device_driver *driver,
                         struct usb_device *device)
{
...
static int
usb_unknown_device_configure(struct usb_device_driver *driver,
                             struct usb_device *device,
                             size_t *req_config)
{
...
static int
usb_unknown_device_init(struct usb_device_driver *driver,
                        struct usb_device *device)
{
...
static int
usb_unknown_device_deinit(struct usb_device_driver *driver,
                          struct usb_device *device)
{
    int res;
    for(size_t i = 0; i < device->configs[0].num_interfaces; i++)
...
struct usb_device_driver_ops usb_unknown_device_driver_ops = {
...
static struct usb_device_driver usb_unknown_device_driver = {
