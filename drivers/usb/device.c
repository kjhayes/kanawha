...
usb_host_register_device(struct usb_device *device, struct usb_device_ops *ops)
{
    ...
    res = usb_device_get_descriptor(device,
                                    1,
                            ...
                            size_t buflen)
{
    ...
    setup = usb_device_create_setup_stage_transfer(device,
                                                   dci,
                                                   bmRequestType,
                                                   bRequest,
                                                   wValue,
    ...
    data = usb_device_create_data_stage_transfer(device,
                                                 dci,
                                                 buffer,
                                                 buflen,
                                                 data_dir);
    ...
    status = usb_device_create_status_stage_transfer(device, dci, status_dir);
    if(status == NULL)
    {
        return -EINVAL;
    }
