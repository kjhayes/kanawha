...
static int
usb_host_init_device_configurations(struct usb_device *device)
{
    int res;
    ...
    for(size_t ci = 0; ci < device->num_configs; ci++)
    {
        ...
        {
            wprintk(
                "USB Device: Configuration %d descriptor has invalid type!\n",
                (int)ci);
            continue;
        }
        ...
        {
            kfree(buffer);
            wprintk("USB Device: Configuration %d Failed to Read Full "
                    "Descriptor! (err=%s)\n",
                    (int)ci,
                    errnostr(res));
        ...
        config->interfaces =
            kzmalloc(sizeof(struct usb_interface) * config->num_interfaces,
                     KM_KERNEL);
        if((config->interfaces == NULL) && (config->num_interfaces > 0))
        {
            config->num_interfaces = 0;
            wprintk("Failed to allocate buffer for USB device configuration "
                    "interface info!\n");
            continue;
        }
        ...
        for(size_t ii = 0; ii < config->num_interfaces; ii++)
        {
            ...
            interface->interface_endpoints =
                kzmalloc(sizeof(struct usb_interface_endpoint) *
                             interface->num_interface_endpoints,
                         KM_KERNEL);
            if((interface->interface_endpoints == NULL) &&
               (interface->num_interface_endpoints > 0))
            {
                interface->num_interface_endpoints = 0;
                wprintk("Failed to allocate buffer for USB device interface "
                        "endpoint info!\n");
                continue;
            }
            ...
            {
                struct usb_interface_endpoint *endpoint =
                    &interface->interface_endpoints[ei];
...
usb_host_init_device(struct usb_device *device, struct usb_device_ops *ops)
{
    ...
    {
        device->configs =
            kzmalloc(sizeof(struct usb_configuration) * device->num_configs,
                     KM_KERNEL);
                            ...
                            size_t bufsize)
{
    ...
    xfer = usb_device_create_control_transfer(device,
                                              endpoint,
                                              bmRequestType,
                                              bRequest,
                                              wValue,
