...
static int
usb_xhci_interruptor_set_segment_table_size(
        struct usb_xhci_interruptor *intr,
        uint16_t entries)
{
    pci_bar_writel(
...
static int
usb_xhci_interruptor_set_dequeue_pointer(
        struct usb_xhci_interruptor *intr,
        void __phys *ptr,
        ...
        struct usb_xhci_trb *trb)
{
    ...
    switch(type) {
        ...
        case USB_XHCI_TRB_TYPE_MFINDEX_WRAP_EVENT:
            wprintk("Received USB Event of Unimplemented Type %s (code=%s) (Ignoring...)\n",
        ...
        struct usb_xhci_interruptor *intr)
{
    ...
    do {
        ...
        struct usb_xhci_trb *cur_segment = dma_virt_addr(intr->segments[intr->dequeue_segment]);
        ...
        if(!cycle == !intr->ccs)
        {
            ...
            if(intr->dequeue_index >= intr->trbs_per_segment) {
                ...
                if(intr->dequeue_segment >= intr->num_segments) {
                    dprintk("USB XHCI Event Queue Flipping CCS Bit...\n");
                    intr->ccs ^= 1;
    ...
    if(dequeued_a_trb) {
        ...
        struct usb_xhci_trb __phys *phys_seg = dma_phys_addr(intr->segments[segment]);
...
static int
usb_xhci_interruptor_irq_handler(
        ...
        size_t index)
{
    ...
    intr->xhci = dev;
    intr->index = index;
    intr->register_offset = (intr->xhci->runtime_reg_offset + 0x20) + (32 * index);
    ...
    intr->action = irq_install_handler(desc, intr, usb_xhci_interruptor_irq_handler);
    if(intr->action == NULL) {
        return -EINVAL;
    }
    ...
    if(intr->trbs_per_segment > 1ULL<<16) {
        // Too large pages? (We could recover from this but for now assume this won't happen)
        irq_uninstall_action(intr->action);
        return -EINVAL;
    }
    ...
    memset(erst_ptr, 0, sizeof(struct usb_xhci_erst_entry) * intr->num_segments);
    ...
    for(size_t i = 0; i < intr->num_segments; i++) {
        ...
        erst[i].segment_phys = htole64((uint64_t)dma_phys_addr(intr->segments[i]));
    ...
    if(res) {
        ...
        kfree(intr->segments);
        dma_free(intr->segment_table, sizeof(struct usb_xhci_erst_entry) * intr->num_segments);
        irq_uninstall_action(intr->action);
        return res;
    }
