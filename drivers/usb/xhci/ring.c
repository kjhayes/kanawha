                       ...
                       size_t size)
{
    ...
    ring->dma_regions =
        kmalloc(sizeof(dma_addr_t) * ring->num_dma_regions, KM_KERNEL);
    ...
    {
        struct usb_xhci_trb *from_trbs =
            dma_virt_addr(ring->dma_regions[i - 1]);
    ...
    {
        struct usb_xhci_trb *final_trbs =
            dma_virt_addr(ring->dma_regions[ring->num_dma_regions - 1]);
        struct usb_xhci_trb *link_trb = &final_trbs[trbs_per_page];
        link_trb->param = htole64((uintptr_t)start_addr);
        link_trb->status = 0;
        link_trb->control = 0;
        link_trb->control |=
            htole32(1ULL << 1); // Toggle the meaning of the cycle
                                // bit when we cross this boundary
        usb_xhci_trb_set_type(link_trb, USB_XHCI_TRB_TYPE_LINK);
    }
