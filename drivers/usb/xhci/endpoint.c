...
static int
usb_xhci_normal_transfer_launch(
        struct usb_transfer *gen_xfer)
{
    ...
    //ilist_push_tail(&xfer->endpoint->transfer_queue, &xfer->endpoint_queue_node);
...
static struct usb_transfer_ops
usb_xhci_normal_transfer_ops = {
    .launch = usb_xhci_normal_transfer_launch,
};

static int
usb_xhci_setup_stage_transfer_launch(
        struct usb_transfer *gen_xfer)
{
    int res;
    ...
    param =
        (((uint64_t)xfer->setup_stage.bmRequestType) << 0) |
    ...
    trb->control = (htole32(control & ~0b1) & ~0b1) | (trb->control & htole32(0b1));

    ilist_push_tail(&xfer->endpoint->transfer_queue, &xfer->endpoint_queue_node);
    usb_transfer_set_status(&xfer->xfer, USB_TRANSFER_STATUS_LAUNCHED);

    res = usb_xhci_trb_ring_advance_enqueued(
            &xfer->endpoint->ring,
            1);
    if(res) {
        usb_transfer_set_status(&xfer->xfer, res);
        ilist_remove(&xfer->endpoint->transfer_queue, &xfer->endpoint_queue_node);
        irq_lock_release(&xfer->endpoint->lock);
        return 0;
    }
...
static struct usb_transfer_ops
usb_xhci_setup_stage_transfer_ops = {
    .launch = usb_xhci_setup_stage_transfer_launch,
};

static int
usb_xhci_data_stage_transfer_launch(
        struct usb_transfer *gen_xfer)
{
    struct usb_xhci_transfer *xfer = container_of(gen_xfer, struct usb_xhci_transfer, xfer);
    irq_lock_acquire(&xfer->endpoint->lock);
...
static struct usb_transfer_ops
usb_xhci_data_stage_transfer_ops = {
    .launch = usb_xhci_data_stage_transfer_launch,
};

static int
usb_xhci_status_stage_transfer_launch(
        struct usb_transfer *gen_xfer)
{
    struct usb_xhci_transfer *xfer = container_of(gen_xfer, struct usb_xhci_transfer, xfer);
    irq_lock_acquire(&xfer->endpoint->lock);
...
static struct usb_transfer_ops
usb_xhci_status_stage_transfer_ops = {
    .launch = usb_xhci_status_stage_transfer_launch,
};

static int
usb_xhci_isoch_transfer_launch(
        struct usb_transfer *gen_xfer)
{
    struct usb_xhci_transfer *xfer = container_of(gen_xfer, struct usb_xhci_transfer, xfer);
    irq_lock_acquire(&xfer->endpoint->lock);
...
usb_xhci_endpoint_create_normal_transfer(
        ...
        size_t buflen)
{
    struct usb_xhci_transfer *xfer = usb_xhci_alloc_transfer(endp, USB_TRANSFER_NORMAL);
...
usb_xhci_endpoint_create_setup_stage_transfer(
        ...
        int trt)
{
    struct usb_xhci_transfer *xfer = usb_xhci_alloc_transfer(endp, USB_TRANSFER_SETUP_STAGE);
...
usb_xhci_endpoint_create_data_stage_transfer(
        ...
        int dir)
{
    struct usb_xhci_transfer *xfer = usb_xhci_alloc_transfer(endp, USB_TRANSFER_DATA_STAGE);
...
usb_xhci_endpoint_create_status_stage_transfer(
        ...
        int dir)
{
    struct usb_xhci_transfer *xfer = usb_xhci_alloc_transfer(endp, USB_TRANSFER_STATUS_STAGE);
...
usb_xhci_endpoint_create_isoch_transfer(
        struct usb_xhci_endpoint *endp)
{
    struct usb_xhci_transfer *xfer = usb_xhci_alloc_transfer(endp, USB_TRANSFER_ISOCH);
