...
uint8_t
usb_xhci_trb_get_type(
        struct usb_xhci_trb *trb)
{
    return (trb->control >> 10) & 0x3F;
}
void
usb_xhci_trb_set_type(
        struct usb_xhci_trb *trb,
        uint8_t value)
{
...
static int
usb_xhci_trb_completion_code_is_vendor_error(
        uint8_t type)
{
    return (type >= 192 && type <= 223);
}
static int
usb_xhci_trb_completion_code_is_vendor_info(
        uint8_t type)
{
...
int
usb_xhci_trb_completion_code_is_success(
        uint8_t value)
{
...
const char *
usb_xhci_trb_completion_code_to_string(
        uint8_t type)
{
    switch(type) {
#define CASE(__NAME, __VAL)\
        case __VAL: return #__NAME;
