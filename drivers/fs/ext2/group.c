...
static int
ext2_mount_read_group_desc(struct ext2_mount *mnt,
                           size_t grp_index,
                           struct ext2_group_desc *desc)
{
    ...
    size_t desc_list_offset =
        EXT2_BLOCK_OFFSET(table_block_index, mnt->block_size);
    size_t desc_offset =
        desc_list_offset + (sizeof(struct ext2_group_desc) * grp_index);

    dprintk(
        "ext2_mount_read_group_desc(mnt=%p, grp_index=0x%lx) desc_offset=%p\n",
        mnt,
        grp_index,
        desc_offset);
    res = fs_node_paged_read(mnt->backing_node,
                             desc_offset,
                             desc,
...
static int
ext2_mount_write_group_desc(struct ext2_mount *mnt,
                            size_t grp_index,
                            struct ext2_group_desc *desc)
{
    int res;
    ...
    res = fs_node_paged_write(mnt->backing_node,
                              desc_offset,
                              desc,
                              sizeof(struct ext2_group_desc),
                              0);
...
struct ext2_group *
ext2_get_group(struct ext2_mount *mnt, size_t index)
{
    int res;
    ...
    if(mnt->group_cache[index] == NULL)
    {
        struct ext2_group *group =
            kmalloc(sizeof(struct ext2_group), KM_KERNEL);
        if(group == NULL)
        {
        ...
        res = ext2_mount_read_group_desc(mnt, index, &group->desc);
        if(res)
        {
            kfree(group);
            spin_unlock(&mnt->group_cache_lock);
...
static int
ext2_group_populate_blk_bitmap(struct ext2_group *grp)
{
    int res;
    ...
    res = fs_node_paged_read(grp->mnt->backing_node,
                             bitmap_offset,
                             grp->blk_bitmap,
                             grp->mnt->block_size,
                             0);
...
static int
ext2_group_flush_blk_bitmap(struct ext2_group *grp)
{
    int res;
    ...
    res = fs_node_paged_write(grp->mnt->backing_node,
                              bitmap_offset,
                              grp->blk_bitmap,
                              grp->mnt->block_size,
                              0);
...
static int
ext2_group_populate_inode_bitmap(struct ext2_group *grp)
{
    int res;
...
static int
ext2_group_flush_inode_bitmap(struct ext2_group *grp)
{
    int res;
...
int
ext2_put_group(struct ext2_mount *mnt, struct ext2_group *group)
{
    int res;
    ...
    if(group->refs <= 0)
    {
        ...
        if(res)
        {
            wprintk("Failed to flush ext2 group! (may corrupt disk but "
                    "continuing...)\n");
            // :(((
        }
...
int
ext2_flush_group(struct ext2_mount *mnt, struct ext2_group *group)
{
    int res;
    ...
    {
        group->desc_dirty = 0;
        res = ext2_mount_write_group_desc(mnt, group->index, &group->desc);
        if(res)
        {
            eprintk("Failed to write EXT2 group descriptor back to "
                    "disk! (err=%s)\n",
                    errnostr(res));
            return res;
        }
    ...
    if(res)
    {
        eprintk("Failed to write EXT2 group block bitmap back to disk! "
                "(err=%s)\n",
                errnostr(res));
        return res;
    }
    ...
    if(res)
    {
        eprintk("Failed to write EXT2 group inode bitmap back to disk! "
                "(err=%s)\n",
                errnostr(res));
        return res;
    }
...
// inode Bitmap
static int
ext2_group_inode_bitmap_check(struct ext2_group *group,
                              size_t rel_index,
                              int *value)
{
    int res;
...
int
ext2_group_read_inode(struct ext2_group *group,
                      size_t rel_index,
                      struct ext2_inode *inode)
{
    ...
    size_t size = group->mnt->inode_size < sizeof(struct ext2_inode)
                      ? group->mnt->inode_size
                      : sizeof(struct ext2_inode);

    res = fs_node_paged_read(group->mnt->backing_node,
                             inode_offset,
                             inode,
                             size,
                             0);
...
int
ext2_group_write_inode(struct ext2_group *group,
                       size_t rel_index,
                       struct ext2_inode *inode)
{
    int res;
    ...
    res = fs_node_paged_write(group->mnt->backing_node,
                              inode_offset,
                              inode,
                              size,
                              0);
...
int
ext2_group_alloc_inode(struct ext2_group *group, size_t *inode_out)
{
    int res;
    ...
    *inode_out = (group->index * group->mnt->inodes_per_group) + (bit + 1);
    ...
    // Clear the inode
    struct ext2_inode inode;
    res = ext2_group_read_inode(group, bit, &inode);
    if(res)
    {
        spin_unlock(&group->inode_lock);
        return res;
    ...
    res = ext2_group_write_inode(group, bit, &inode);
    if(res)
    {
        spin_unlock(&group->inode_lock);
        return res;
...
int
ext2_group_free_inode(struct ext2_group *group, size_t inode)
{
    int res;

    size_t rel_index =
        (inode - 1) - (group->mnt->inodes_per_group * group->index);
    if(rel_index > group->mnt->inodes_per_group)
    {
        return -EINVAL;
    }
...
int
ext2_group_alloc_block(struct ext2_group *group, size_t *blk_out)
{
    int res;
    ...
    size_t bit = bitmap_find_clear_range(group->blk_bitmap,
                                         group->mnt->blks_per_group,
                                         1);
...
int
ext2_group_free_block(struct ext2_group *group, size_t block_index)
{
    int res;

    size_t rel_index =
        block_index - (group->index * group->mnt->blks_per_group);
    if(rel_index > group->mnt->blks_per_group)
    {
        return -EINVAL;
    }
...
int
ext2_group_inode_allocated(struct ext2_group *group,
                           ...
                           int *value)
{
    ...
    size_t rel_index =
        abs_block_index - (group->index * group->mnt->inodes_per_group);
    if(rel_index > group->mnt->inodes_per_group)
    {
        return -EINVAL;
    }

    res = ext2_group_inode_bitmap_check(group,
