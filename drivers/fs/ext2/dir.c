...
static int
ext2_dir_read_at(struct fs_node *fs_node,
                 size_t offset,
...
static int
ext2_dir_write_at(struct fs_node *fs_node,
                  size_t offset,
                  struct ext2_linked_dir_entry *out)
{
    struct ext2_fs_node *node = fs_node->backing.priv_state;
...
static int
ext2_dir_add_linked_entry(struct fs_node *backing_node,
                          ...
                          const char *name)
{
    ...
    if(inode == 0)
    {
        wprintk("ext2_dir_add_linked_entry: request to add link to reserved "
                "inode=0! (name=%s)\n",
                name);
        return -EINVAL;
    }
    ...
    size_t offset_into_block =
        offset - (parent_node->mount->block_size * offset_block_no);
    size_t room_left_in_block =
        parent_node->mount->block_size - offset_into_block;

    if(room_left_in_block < min_len_req)
    {
        // If there is not enough room left in the block, move to the next
        // block offset.

        if(prev_offset > 0)
        {
            // We need to patch the previous entry to extend the
            // rec_len filling out the entire block
    ...
    res = fs_node_paged_write(backing_node,
                              offset + 8,
                              (void *)name,
                              namelen,
                              FS_NODE_PAGED_WRITE_MAY_EXTEND);
...
static int
ext2_dir_read_cur(struct file *file, struct ext2_linked_dir_entry *out)
{
    ...
    return ext2_dir_read_at(fs_node, file->dir_offset, out);
}

static int
ext2_dir_next(struct file *file)
{
    int res;
...
static int
ext2_dir_begin(struct file *file)
{
    int res;
    file->dir_offset = 0;
...
static int
ext2_dir_readattr(struct file *file, int attr, size_t *value)
{
    return -EUNIMPL;
}

static int
ext2_dir_readname(struct file *file, char *buf, size_t buflen)
{
    int res;
...
int
ext2_dir_node_lookup(struct fs_node *fs_node,
                     const char *name,
                     size_t *inode,
                     char *sym_buffer,
                     size_t sym_buflen)
{
    ...
    while(1)
    {
        dprintk("ext2_dir_node_lookup: checking directory entry at "
                "offset=%p\n",
                offset);
        res = ext2_dir_read_at(fs_node, offset, &entry);
        if(res)
        {
            return res;
        }
...
static int
ext2_dir_mkfile(struct fs_node *fs_node,
                const char *filename,
                unsigned long flags)
{
    int res;
    ...
    size_t inode;
    res = ext2_mount_alloc_inode(node->mount,
    ...
    if(inode == 0)
    {
        wprintk("ext2_dir_mkfile: ext2_mount_alloc_inode returned reserved "
                "inode=0!\n");
        return -EINVAL;
    }
    ...
    inode_data.mode = 0x8000    // directory
                      | (0666); // R/W for everyone
    inode_data.links_count = 1;

    res = ext2_mount_write_inode_data(node->mount, inode, &inode_data);
    if(res)
    {
    ...
    res = ext2_dir_add_linked_entry(fs_node,
                                    node,
                                    inode,
    ...
    if(res)
    {
        ext2_mount_free_inode(node->mount, inode);
        return res;
    }
...
static int
ext2_dir_mkdir(struct fs_node *parent_fs_node,
               const char *filename,
               unsigned long flags)
{
    int res;
    ...
    size_t inode;
    res = ext2_mount_alloc_inode(parent_node->mount, group_num, &inode);
    if(res)
    {
        eprintk("EXT2: Failed to allocate inode! (err=%s)\n", errnostr(res));
        return res;
    ...
    if(inode == 0)
    {
        wprintk("ext2_dir_mkdir: ext2_mount_alloc_inode returned reserved "
                "inode=0!\n");
        return -EINVAL;
    }
    ...
    struct ext2_inode inode_data;
    res = ext2_mount_read_inode_data(parent_node->mount, inode, &inode_data);
    if(res)
    {
    ...
    memset(&inode_data, 0, sizeof(struct ext2_inode));
    inode_data.links_count = 1;
    inode_data.mode = 0x4000    // directory
                      | (0666); // R/W for everyone
    inode_data.links_count = 1;

    res = ext2_mount_write_inode_data(parent_node->mount, inode, &inode_data);
    if(res)
    {
        eprintk("EXT2: Failed to write allocated inode! (err=%s)\n",
                errnostr(res));
    ...
    {
        eprintk("EXT2: Failed to get fs_node of newly created directory! "
                "(err=%s)\n",
                errnostr(res));
        ext2_mount_free_inode(parent_node->mount, inode);
        return res;
    ...
    res = ext2_dir_add_linked_entry(child_fs_node,
                                    child_node,
                                    inode,
                                    EXT2_DIR_FT_DIR,
                                    ".");
    ...
    res = ext2_dir_add_linked_entry(parent_fs_node,
                                    parent_node,
                                    inode,
                                    EXT2_DIR_FT_DIR,
                                    filename);
    if(res)
    {
        eprintk("EXT2: mkdir failed to add linked entry to directory! "
                "(err=%s)\n",
                errnostr(res));
        ext2_mount_free_inode(parent_node->mount, inode);
        return res;
...
static int
ext2_dir_unlink(struct fs_node *parent_fs_node, const char *name)
{
    int res;
...
int
ext2_dir_getattr(struct fs_node *fs_node, int attr, size_t *value)
{
    struct ext2_fs_node *node = fs_node->backing.priv_state;
...
int
ext2_dir_setattr(struct fs_node *fs_node, int attr, size_t value)
{
    struct ext2_fs_node *node = fs_node->backing.priv_state;
...
struct fs_node_ops ext2_dir_node_ops = {
...
struct fs_file_ops ext2_dir_file_ops = {
