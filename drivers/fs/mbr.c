...
static int
mbr_mount_load_partition_entry(struct mbr_mount *mnt,
                               size_t index,
                               struct mbr_partition_entry *entry)
{
    ...
    if(index > 0 && index <= 4)
    {
        offset = MBR_PARTITION_ENTRY_OFFSET +
                 (sizeof(struct mbr_partition_entry) * (index - 1));
    }
    else
    {
        return -ENXIO;
    ...
    res =
        fs_node_paged_read(mnt->backing_node, offset, entry, sizeof(*entry), 0);
    if(res)
    {
        return res;
    }
...
static int
mbr_mount_load_node(struct fs_mount *fs_mount,
                    size_t node_index,
                    struct fs_node *fs_node)
{
    int res;
...
static int
mbr_mount_unload_node(struct fs_mount *fs_mount,
                      size_t index,
                      struct fs_node *fs_node)
{
    int res;
...
static int
mbr_mount_root_index(struct fs_mount *mnt, size_t *index)
{
    int res;
    *index = 0;
    return 0;
...
static int
mbr_mount_sync(struct fs_mount *fs_mount)
{
    int res;
...
int
mbr_mount_file(struct fs_type *fs_type,
               struct fs_node *backing_node,
               struct fs_mount **out_ptr)
{
    ...
    res = init_fs_mount_struct(&mnt->fs_mount, &mbr_fs_mount_ops);
    if(res)
    {
        goto err0;
    }
...
static int
mbr_unmount(struct fs_type *type, struct fs_mount *fs_mount)
{
    struct mbr_mount *mnt = container_of(fs_mount, struct mbr_mount, fs_mount);
    fs_node_put(mnt->backing_node);
    kfree(mnt);
...
static int
mbr_root_dir_getattr(struct fs_node *fs_node, int attr, size_t *value)
{
...
static int
mbr_root_dir_setattr(struct fs_node *fs_node, int attr, size_t value)
{
    struct mbr_mount *mnt = fs_node->backing.priv_state;
...
static int
mbr_root_dir_lookup(struct fs_node *fs_node,
                    const char *name,
                    size_t *inode,
                    char *sym_buffer,
                    size_t sym_buflen)
...
int
mbr_root_dir_begin(struct file *file)
{
    int res;
...
int
mbr_root_dir_next(struct file *file)
{
    int res;
...
int
mbr_root_dir_readattr(struct file *file, int attr, size_t *value)
{
    int res;
...
int
mbr_root_dir_readname(struct file *file, char *buffer, size_t buflen)
{
    int res;
...
static int
mbr_partition_getattr(struct fs_node *fs_node, int attr, size_t *value)
{
    struct mbr_partition *part = fs_node->backing.priv_state;
    struct mbr_mount *mnt =
        container_of(fs_node->mount, struct mbr_mount, fs_mount);
...
static int
mbr_partition_setattr(struct fs_node *fs_node, int attr, size_t value)
{
    struct mbr_mount *mnt = fs_node->backing.priv_state;
...
static int
mbr_unaligned_partition_read_page(struct fs_node *fs_node,
                                  void *page,
                                  uintptr_t pfn,
                                  unsigned long flags)
{
    ...
    return fs_node_paged_read(mnt->backing_node,
                              offset,
                              page,
                              1ULL << mnt->page_order,
                              0);
...
static int
mbr_unaligned_partition_write_page(struct fs_node *fs_node,
                                   void *page,
                                   uintptr_t pfn,
                                   unsigned long flags)
{
    ...
    return fs_node_paged_write(mnt->backing_node,
                               offset,
                               page,
                               1ULL << mnt->page_order,
                               0);
...
static int
mbr_aligned_partition_load_page(struct fs_node *fs_node,
                                uintptr_t pfn,
                                unsigned long flags,
                                void __phys **addr_out)
{
    ...
    size_t pfn_base =
        (part->sector_offset << mnt->sector_order) >> mnt->page_order;
    struct fs_page *page =
        fs_node_get_page(mnt->backing_node, pfn_base + pfn, 0);
    if(page == NULL)
    {
        return -ENXIO;
    }
...
static int
mbr_aligned_partition_unload_page(struct fs_node *fs_node,
                                  uintptr_t pfn,
                                  unsigned long flags,
                                  void __phys *addr)
{
    ...
    fs_node_put_page(mnt->backing_node,
                     page,
                     1); // Have to assume the page has been modified
...
static int
mbr_aligned_partition_flush_page(struct fs_node *fs_node,
                                 uintptr_t pfn,
                                 unsigned long flags,
                                 void __phys *addr)
{
