...
cpio_read_header(struct cpio_mount *mnt, size_t offset, struct cpio_header *hdr)
{
    ...
    res = fs_node_paged_read(mnt->backing_file, offset, (void *)hdr, size, 0);
...
static int
cpio_mount_load_node(struct fs_mount *fs_mnt,
                     size_t node_index,
                     struct fs_node *fs_node)
{
    ...
    struct cpio_file_node *node =
        kzmalloc(sizeof(struct cpio_file_node), KM_KERNEL);
    if(node == NULL)
    {
        return -ENOMEM;
    }
    ...
    node->data_offset = node->header_offset +
...
static int
cpio_mount_unload_node(struct fs_mount *fs_mount,
                       size_t index,
                       struct fs_node *fs_node)
{
...
static int
cpio_mount_root_index(struct fs_mount *mnt, size_t *index)
{
...
static int
cpio_mount_sync(struct fs_mount *fs_mount)
{
    int res;
...
int
cpio_mount_file(struct fs_type *fs_type,
                ...
                struct fs_mount **out_ptr)
{
    ...
    res = init_fs_mount_struct(&mnt->fs_mount, &cpio_fs_mount_ops);
    if(res)
    {
        goto err0;
    }
...
static int
cpio_unmount(struct fs_type *type, struct fs_mount *mnt)
{
    return -EUNIMPL;
}
