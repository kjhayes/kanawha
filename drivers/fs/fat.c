	...
	size_t buflen)
{
    ...
    dprintk("fat_mount_media_read(mnt=%p, offset=0x%lx, buffer=%p, buflen=0x%lx) starting\n",
	    mnt,
	    offset,
	    buffer,
	    buflen);
    res = fs_node_paged_read(
	    ...
	    buflen,
	    0);
    dprintk("fat_mount_media_read(mnt=%p, offset=0x%lx, buffer=%p, buflen=0x%lx) complete\n",
	    mnt,
	    offset,
	    buffer,
	    buflen);
	...
	struct fat_node *node)
{
	...
	case FAT_TYPE_FAT16:
	case FAT_TYPE_FAT32:
	    return (((uint32_t)letoh16(node->dirent.fat.cluster_high)) << 16) | letoh16(node->dirent.fat.cluster_low);
	case FAT_TYPE_EXFAT:
	    return -EUNIMPL;
	default:
	    return -EINVAL;
	...
	size_t index)
{
    ...
    size_t data_offset = node->mnt->data_sector_offset << node->mnt->sector_order;
    size_t cluster_size = 1ULL << node->mnt->cluster_order;

    if(((index+1) * 1ULL<<(node->mnt->cluster_order - node->mnt->sector_order) > node->mnt->data_num_sectors)) {
	return -EINVAL;
    }

    res = fat_mount_media_read(
	    node->mnt,
	...
	struct fs_node *fs_node)
{
    ...
    // Besides the root directory, we denote inodes by the offset of any files' directory entry
    // (This is a FAT based system so we don't really have inodes)
	...
	struct fs_mount **out)
{
	...
	mnt->data_num_sectors = mnt->num_clusters << (mnt->cluster_order - mnt->sector_order);
	mnt->data_sector_offset = mnt->fat_sector_offset + (mnt->num_fats * mnt->fat_num_sectors);
    } else {
	...
	mnt->cluster_order = ptr_orderof(letoh16(bpb->sectors_per_cluster)) + mnt->sector_order;
	...
	mnt->fat_num_sectors = letoh16(bpb->sectors_per_fat) > 0 ? letoh16(bpb->sectors_per_fat) : letoh32(fat32_bs->ebr.sectors_per_fat);

	mnt->num_fats = bpb->num_fats;

        size_t root_directory_entries = letoh16(bpb->num_root_dir_entries);
        size_t root_dir_sectors = ((root_directory_entries * sizeof(struct fat_dirent)) + ((1ULL<<mnt->sector_order)-1)) >> mnt->sector_order;

        mnt->data_sector_offset = mnt->fat_sector_offset + (mnt->num_fats * mnt->fat_num_sectors) + root_dir_sectors;
        mnt->data_num_sectors = mnt->num_sectors - (mnt->fat_sector_offset + (mnt->num_fats * mnt->fat_num_sectors) + root_dir_sectors);
        mnt->num_clusters = mnt->data_num_sectors >> (mnt->cluster_order - mnt->sector_order);
	...
	wprintk("0x%lx + 0x%lx (0x%lx) > 0x%lx (FAT data region overflows the disk)\n",
