...
virtio_request_create(struct virtio_queue *queue)
{
    struct virtio_request *request =
        kzmalloc(sizeof(struct virtio_request), KM_KERNEL);
    if(request == NULL)
    {
        return NULL;
    }
...
virtio_request_destroy(struct virtio_request *request)
{
    ...
    if(queue != NULL)
    {
        ...
        {
            res = virtio_queue_free_desc_chain(queue, request->root_descriptor);
            if(res)
            {
                eprintk("Failed to free virtio_request "
                        "descriptor chain!\n");
            }
        }
    }
...
static int
virtio_request_append(struct virtio_request *req,
                      void __phys *buffer,
                      size_t bufsize,
                      int output)
{
    ...
    {
        res = virtio_queue_alloc_desc(queue, &desc);
        if(res)
        {
            return res;
        }
    ...
    {
        res =
            virtio_queue_alloc_chained_desc(queue, req->tail_descriptor, &desc);
        if(res)
        {
            return res;
        }
    ...
    res = virtio_queue_point_desc(queue, desc, buffer, bufsize, output);
    if(res)
    {
        eprintk("virtio_queue: Failed to point allocated descriptor! (leaking "
                "descriptor memory) (err=%s)\n",
                errnostr(res));
        return res;
    }
...
virtio_request_append_input(struct virtio_request *req,
                            ...
                            size_t bufsize)
{
    return virtio_request_append(req, buffer, bufsize, 0);
}
...
virtio_request_append_output(struct virtio_request *req,
                             ...
                             size_t bufsize)
{
    return virtio_request_append(req, buffer, bufsize, 1);
}
...
int
virtio_request_clear_complete_callback(struct virtio_request *req)
{
                ...
                size_t *output_sizes)
{
    ...
    {
        res = dma_alloc(input_sizes[i], 0, 0, &input_buffers[i]);
        if(res)
        {
            goto exit;
        }
        ...
        res = virtio_request_append_input(req,
    ...
    {
        res = dma_alloc(output_sizes[i], 0, 0, &output_buffers[i]);
        if(res)
        {
            goto exit;
        }
        ...
        res = virtio_request_append_output(req,
