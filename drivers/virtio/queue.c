                         ...
                         uint16_t queue_size)
{
    ...
    size_t used_ring_offset =
        ((desc_table_size + avail_ring_size) + (PAGE_SIZE_4KB - 1)) &
        ~(PAGE_SIZE_4KB - 1);

    queue->dma_size = used_ring_offset + used_ring_size;

    res = dma_alloc(queue->dma_size, 12, DMA_PHYS_64, &queue->dma_region);
    if(res)
    {
    ...
    res = virtio_queue_set_desc_table(queue,
                                      dma_phys_addr(queue->dma_region) +
                                          desc_table_offset);
    if(res)
    {
        kfree(queue->desc_bitmap);
    ...
    res = virtio_queue_set_avail_ring(queue,
                                      dma_phys_addr(queue->dma_region) +
                                          avail_ring_offset);
    if(res)
    {
    ...
    res = virtio_queue_set_used_ring(queue,
                                     dma_phys_addr(queue->dma_region) +
                                         used_ring_offset);
    if(res)
    {
...
int
virtio_queue_alloc_desc(struct virtio_queue *queue, uint16_t *index_out)
{
    spin_lock(&queue->desc_lock);

    size_t bit =
        bitmap_find_clear_range(queue->desc_bitmap, queue->queue_size, 1);
                                ...
                                uint16_t *index_out)
{
    ...
    res = virtio_queue_alloc_desc(queue, index_out);
    if(res)
    {
        return res;
    }
...
static inline int
virtio_queue_try_push_avail_ring(struct virtio_queue *queue,
...
virtio_queue_find_launched_req_by_desc(struct virtio_queue *queue,
                                       uint16_t desc_id)
{
    ilist_node_t *node;
...
virtio_queue_handle_used_notification(struct virtio_queue *queue)
{
    ...
    for(size_t i = 0; i < num_new_elem; i++)
    {
        ...
        if(req == NULL)
        {
            wprintk("virtio_queue: Device or Driver Issue, "
                    "Descriptor in Used "
                    "Buffer without a corresponding launched "
                    "request! (id=0x%lx)\n",
                    (ul_t)id);
            continue;
        }
        ...
        {
            (*req->complete_callback)(req, req->complete_callback_state);
        }
    }
