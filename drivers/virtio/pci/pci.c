...
static int
virtio_pci_deinit_capabilities(struct virtio_pci_device *vpci_dev)
{
    do
    {
...
static int
virtio_pci_init_capabilities(struct virtio_pci_device *vpci_dev)
{
    int res = 0;
    ...
    struct pci_cap *cap =
        pci_func_find_cap(vpci_dev->func, PCI_CAP_ID_VENDOR_SPECIFIC);
    ...
    while(cap != NULL)
    {
        ...
        dprintk("vcap->bar=0x%x, offset=0x%lx, length=0x%lx, type=0x%x\n",
                (uint32_t)bar_index,
                (uint32_t)vcap->offset,
                (uint32_t)vcap->length,
                (uint32_t)vcap->type);
        ...
        cap = pci_func_find_next_cap(vpci_dev->func,
                                     cap,
...
virtio_pci_find_cap(struct virtio_pci_device *device, uint8_t type)
{
    ilist_node_t *node;
...
static int
virtio_pci_probe(struct pci_driver *driver, struct pci_func *func)
{
...
static int
virtio_pci_init_device(struct pci_driver *driver, struct pci_func *func)
{
    ...
    if(res)
    {
        eprintk("Failed to start IRQ's on virtio-pci device (%d.%d.%d.%d)! "
                "(err=%s)\n",
    ...
    if(res)
    {
        eprintk("Failed to enable bus mastering on virtio-pci device! "
                "(err=%s)\n",
                errnostr(res));
    ...
    struct virtio_pci_device *vpci_dev =
        kzmalloc(sizeof(struct virtio_pci_device), KM_KERNEL);
    if(vpci_dev == NULL)
    {
        return -ENOMEM;
    }
    ...
    vpci_dev->notify_multiplier =
        virtio_pci_cap_ext_cfg_readl(vpci_dev, vpci_dev->notify_cap, 0);
...
static int
virtio_pci_deinit_device(struct pci_driver *driver, struct pci_func *func)
{
    dprintk("virtio-pci: Deinit Device\n");
    return -EUNIMPL;
}

static struct pci_id virtio_pci_ids[] = {
...
static struct pci_driver_ops virtio_pci_driver_ops = {
