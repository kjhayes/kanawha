...
static int
virtio_pci_queue_used_notification_irq_handler(struct excp_state *excp_state,
                                               struct irq_action *action)
{
    ...
    if(res)
    {
        eprintk("virtio_pci_queue_used_notification_irq_handler: Failed to "
                "handle used notification (err=%s)\n",
                errnostr(res));
    }
...
virtio_pci_create_queue(struct virtio_pci_device *device, uint16_t queue_no)
{
    ...
    res = virtio_pci_device_set_queue_cfg(device, queue_no);
    if(res)
    {
        kfree(queue);
        return NULL;
    ...
    queue->notify_offset =
        virtio_pci_device_queue_cfg_get_notify_offset(device);
    ...
    irq_t irq = pci_func_get_irq(device->func, queue->msix_vector);
    if(irq == NULL_IRQ)
    {
        kfree(queue);
    ...
    {
        eprintk("virtio_pci_queue: Queue rejected MSI-X vector 0x%lx "
                "(resp_vector=0x%lx)\n",
    ...
    res = virtio_queue_init_struct(&queue->queue,
    ...
    if(res)
    {
        eprintk("virtio_pci_queue: Failed to initialize queue struct "
                "(err=%s)\n");
        kfree(queue);
        return NULL;
    }
...
static int
virtio_pci_set_desc_table(struct virtio_queue *queue, void __phys *ptr)
{
    int res;

    struct virtio_pci_queue *pci_queue =
        container_of(queue, struct virtio_pci_queue, queue);
    struct virtio_device *vdev = queue->device;
    struct virtio_pci_device *vpci_dev =
        container_of(vdev, struct virtio_pci_device, virtio_dev);

    res = virtio_pci_device_set_queue_cfg(vpci_dev, queue->index);
    if(res)
    {
        return res;
    }

    virtio_pci_device_queue_cfg_set_desc(vpci_dev, (uint64_t)ptr);

    return 0;
}

static int
virtio_pci_set_avail_ring(struct virtio_queue *queue, void __phys *ptr)
{
    int res;
    ...
    virtio_pci_device_queue_cfg_set_driver_area(vpci_dev, (uint64_t)ptr);

    return 0;
}

static int
virtio_pci_set_used_ring(struct virtio_queue *queue, void __phys *ptr)
{
    int res;
    ...
    virtio_pci_device_queue_cfg_set_device_area(vpci_dev, (uint64_t)ptr);

    return 0;
}

static int
virtio_pci_notify(struct virtio_queue *queue)
{
    struct virtio_pci_queue *pci_queue =
        container_of(queue, struct virtio_pci_queue, queue);
    struct virtio_device *vdev = queue->device;
    ...
    dprintk("virtio_pci_notify(queue=0x%x, notify_offset=0x%lx, "
            "notify_mult=0x%lx, offset=0x%lx, notify_cap_offset=%p, "
            "notify_cap_length=%p) index=0x%lx, data=0x%x\n",
    ...
    virtio_pci_cap_bar_writew(vpci_dev,
                              vpci_dev->notify_cap,
                              offset,
                              queue->index);
...
static int
virtio_pci_enable_queue(struct virtio_queue *queue)
{
    int res;
    ...
    virtio_pci_device_queue_cfg_set_enabled(vpci_dev, (uint64_t)1);

    return 0;
}

static int
virtio_pci_disable_queue(struct virtio_queue *queue)
{
    int res;
    ...
    virtio_pci_device_queue_cfg_set_enabled(vpci_dev, (uint64_t)0);

    return 0;
}

struct virtio_queue_ops virtio_pci_queue_ops = {
