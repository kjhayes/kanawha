...
static int
virtio_try_match(struct virtio_driver *driver, struct virtio_device *device)
{
    int res;
    ...
    {
        eprintk("virtio_try_match: Device Rejected Subset of Features After "
                "Negotiation\n");
        return -EINVAL;
    }
...
int
register_virtio_device(struct virtio_device *device)
{
    int res;
    ...
    if(res)
    {
        eprintk("register_virtio_device: Failed to reset the device "
                "(err=%s)\n",
                errnostr(res));
        return res;
    }
    ...
    if(status)
    {
        eprintk("register_virtio_device: Device Status is non-zero! "
                "(status=0x%lx)\n",
                status);
        return -EINVAL; // Nothing should have configured the device at
                        // this point
    }
...
register_virtio_driver(struct virtio_driver *driver)
{
    ...
    if(matched)
    {
        ilist_for_each(node, &driver->device_list)
        {
            ...
            DEBUG_ASSERT(ilist_contains(&virtio_unmatched_device_list,
                                        &device->global_node));
