...
#ifdef CONFIG_PORT_IO
static struct clk_driver acpi_pm_clk_driver_port = {
    .freq = acpi_pm_timer_freq,
    .mono_cycles = acpi_pm_timer_cycles_port,
};
#endif

static struct clk_driver acpi_pm_clk_driver_mmio = {
    .freq = acpi_pm_timer_freq,
...
init_acpi_pm_timer_clk(void)
{
    ...
    if(table == NULL)
    {
        printk("Could not find ACPI FADT Table to initialize ACPI PM "
               "Timer\n");
        return 0;
    }
    ...
    struct acpi_pm_timer *clk =
        kzmalloc(sizeof(struct acpi_pm_timer), KM_KERNEL);
    if(clk == NULL)
    {
    ...
    if(acpi_revision() >= 2 && fadt->x_pm_tmr_blk.address != 0 &&
       fadt->x_pm_tmr_blk.access_size == 4)
    {
        ...
        case ACPI_GAS_ASID_MMIO:
            clk->mmio.blk =
                mmio_map((void __phys *)fadt->x_pm_tmr_blk.address, 4);
            if(clk->mmio.blk == NULL)
            {
                eprintk("Failed to map MMIO register for ACPI PM "
                        "Timer!\n");
                kfree(clk);
                return -ENOMEM;
            }
        ...
        case ACPI_GAS_ASID_PIO:
...
#else
            eprintk("ACPI Timer has Port I/O register but "
                    "CONFIG_PORT_IO is "
                    "not enabled!\n");
            kfree(clk);
            return -EINVAL;
            ...
            break;
        default:
            eprintk("Unsupported Address Space for ACPI PM Timer "
                    "Generic Address\n");
            kfree(clk);
            return 0;
        }
...
declare_init_desc(dynamic,
                  init_acpi_pm_timer_clk,
                  "ACPI PM Timer Clock Source Init");
