...
// (e.g. configuring a generic "unknown" ps2 device(s) needs to be done one at
// a time)
...
static int
__ps2_recv_timeout_buffer_push(uint8_t data)
{
...
static int
__ps2_recv_timeout_buffer_pop(uint8_t *recv)
{
...
static void
ps2_port_recv_timeout_callback(struct ps2_port *port,
...
static int
ps2_port_recv_timeout(uint8_t *data, duration_t duration, ssize_t attempts)
{
    int res;
...
static int
ps2_reset(struct ps2_port *port)
{
    int res;
    ...
    uint8_t recv;
    res = ps2_port_recv_timeout(&recv, msec_to_duration(1), 10);
    if(res)
    {
...
static int
ps2_identify(struct ps2_port *port, uint8_t *buffer, size_t *buflen_out_idlen)
{
    int res;
    ...
    {
        uint8_t ack;
        res = ps2_port_recv_timeout(&ack, msec_to_duration(1), 10);
        if(res)
        {
            goto err0;
        }
    ...
    {
        uint8_t extra_byte;
        res = ps2_port_recv_timeout(&extra_byte, msec_to_duration(1), 10);
        if(res == 0)
        {
            eprintk("PS/2 Device ID Was Longer Than Provided Buffer "
                    "(buflen=0x%llx)\n",
                    (ull_t)buflen);
            return -EINVAL;
        }
...
static int
ps2_ids_match(uint8_t *id_bytes,
              size_t num_id_bytes,
...
int
ps2_register_port(struct ps2_port *port)
{
    int res;
...
#undef IDBUFSIZE
    ...
    ilist_for_each(driver_node, &ps2_driver_list)
    {
        ...
        if(ps2_ids_match(id_bytes, num_id_bytes, driver->ids, driver->num_ids))
        {
...
int
ps2_unregister_port(struct ps2_port *port)
{
    spin_lock(&ps2_global_lock);
...
int
ps2_register_driver(struct ps2_driver *driver)
{
    int res;
...
int
ps2_unregister_driver(struct ps2_driver *driver)
{
    spin_lock(&ps2_global_lock);
...
int
ps2_port_set_callback(struct ps2_port *port,
...
int
ps2_port_enable_scanning(struct ps2_port *port)
{
    int res;
...
int
ps2_port_disable_scanning(struct ps2_port *port)
{
    int res;
