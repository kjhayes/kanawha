...
static int
qemu_edu_irq_handler(struct excp_state *excp_state, struct irq_action *action)
{
...
static int
qemu_edu_probe(struct pci_driver *driver, struct pci_func *func)
{
    int res;
...
static int
qemu_edu_init_device(struct pci_driver *driver, struct pci_func *func)
{
    int res;
    ...
    struct irq_action *action = irq_install_handler(irq_to_desc(irq),
                                                    (void *)bar,
    ...
    // Raise an interrupt
    pci_bar_writel(bar, 0x60, 0x5);

    return 0;
}

static int
qemu_edu_deinit_device(struct pci_driver *driver, struct pci_func *dev)
{
    printk("QEMU PCI EDU: deinit\n");
    return 0;
}

static struct pci_id qemu_edu_pci_ids[] = {
    {
        .vendor = 0x1234,
        .device = 0x11e8,
    },
...
static struct pci_driver_ops qemu_edu_pci_driver_ops = {
    .probe = &qemu_edu_probe,
    .init_device = &qemu_edu_init_device,
    .deinit_device = &qemu_edu_deinit_device,
};

static struct pci_driver qemu_edu_pci_driver = {
