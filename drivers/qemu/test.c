...
static int
qemu_test_probe(struct pci_driver *driver, struct pci_func *dev)
{
...
#define NAME_BUFLEN 0x100
            ...
            for(size_t i = 0; i < NAME_BUFLEN; i++)
            {
                name_buf[i] =
                    pci_bar_readb(bar,
                                  offsetof(struct pci_test_dev_hdr, name) + i);
                if(name_buf[i] == '\0')
                {
                    break;
                }
...
static struct pci_id qemu_test_pci_ids[] = {
    {
        .vendor = 0x1b36,
        .device = 0x0005,
    },
...
static struct pci_driver_ops qemu_test_pci_driver_ops = {
    .probe = &qemu_test_probe,
    .init_device = &qemu_test_init_device,
    .deinit_device = &qemu_test_deinit_device,
};

static struct pci_driver qemu_test_pci_driver = {
