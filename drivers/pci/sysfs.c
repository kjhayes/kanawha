...
static int
pci_cfg_fs_node_read_page(struct fs_node *fs_node,
                          void *buffer,
                          uintptr_t pfn,
                          unsigned long flags)
{
    ...
    struct pci_func *func = container_of(vfs_node, struct pci_func, vfs_node);
...
static int
pci_cfg_fs_node_getattr(struct fs_node *fs_node, int attr, size_t *value)
{
    int res;
...
static int
insert_func_with_match_lock(struct pci_func *func)
{
    int res;
    ...
    size_t inode;
    res = vfs_mount_insert_node(pci_fs_mount, &func->vfs_node, &inode);
    if(res)
    {
        return res;
    }

    res = vfs_mount_link_root(pci_fs_mount, namebuf, inode);
    if(res)
    {
        return res;
    }
