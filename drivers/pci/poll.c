...
static int
polling_msi_req_32(struct pci_mailbox *pci_mb,
                   size_t num_req,
                   uint32_t *addr_out,
                   uint16_t *data_out)
{
...
static int
polling_msi_req_64(struct pci_mailbox *pci_mb,
                   size_t num_req,
                   uint64_t *addr_out,
                   uint16_t *data_out)
{
...
static int
polling_msix_req(struct pci_mailbox *pci_mb,
                 size_t num_req,
                 uint64_t *addrs_out,
                 uint32_t *datas_out)
{
...
polling_msi_get_desc_32(struct pci_mailbox *pci_mb,
                        uint32_t addr,
                        uint16_t data,
                        size_t index)
{
    ...
    poll_box_t __phys *p_boxes = dma_phys_addr(mb->boxes);
    size_t byte_offset =
        ((void __phys *)(uintptr_t)addr) - ((void __phys *)p_boxes);
...
static struct irq_desc *
polling_msi_get_desc_64(struct pci_mailbox *pci_mb,
                        uint64_t addr,
                        uint16_t data,
                        size_t index)
{
...
static struct irq_desc *
polling_msix_get_desc(struct pci_mailbox *pci_mb,
                      uint64_t addr,
                      uint32_t data,
                      size_t index)
{
    ...
    printk(
        "polling_msix_get_desc: addr=%p, data=0x%x, index=0x%lx, hwirq=0x%lx\n",
        (uintptr_t)addr,
        (u_t)data,
        (ul_t)index,
        (ul_t)hwirq);
...
static struct pci_mailbox_ops polling_pci_mailbox_ops = {
...
create_polling_pci_mailbox(size_t num_irqs)
{
    int res;

    struct polling_pci_mailbox *mb = kzmalloc(sizeof(*mb), KM_KERNEL);
    if(mb == NULL)
    {
        return NULL;
    }
    ...
    res = dma_alloc(sizeof(poll_box_t) * num_irqs,
                    orderof(poll_box_t),
                    DMA_PHYS_64,
                    &mb->boxes);
    if(res)
    ...
    mb->poll = create_periodic_event(msec_to_duration(10),
                                     mb,
    ...
    res = register_pci_mailbox(&mb->mailbox, &polling_pci_mailbox_ops);
    if(res)
    {
...
declare_init_desc(bus,
                  init_polling_pci_mailbox,
                  "Providing Polling PCI Mailbox");
