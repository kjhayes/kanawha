...
pcie_acpi_probe_mcfg_table(void)
{
    ...
    for(size_t i = 0; i < num_entries; i++)
    {
        ...
        dprintk("MCFG Entry: base=%p, segment=0x%lx, start_bus=0x%x, "
                "end_bus=0x%x\n",
                cur->base_addr,
                (ul_t)cur->segment,
                cur->start_bus,
        ...
        struct mmio_pci_ecam *ecam =
            kmalloc(sizeof(struct mmio_pci_ecam), KM_KERNEL);
        if(ecam == NULL)
        {
...
declare_init_desc(bus,
                  pcie_acpi_probe_mcfg_table,
                  "Looking for ACPI MCFG Table for PCIe");
