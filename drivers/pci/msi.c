        ...
        struct pci_func *func)
{
    ...
    struct irq_domain *domain =
        alloc_irq_domain_linear(0, msi_dev->num_irqs);
    if(domain == NULL) {
        kfree(msi_dev->link_actions);
        kfree(msi_dev);
        return -ENOMEM;
    ...
    // Install IRQ Links from Mailbox to MSI IRQ Domain
    {
       ...
       for(size_t i = 0; i < msi_dev->num_irqs; i++) {
           ...
           msi_dev->link_actions[i] = irq_install_direct_link(link_from, link_to);
