...
#include <drivers/term/8250/mmio.h>
...
static int
dt_8250_probe(struct dt_driver *driver, struct dt_node *node)
{
    return 0;
}

static int
dt_8250_init_node(struct dt_driver *driver, struct dt_node *node)
{
    int res;
    ...
    res = dt_node_read_reg(node, 1, &phys_base, &phys_size);
    if(res)
    {
    ...
    uintptr_t shift;
    res = dt_node_read_property_unsigned(node, "reg-shift", &shift);
    if(res)
    {
        shift = 0;
    }
    ...
    {
        wprintk(
            "Device Tree 8250 Driver Failed to get number of IRQ's (err=%s)!\n",
            errnostr(res));
        kfree(uart);
        return res;
    }
    ...
    {
        wprintk(
            "Device Tree 8250 Driver Unexpected Number of IRQ's (num=%ld)!\n",
            (sl_t)num_irqs);
        kfree(uart);
        return res;
    }
...
static int
dt_8250_deinit_node(struct dt_driver *driver, struct dt_node *node)
{
    int res;
    return -EUNIMPL;
}

struct dt_driver_ops dt_8250_driver_ops = {
    .probe = dt_8250_probe,
...
struct dt_driver dt_8250_driver = {
...
declare_init_desc(device,
                  register_dt_8250_driver,
                  "Registering Device Tree 8250 Driver");
