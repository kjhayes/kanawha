...
static struct pc_com platform_pc_com_ports[NUM_PLATFORM_PC_COM_PORTS];

static pio_t platform_pc_com_ports_base[] = {
    0x3F8,
    0x2F8,
    0x3E8,
    0x2E8,
...
static hwirq_t platform_pc_com_pic_irq[] = {
    4,
    3,
    4,
    3,
...
static void
pc_com_set_dlab(struct pc_com *com, int dlab)
{
...
static pio_t
pc_com_enable_reg_port(struct uart_8250 *uart, reg_8250_t reg)
{
    struct pc_com *com = container_of(uart, struct pc_com, uart_8250);
...
static uint8_t
pc_com_8250_read_reg(struct uart_8250 *uart, reg_8250_t reg)
{
...
static int
pc_com_8250_write_reg(struct uart_8250 *uart, reg_8250_t reg, uint8_t value)
{
    pio_t port = pc_com_enable_reg_port(uart, reg);
    outb(port, value);
    return 0;
...
static struct uart_8250_ops pc_com_8250_uart_ops = {
...
static int
pc_com_8250_register(struct pc_com *com, int index)
{
    int res;
    ...
    res = register_uart_8250(com->name,
                             &com->uart_8250,
...
declare_init_desc(device,
                  pc_com_8250_init,
                  "Registering PC COM Serial Port(s)");
