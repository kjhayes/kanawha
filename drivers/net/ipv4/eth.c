        ...
        struct eth_dev *dev)
{
    ...
    struct ptree_node *pnode = ptree_get(&ipv4_over_eth_dev_eth_dev_tree, (uintptr_t)dev);
...
static struct ipv4_driver
ipv4_over_eth_driver = {
        ...
        struct eth_dev *eth_dev)
{
    ...
    ipv4_over_eth_dev_eth_dev_tree_lock_acquire();
    res = ptree_insert(&ipv4_over_eth_dev_eth_dev_tree, &dev->eth_dev_node, (uintptr_t)eth_dev);
    if(res) {
        ipv4_over_eth_dev_eth_dev_tree_lock_release();
        kfree(dev);
        return res;
