...
#define VIRTIO_NET_HDR_GSO_ECN 0x80
    ...
    le16_t csum_start;
    le16_t csum_offset;
    le16_t num_buffers; // Not included without VIRTIO_NET_F_MRG_RXBUF if using
                        // legacy interface
    ...
    //    le16_t padding_reserved; // (Only if VIRTIO_NET_F_HASH_REPORT
    //    negotiated)
};
...
static int
virtio_net_eth_read_mac(struct eth_dev *eth_dev, struct eth_mac_addr *addr_out)
{
    int res;
    ...
    {
        res =
            virtio_device_cfg_readb(dev->virtio_dev,
...
static int
virtio_net_destroy_eth_frame(struct virtio_net *dev,
...
virtio_net_eth_alloc_frame(struct eth_dev *eth_dev,
                           size_t frame_len,
                           unsigned long flags)
{
    int res;
    ...
    res = dma_alloc(virtio_net_hdr_len(dev),
    ...
    res = dma_alloc(frame->frame_len, 0, 0, &frame->frame_dma_buffer);
    if(res)
    {
        dma_free(frame->header_dma_buffer, virtio_net_hdr_len(dev));
        kfree(frame);
    ...
    res = virtio_request_append_input(frame->req,
...
static int
virtio_net_eth_send_frame(struct eth_dev *eth_dev, struct eth_frame *eth_frame)
{
    int res;
...
static int
virtio_net_eth_drop_frame(struct eth_dev *eth_dev, struct eth_frame *eth_frame)
{
    int res;
...
static void
virtio_net_eth_recv_request_callback(struct virtio_request *req,
                                     void *priv_state)
{
    int res;
    ...
    for(size_t i = 0; i < dev->num_recv_requests; i++)
    {
        if(dev->recv_requests[i] == req)
        {
            ...
            res = eth_dev_internal_on_recv(&dev->eth_dev, frame, len, 0);
            if(res)
            {
                // Weird but OK...
            }
            ...
            if(res)
            {
                wprintk("Failed to re-launch virtio-net receive "
                        "buffer request!\n");
            }
    ...
    if(found == 0)
    {
        wprintk("Failed to find receive buffer for completed virtio-net "
                "request!\n");
        return;
    }
}
...
static int
virtio_net_eth_begin_recv(struct eth_dev *eth_dev, unsigned long flags)
{
    int res;
    ...
    dev->recv_buffers =
        kzmalloc(sizeof(dma_addr_t) * dev->num_recv_requests, KM_KERNEL);
    dev->recv_requests =
        kzmalloc(sizeof(struct virtio_request *) * dev->num_recv_requests,
                 KM_KERNEL);
    ...
    {
        struct virtio_request *req =
            virtio_request_create(dev->queue_pairs[0].recv);
        if(req != NULL)
        {
            res = dma_alloc(dev->recv_buffer_size,
                            0,
                            DMA_PHYS_64,
                            &dev->recv_buffers[i]);
            if(res)
            ...
            {
                virtio_request_append_input(req,
    ...
    for(size_t i = 0; i < dev->num_recv_requests; i++)
    {
        ...
        if(res)
        {
            wprintk("virtio-net: Failed to launch virtio request to begin "
                    "receiving packets!\n");
        }
    }
...
static int
virtio_net_eth_end_recv(struct eth_dev *eth_dev, unsigned long flags)
{
    int res;
...
static struct eth_driver virtio_eth_driver = {
...
static int
virtio_net_probe(struct virtio_driver *driver, struct virtio_device *device)
{
...
static int
virtio_net_negotiate(struct virtio_driver *driver, struct virtio_device *device)
{
    int res;
...
static int
virtio_net_init_device(struct virtio_driver *driver,
                       struct virtio_device *device)
{
    ...
    struct virtio_net *net =
        kzmalloc(sizeof(struct virtio_net) +
                     (sizeof(struct virtio_net_queue_pair) * num_queue_pairs),
                 KM_KERNEL);
    if(net == NULL)
    {
        return -ENOMEM;
...
#undef NAMEBUFLEN
    ...
    res = register_eth_dev(&net->eth_dev, net->name);
    if(res)
    {
        for(int i = 0; i < net->num_queue_pairs; i++)
        {
...
static int
virtio_net_deinit_device(struct virtio_driver *driver,
                         struct virtio_device *device)
{
    return -EUNIMPL;
}

static struct virtio_driver_ops virtio_net_virtio_driver_ops = {
...
static uint16_t virtio_net_virtio_ids[] = {
    1,
};

static struct virtio_driver virtio_net_virtio_driver = {
...
declare_init_desc(device,
                  register_virtio_eth_driver,
                  "Registered Virtio Network Driver");
