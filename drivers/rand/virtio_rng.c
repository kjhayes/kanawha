...
static ssize_t
virtio_rng_read(struct rand_dev *rand_dev, void *buffer, size_t buflen)
{
    int res;
    ...
    if(dev->data_avail == 0)
    {
        ...
        dprintk("Received Response queue=%p, index=0x%lx\n",
                dev->request->queue,
                dev->request->queue->index);
        DEBUG_ASSERT_MSG(dev->request->len_written <= VIRTIO_RNG_BUFSIZE,
                         "WTF (len_written=0x%lx)\n",
...
static int
virtio_rng_probe(struct virtio_driver *driver, struct virtio_device *device)
{
...
static int
virtio_rng_negotiate(struct virtio_driver *driver, struct virtio_device *device)
{
...
static int
virtio_rng_init_device(struct virtio_driver *driver,
                       struct virtio_device *device)
{
    ...
    res = dma_alloc(rng->bufsize, 0, DMA_PHYS_64, &rng->buffer);
    if(res)
    {
    ...
    res = virtio_request_append_output(rng->request,
...
#define NAMEBUFLEN 64
    ...
    res = register_rand_dev(&rng->rand_dev, rng->name);
    if(res)
    {
        virtio_queue_disable(rng->request_queue);
        virtio_request_destroy(rng->request);
...
static int
virtio_rng_deinit_device(struct virtio_driver *driver,
                         struct virtio_device *device)
{
    return -EUNIMPL;
}

static struct virtio_driver_ops virtio_rng_driver_ops = {
...
static uint16_t virtio_rng_virtio_ids[] = {
    4,
};
...
declare_init_desc(device,
                  register_virtio_rng_driver,
                  "Registering Virtio RNG Driver");
