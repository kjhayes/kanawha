...
static inline int
scsi_command_error_to_errno(struct scsi_command *cmd)
{
    if(cmd->error == 0)
    {
        return 0;
...
static int
scsi_adaptor_get_luns(struct scsi_adaptor *scsi,
                      uint64_t *luns_out,
                      uint64_t *max_luns_out)
{
    ...
    struct scsi_command *cmd =
        scsi_adaptor_create_command(scsi,
                                    dummy_target,
    ...
    res = scsi_adaptor_write_cdb(scsi, cmd, &cdb, sizeof(struct scsi_cdb_12));
    if(res)
    {
    ...
    res = scsi_adaptor_point_in_data(scsi,
                                     cmd,
...
int
register_scsi_adaptor(struct scsi_adaptor *adaptor, const char *name)
{
    int res;
    ...
    res = scsi_adaptor_get_luns(adaptor, luns_buffer, &luns_avail);
    if(res)
    {
    ...
    printk("SCSI: Read (%d/%d) LUN(s) off the device!\n",
           ...
           (int)(luns_avail));
    for(int i = 0;
        i < (int)(luns_bufcount > luns_avail ? luns_avail : luns_bufcount);
        i++)
    {
...
int
unregister_scsi_adaptor(struct scsi_adaptor *adaptor)
{
    int res;
...

    {
        while(1)
        {
            ...
            struct scsi_dev *dev =
                container_of(node, struct scsi_dev, adaptor_node);

            res = scsi_dev_deinit(dev);
            if(res)
            {
                panic("Failed to destroy SCSI device during "
                      "unregister_scsi_adaptor");
            }
...
int
scsi_adaptor_run_virtual_command(struct scsi_adaptor *adaptor,
                                 struct scsi_target target,
                                 void *cdb,
                                 size_t cdb_len,
                                 ...
                                 size_t to_dev_buffer_len)
{
    ...
    {
        res = dma_alloc(from_dev_buffer_len,
                        5, // 32-byte aligned
                        DMA_PHYS_64,
                        &in_dma);
        if(res)
    ...
    {
        res = dma_alloc(to_dev_buffer_len,
                        5, // 32-byte aligned
                        DMA_PHYS_64,
                        &out_dma);
        if(res)
    ...
    res = scsi_adaptor_run_physical_command(adaptor,
                                            target,
                                            cdb,
                                            cdb_len,
                                            in_phys,
...
int
scsi_adaptor_run_physical_command(struct scsi_adaptor *adaptor,
                                  struct scsi_target target,
                                  void *cdb,
                                  size_t cdb_len,
                                  ...
                                  size_t to_dev_buffer_len)
{
    ...
    struct scsi_command *cmd;
    cmd = scsi_adaptor_create_command(adaptor, target, 0);
    ...
    if(from_dev_buffer_len > 0)
    {
        res = scsi_adaptor_point_in_data(adaptor,
                                         cmd,
                                         from_dev_buffer,
                                         from_dev_buffer_len);
        if(res)
    ...
    if(to_dev_buffer_len > 0)
    {
        res = scsi_adaptor_point_out_data(adaptor,
                                          cmd,
                                          to_dev_buffer,
                                          to_dev_buffer_len);
        if(res)
