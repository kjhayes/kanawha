...
declare_init(dynamic, module_framework_init);
...
unload_module(struct module *mod)
{
        ...
        pnode = ptree_get_next(pnode))
    {
        struct module_dependency *dep =
            container_of(pnode, struct module_dependency, tree_node);
        refcount_dec(&dep->mod->refcount);
    }
    ...
    while(1) {
        ...
        struct module_dependency *dep =
            container_of(node, struct module_dependency, tree_node);
        kfree(dep);
    }
