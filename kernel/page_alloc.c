...
DEFINE_OP_LIST_WRAPPERS(PAGE_ALLOCATOR_OP_LIST,
                        static inline,
                        /* No Prefix */,
                        page_allocator,
...
{
    page_allocator_slab_allocator =
        create_static_slab_allocator(page_allocator_slab_buffer,
...
declare_init_desc(static,
                  page_alloc_static_init,
                  "Initializing page_alloc Framework");
...
alloc_page_allocator(void)
{
    ...
    {
        /*
        dprintk("Allocating page_allocator struct from slab allocator:
        (num_slots=%lu, num_free=%lu, num_alloc=%lu)\n", (unsigned
        long)slab_objs_total(page_allocator_slab_allocator), (unsigned
        long)slab_objs_free(page_allocator_slab_allocator), (unsigned
        long)slab_objs_alloc(page_allocator_slab_allocator));
        */
...
int
register_page_allocator(struct page_allocator_ops *ops,
                        void *state,
                        ...
                        unsigned long flags)
{
    ...
    struct ptree_node *region_before =
        ptree_get_max_less(&page_allocator_interval_tree, paddr_start);

    if(region_before != NULL)
    {
        struct page_allocator *other =
            container_of(region_before, struct page_allocator, ptree_node);
        ...
        if(other_region_end > (uintptr_t)region_base)
        {
            ...
            eprintk("Cannot register page allocator which has lower "
                    "overlap!\n");
            return -EEXIST;
        }
    }

    struct ptree_node *region_before_end =
        ptree_get_max_less(&page_allocator_interval_tree, paddr_end);
    ...
    res = ptree_insert(&page_allocator_interval_tree,
...
page_alloc_get_allocator(order_t order, void __phys **addr, unsigned long flags)
{
    int res;
    ilist_node_t *node;
    struct page_allocator *alloc;

    DEBUG_ASSERT_MSG(order >= PAGE_ALLOC_MIN_ORDER,
                     "Cannot allocate page of size=%d",
                     (s_t)order);
    DEBUG_ASSERT_MSG(order <= PAGE_ALLOC_MAX_ORDER,
                     "Cannot allocate page of size=%d",
                     (s_t)order);
    ...
    // We couldn't find a perfect match, so fall back to using any memory we
    // can find which has all of our flags set.
    ...
    ilist_for_each(node, &page_allocator_list)
    {
        ...
        if((alloc->flags & flags) == flags)
        {
            ...
            unsigned long required_flags =
                alloc->flags & PAGE_ALLOC_EXPLICIT_FLAGS;
...
{
    struct page_allocator *alloc = page_alloc_get_allocator(order, addr, flags);
    if(alloc == NULL)
    {
        return -ENOMEM;
    }
    DEBUG_ASSERT_MSG(KERNEL_ADDR(alloc), "alloc = %p", (void *)alloc);
...
{
    dprintk("page_free -> %p\n", addr);
    struct ptree_node *alloc_ptree_node =
        ptree_get_max_less(&page_allocator_interval_tree, (uintptr_t)addr);
    if(alloc_ptree_node == NULL)
    {
        return -EINVAL;
    }
    struct page_allocator *alloc =
        container_of(alloc_ptree_node, struct page_allocator, ptree_node);
    int res;
