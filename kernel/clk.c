...
clk_delay(duration_t duration)
{
    ...
    while(duration > 0)
    {
        ...
        if(cur < prev)
        {
            ...
            //            wprintk("clk_delay overflow (initial=0x%llx,
            //            cur=0x%llx, prev=0x%llx)\n",
            //                    (ull_t)initial, (ull_t)cur,
            //                    (ull_t)prev);
            prev = cur;
            continue;
        }
...
clk_mono_current(void)
{
    ...
    duration_t dur =
        freq_cycles_to_duration(clk_dev_freq(clk_source), cur_count);
    clk_source_lock_release();
    return dur;
}

static void
clk_mono_on_clk_dev_register(struct clk_dev *dev)
{
    ...
    if(clk_source == NULL || (new_freq > clk_dev_freq(clk_source)))
    {
        ...
        {
            printk("Replacing current clock source with \"%s\" due "
                   "to higher "
                   "frequency\n",
                   clk_dev_get_name(dev));
        }
...
static void
clk_mono_on_clk_dev_unregister(struct clk_dev *dev)
{
    clk_source_lock_acquire();
    if(clk_source == dev)
    {
...
LOCAL_REGISTRY_HOOK(clk_mono_clk_dev_hook,
                    clk_dev,
