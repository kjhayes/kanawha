...
static int
exec_elf64_handle_load_segment(
        struct process *process,
        fd_t file,
        Elf64_Phdr *phdr)
{
    ...
    if((offset & ((1ULL<<VMEM_MIN_PAGE_ORDER)-1)) != (vaddr & ((1ULL<<VMEM_MIN_PAGE_ORDER)-1))) {
        wprintk("exec_elf64_load_segment: %s segment with file_offset=%p, vaddr=%p with different page offsets!\n",
		elf_get_phdr_type_string(phdr->p_type),
    ...
    if(ptr_orderof(filesz) < VMEM_MIN_PAGE_ORDER) {
        dprintk("exec_elf64_handle_load_segment: %s segment with file_size=%p not a multiple of the minimum page size: rounding up\n",
		elf_get_phdr_type_string(phdr->p_type),
                (uintptr_t)filesz);
    ...
    if(ptr_orderof(bsssz) < VMEM_MIN_PAGE_ORDER) {
        dprintk("exec_elf64_handle_load_segment: %s segment with bss_size=%p not a multiple of the minimum page size: rounding up\n",
		elf_get_phdr_type_string(phdr->p_type),
                (uintptr_t)bsssz);
        ...
        Elf64_Phdr *phdr)
{
    ...
    if((offset & ((1ULL<<VMEM_MIN_PAGE_ORDER)-1)) != (vaddr & ((1ULL<<VMEM_MIN_PAGE_ORDER)-1))) {
        wprintk("exec_elf64_handle_load_segment: %s segment with file_offset=%p, vaddr=%p with different page offsets!\n",
		elf_get_phdr_type_string(phdr->p_type),
                (uintptr_t)offset, (uintptr_t)vaddr);
        return -EINVAL;
    }
    ...
    if(ptr_orderof(filesz) < VMEM_MIN_PAGE_ORDER) {
        dprintk("exec_elf64_handle_tls_segment: %s segment with file_size=%p not a multiple of the minimum page size: rounding up\n",
		elf_get_phdr_type_string(phdr->p_type),
                (uintptr_t)filesz);
        filesz += ((1ULL<<VMEM_MIN_PAGE_ORDER)-1);
        filesz &= ~((1ULL<<VMEM_MIN_PAGE_ORDER)-1);
    ...
    if(ptr_orderof(bsssz) < VMEM_MIN_PAGE_ORDER) {
        dprintk("exec_elf64_handle_tls_segment: %s segment with bss_size=%p not a multiple of the minimum page size: rounding up\n",
		elf_get_phdr_type_string(phdr->p_type),
                (uintptr_t)bsssz);
        bsssz += ((1ULL<<VMEM_MIN_PAGE_ORDER)-1);
        bsssz &= ~((1ULL<<VMEM_MIN_PAGE_ORDER)-1);
        ...
        Elf64_Phdr *phdr)
{
    switch(phdr->p_type) {
        case PT_LOAD:
            return exec_elf64_handle_load_segment(
                    process,
                    file,
                    phdr);
	case PT_TLS:
	    return exec_elf64_handle_tls_segment(
		    process,
		    file,
		    phdr);
        case PT_NULL:
...
static int
elf64_exec_type_load(
	struct exec_type *exec_type,
	struct process *process,
	struct file *file)
{
    return process_exec_elf64(
	    process,
	    file->table_node.key,
	    file);
}

static struct exec_type_ops
elf64_exec_type_ops = {
    .probe = elf64_exec_type_probe,
    .load = elf64_exec_type_load,
};
static struct exec_type
elf64_exec_type = {
...
declare_init_desc(late, register_elf64_exec_type, "Registering ELF64 Executable Format");
