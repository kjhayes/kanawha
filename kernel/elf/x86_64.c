...
static const char *
x86_64_elf64_get_reloc_string(uint32_t reloc_type)
{
...
static int
x86_64_elf64_apply(struct module *mod,
                   ...
                   int64_t addend)
{
    ...
    if(need_symbol)
    {
        ...
        {
            eprintk("Relocation referred to non-existant symbol "
                    "index 0x%lx (%s)\n",
                    symbol_index,
                    x86_64_elf64_get_reloc_string(type));
            return -EINVAL;
        ...
        case SHN_UNDEF:
            ...
            if(ksymbol == NULL)
            {
                eprintk("Failed to get kernel symbol \"%s\" needed "
                        "when "
                        "resolving module relocations!\n",
                        (char *)(state->symstrtab + symbol->st_name));
                return -ENXIO;
            }
        ...
        default:
            ...
            {
                eprintk("Symbol refers to section index %u "
                        "during relocation, "
            ...
            {
                eprintk("Cannot handle relocation to symbol in "
                        "non-SHF_ALLOC "
                        "section! symbol=\"%s\", section=\"%s\"\n",
                        (char *)(state->symstrtab + symbol->st_name),
...
#define __P ((uintptr_t)(section + offset))
    ...
    case R_X86_64_64:
        ...
        {
            *(uint64_t *)(section + offset) = (uint64_t)(__S + __A);
            res = 0;
        }
        break;
    ...
    case R_X86_64_PC64:
        ...
        {
            *(uint64_t *)(section + offset) = (uint64_t)(__S + __A - __P);
            res = 0;
        }
        break;
...
static struct elf64_machine_reloc x86_64_elf64_relocs = {
    .machine = EM_X86_64,
...
declare_init_desc(fs,
                  x86_64_register_elf_relocs,
                  "Registering x64 Elf Relocation Information");
