...
{
    size_t num_longs =
        (num_entries / BITS_PER_LONG) + (num_entries % BITS_PER_LONG != 0);
...
bitmap_find_first_clear(unsigned long *bitmap, size_t num_entries)
{
    ...
    for(size_t long_index = 0; long_index < num_longs; long_index++)
    {
        ...
        size_t index_in_long = __builtin_ctzl(~val);
        bit = (long_index * BITS_PER_LONG) + index_in_long;
        dprintk("bitmap_find_first_clear: bit=0x%llx, long_index=0x%llx, "
                "index_in_long=0x%llx, val=0x%llx, ~val=0x%llx\n",
                (ull_t)bit,
                (ull_t)long_index,
                (ull_t)index_in_long,
                (ull_t)val,
                      ...
                      size_t num_needed)
{
    ...
    for(size_t long_index = 0; long_index < num_longs; long_index++)
    {
        ...
        // This "if" is unnecessary but should speed up large range
        // searches
