 ...
 *
 * But it allows every heap allocation to have an unmapped page placed directly
 * after it. And so any heap buffer overflows should trigger a page fault which
 * can be debugged, instead of silently corrupting some other data on the heap.
 */
...
_Static_assert(VMEM_MIN_PAGE_ORDER < CONFIG_HEAP_SIZE_ORDER,
               "VMEM_MIN_PAGE_ORDER must be less than CONFIG_HEAP_SIZE_ORDER");

#define KMALLOC_PAGED_NUM_PAGES                                                \
    (1ULL << (CONFIG_HEAP_SIZE_ORDER - VMEM_MIN_PAGE_ORDER))
static DECLARE_BITMAP(kmalloc_page_bitmap, KMALLOC_PAGED_NUM_PAGES);

static int
kmalloc_paged_page_fault_handler(struct vmem_region_ref *region,
                                 uintptr_t offset,
                                 unsigned long flags,
                                 void *priv_state)
{
...
kmalloc_paged_init(void)
{
    ...
    kmalloc_paged_vmem_region =
        vmem_region_create_paged(1ULL << CONFIG_HEAP_SIZE_ORDER,
    ...
    res = vmem_force_mapping(kmalloc_paged_vmem_region,
...
kmalloc(size_t size)
{
    ...
    size_t pages_needed = ((size + ((1ULL << VMEM_MIN_PAGE_ORDER) - 1ULL)) &
                           ~((1ULL << VMEM_MIN_PAGE_ORDER) - 1ULL)) >>
                          VMEM_MIN_PAGE_ORDER;

    // Guard page at the end
    pages_needed++;

    size_t pageno = bitmap_find_clear_range(kmalloc_page_bitmap,
    ...
    for(size_t i = 0; i < pages_needed - 1; i++)
    {
        ...
        void __phys *page;
        res = page_alloc(VMEM_MIN_PAGE_ORDER, &page, 0);
        if(res)
        {
            kmalloc_paged_lock_release();
    ...
    void *page_end =
        page_base + ((pages_needed - 1) * (1ULL << VMEM_MIN_PAGE_ORDER));
...
{
    int res;
    // We leak memory under this system (Trying to detect memory overruns at
    // boot)
    return;
}
