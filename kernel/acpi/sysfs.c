...
static int
acpi_fs_node_read_page(struct fs_node *fs_node,
                       void *buffer,
                       uintptr_t pfn,
                       unsigned long flags)
{
    ...
    struct acpi_table *ptr = container_of(fs_node->backing.priv_state,
                                          struct acpi_table,
                                          sysfs_node);
    ...
    // Zero the entire buffer no matter what (TODO: We could be smarter about
    // this)
...
static int
acpi_fs_node_getattr(struct fs_node *fs_node, int attr, size_t *value)
{
    int res;
...
static int
do_register_table(struct acpi_table *table)
{
    int res;
    ...
    size_t inode;
    res = vfs_mount_insert_node(acpi_fs_mount, &table->sysfs_node, &inode);
    if(res)
    {
        return res;
    }

    res = vfs_mount_link_root(acpi_fs_mount, table->signature_str, inode);
    if(res)
    {
        vfs_mount_remove_node(acpi_fs_mount, &table->sysfs_node);
        return res;
    }
...
acpi_sysfs_init_mount(void)
{
    ...
    ilist_for_each(node, &sysfs_temp_list)
    {
        struct acpi_table *ptr =
            container_of(node, struct acpi_table, sysfs_temp_list_node);
    ...
    res = sysfs_register_mount(&acpi_fs_mount->fs_mount, "acpi");
    if(res)
    {
        return res;
    }
