...
struct acpi_path *
acpi_path_create(size_t length, int parent_prefix_count)
{
...
void
acpi_path_destroy(struct acpi_path *path)
{
    kfree(path);
}

struct acpi_path *
acpi_path_clone(struct acpi_path *path)
{
    struct acpi_path *clone = acpi_path_create(path->pathlen, path->prefixes);
    if(clone == NULL)
    {
        return NULL;
    }
    memcpy(clone->names,
           path->names,
           sizeof(struct acpi_name) * clone->pathlen);
    return clone;
}

struct acpi_path *
acpi_path_create_absolute(struct acpi_node *scope, struct acpi_path *relpath)
{
    ...
    for(size_t i = 0; i < relpath->prefixes; i++)
    {
        ...
        if(scope == NULL)
        {
            wprintk("acpi_path_create_absolute: Failed to create path "
                    "from "
                    "relative path with invalid number of prefixes!\n");
            return NULL;
        }
    }
    ...
    struct acpi_path *abs_path =
        acpi_path_create(pathlen, ACPI_PATH_PREFIX_ROOT);
    if(abs_path == NULL)
    {
        acpi_node_put(scope);
    ...
    for(size_t i = 0; i < num_ancestors; i++)
    {
        if(ancestor == NULL)
        {
            ...
            wprintk("acpi_path_create_absolute: Failed to get path "
                    "ancestor "
                    "(Unexpected)!\n");
            return NULL;
        }
    ...
    memcpy(&abs_path->names[num_ancestors],
           relpath->names,
           relpath->pathlen * sizeof(struct acpi_name));
...
int
acpi_verify_name(struct acpi_name *name)
{
    char c = name->raw[0];
    if(!(c == '_' || (c >= 'A' && c <= 'Z')))
    {
...
// Returns 0 on success
int
acpi_verify_path(struct acpi_path *path)
{
    if(path->prefixes < -1)
    {
        return -EINVAL;
...
int
acpi_dump_name(printk_f *printer, struct acpi_name *name)
{
    {
        char c = name->raw[0];
        if(c == '_' || (c >= 'A' && c <= 'Z'))
...
int
acpi_dump_path(printk_f *printer, struct acpi_path *path)
{
    int res;
