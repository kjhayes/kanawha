...
ssize_t
aml_parse_opcode(void *data, size_t datalen, aml_opcode_t *out)
{
    int res;
...
int
acpi_interp_peek_aml_opcode(struct acpi_interp_state *state, aml_opcode_t *out)
{
    ssize_t opcode_len =
        aml_parse_opcode(state->frame->aml_data + state->frame->rip,
...
int
acpi_interp_aml_opcode(struct acpi_interp_state *state, aml_opcode_t *out)
{
    ssize_t opcode_len =
        aml_parse_opcode(state->frame->aml_data + state->frame->rip,
                         state->frame->aml_len - state->frame->rip,
