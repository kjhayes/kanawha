...
int
acpi_interp_def_field(struct acpi_interp_state *state)
{
    int res;
    ...
    {
        wprintk("acpi_interp_def_field: pkglength extends beyond current "
                "frame!\n");
        return -EINVAL;
    }
    ...
    struct acpi_obj *op_region =
        acpi_interp_get_named_object(state, op_region_path);
    if(op_region == NULL)
    {
        wprintk("acpi_interp_def_field: Failed to find named op region "
                "at path \"");
    ...
    {
    case 0:
        access = ACPI_FIELD_UNIT_ACCESS_ANY;
        break;
    case 1:
        access = ACPI_FIELD_UNIT_ACCESS_BYTE;
        break;
    case 2:
        access = ACPI_FIELD_UNIT_ACCESS_WORD;
        break;
    case 3:
        access = ACPI_FIELD_UNIT_ACCESS_DWORD;
        break;
    case 4:
        access = ACPI_FIELD_UNIT_ACCESS_QWORD;
        break;
    case 5:
        access = ACPI_FIELD_UNIT_ACCESS_BUFFER;
        break;
    default:
        acpi_obj_put(op_region);
    ...
    {
    case 0:
        update_rule = ACPI_FIELD_UNIT_UPDATE_RULE_PRESERVE;
        break;
    case 1:
        update_rule = ACPI_FIELD_UNIT_UPDATE_RULE_WRITE_ONES;
        break;
    case 2:
        update_rule = ACPI_FIELD_UNIT_UPDATE_RULE_WRITE_ZEROS;
        break;
    default:
        acpi_obj_put(op_region);
    ...
    while(state->frame->rip < target_rip)
    {
        ...
        case 0x00: // Reserved field
            ...
            if(tmp_pkglength < 0)
            {
                wprintk("acpi_interp_def_field: Failed to read "
                        "pkglength of a "
                        "reserved field unit!\n");
        ...
        case 0x01: // AccessField
            wprintk("acpi_interp_def_field: encountered AccessField "
                    "(UNIMPL)\n");
            acpi_path_destroy(tmp_path);
            acpi_obj_put(op_region);
            return -EUNIMPL;
        case 0x02: // ConnectField
            wprintk("acpi_interp_def_field: encountered ConnectField "
                    "(UNIMPL)\n");
            acpi_path_destroy(tmp_path);
            acpi_obj_put(op_region);
            return -EUNIMPL;
        case 0x03: // ExtendedAccessField
            wprintk("acpi_interp_def_field: encountered "
                    "ExtendedAccessField "
                    "(UNIMPL)\n");
            acpi_path_destroy(tmp_path);
            acpi_obj_put(op_region);
        ...
        default: // NamedField
            ...
            if(res)
            {
                wprintk("acpi_interp_def_field: Failed to get "
                        "FieldUnit name!\n");
                acpi_path_destroy(tmp_path);
                acpi_obj_put(op_region);
                return res;
            ...
            tmp_obj = acpi_create_field_unit_obj(op_region,
                                                 tmp_pkglength,
                                                 cur_bitoffset,
                                                 access,
                                                 update_rule,
            ...
            if(tmp_obj == NULL)
            {
                wprintk("acpi_interp_def_field: Failed to create "
                        "FieldUnit "
                        "object!\n");
                acpi_path_destroy(tmp_path);
                acpi_obj_put(op_region);
            ...
            res = acpi_interp_create_named_object(state, tmp_path, tmp_obj);
            if(res)
            {
                wprintk("acpi_interp_def_field: Failed to add "
                        "FieldUnit object "
                        "to namespace!\n");
                acpi_path_destroy(tmp_path);
                acpi_obj_put(op_region);
