	...
	struct acpi_obj **obj)
{
	...
	if(acpi_obj_get_type(resolved) == ACPI_OBJ_TYPE_METHOD) {
	    ...
	    wprintk("acpi_interp_term_arg: Method invokation is unimplemented!\n");
	...
	case AML_STRING_PREFIX:
	    ...
	    if(res) {
		wprintk("acpi_interp_term_arg: Malformed AML_STRING_PREFIX!\n");
		return res;
	    }
	    return 0;
	case AML_BUFFER_OP:
	    ...
	    if(res) {
		wprintk("acpi_interp_term_arg: Failed to interpret DefBuffer! (err=%s)\n",
			errnostr(res));
		return res;
	    }
	    return 0;
	case AML_PACKAGE_OP:
	    ...
	    if(res) {
		wprintk("acpi_interp_term_arg: Failed to interpret DefPackage! (err=%s)\n",
			errnostr(res));
		return res;
	    }
	    return 0;
	case AML_VAR_PACKAGE_OP:
	    ...
	    if(res) {
		wprintk("acpi_interp_term_arg: Failed to interpret DefVarPackage! (err=%s)\n",
			errnostr(res));
		return res;
	    }
	    return 0;
	...
	unsigned long *value)
{
    ...
    res = acpi_obj_get_integral_value(
	    obj,
	    value);
    if(res) {
	acpi_obj_put(obj);
