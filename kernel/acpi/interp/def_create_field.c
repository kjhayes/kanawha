...
static int
acpi_interp_create_field_generic(struct acpi_interp_state *state,
                                 ...
                                 unsigned long bits_per_index)
{
    ...
    if(res)
    {
        wprintk("acpi_interp_create_field_generic: Failed to get resolve "
                "buffer\n");
        return res;
    }
    ...
    unsigned long index;
    res = acpi_interp_term_arg_to_integer(state, &index);
    if(res)
    {
        wprintk("acpi_interp_create_field_generic: Failed to get resolve "
                "index\n");
    ...
    if(bitwidth == 0)
    {
        ...
        if(res)
        {
            wprintk("acpi_interp_create_field_generic: Failed to get "
                    "resolve "
                    "bitwidth\n");
            acpi_obj_put(buffer);
            return res;
    ...
    field = acpi_create_buffer_field_obj(buffer, bitoffset, bitwidth);
    acpi_obj_put(buffer);
    if(field == NULL)
    {
        return -ENOMEM;
    ...
    res = acpi_interp_create_named_object(state, name_string, field);
    acpi_obj_put(field);
...
int
acpi_interp_def_create_bit_field(struct acpi_interp_state *state)
{
    return acpi_interp_create_field_generic(state, 1, 1);
}
int
acpi_interp_def_create_byte_field(struct acpi_interp_state *state)
{
    return acpi_interp_create_field_generic(state, 8, 8);
}
int
acpi_interp_def_create_word_field(struct acpi_interp_state *state)
{
    return acpi_interp_create_field_generic(state, 16, 8);
}
int
acpi_interp_def_create_dword_field(struct acpi_interp_state *state)
{
    return acpi_interp_create_field_generic(state, 32, 8);
}
int
acpi_interp_def_create_qword_field(struct acpi_interp_state *state)
{
    return acpi_interp_create_field_generic(state, 64, 8);
}
int
acpi_interp_def_create_field(struct acpi_interp_state *state)
{
