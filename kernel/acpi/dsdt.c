...
static int
acpi_load_aml_into_namespace(void *aml_data, size_t aml_len)
{
    int res;
    ...
    if(ns == NULL)
    {
        wprintk("Could not find ACPI default namespace: Failing to load AML "
                "table!\n");
        return -EDEFER;
    }
    ...
    res = acpi_interpret_aml(scope, aml_data, aml_len);
    if(res)
    {
...
acpi_load_dsdt_into_namespace(void)
{
    ...
    res = acpi_load_aml_into_namespace(aml_data, aml_len);
    if(res)
    {
...
declare_init_desc(bus,
                  acpi_load_dsdt_into_namespace,
                  "Loading ACPI DSDT into Namespace");
