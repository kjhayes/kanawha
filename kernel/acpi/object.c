...
acpi_field_unit_update_rule_to_string(
	acpi_field_unit_update_rule_t type)
{
    switch(type) {
#define ACPI_FIELD_UNIT_UPDATE_RULE_TO_STRING_CASE(__NAME,__PRETTY,...)\
	case ACPI_FIELD_UNIT_UPDATE_RULE_ ## __NAME:\
	    return #__PRETTY;
ACPI_FIELD_UNIT_UPDATE_RULE_XLIST(ACPI_FIELD_UNIT_UPDATE_RULE_TO_STRING_CASE)
	...
	struct acpi_obj *ref)
{
    ...
    struct acpi_obj *obj = acpi_node_get_named_object(
	    ref->typedata.named_reference.scope,
...
struct acpi_obj *
acpi_create_reference_obj(
	struct acpi_obj *referenced)
{
    struct acpi_obj *obj = acpi_obj_create();
    if(obj == NULL) {
	return NULL;
...
acpi_obj_clone(struct acpi_obj *obj)
{
	...
	case ACPI_OBJ_TYPE_BUFFER:
	    return acpi_create_buffer_obj(
		    obj->typedata.buffer.len,
		    obj->typedata.buffer.data,
		    obj->typedata.buffer.len);
	case ACPI_OBJ_TYPE_PACKAGE:
	    {
		struct acpi_obj *package = acpi_create_package_obj(
			obj->typedata.package.len);
		for(size_t i = 0; i < obj->typedata.package.len; i++) {
		    struct acpi_obj *inner = obj->typedata.package.objs[i];
		    if(inner != NULL) {
	...
	struct acpi_obj *obj)
{
	...
	case ACPI_OBJ_TYPE_OP_REGION:
	    ...
	    break;
	case ACPI_OBJ_TYPE_FIELD_UNIT:
	    (*printer)("Bits=0x%lx,Offset=0x%lx,%s,%s,%s,OpRegion=",
		    ...
		    acpi_field_unit_update_rule_to_string(obj->typedata.field_unit.update_rule),
	...
	case ACPI_OBJ_TYPE_METHOD:
	    (*printer)("Arg-Count=%u,Sync-Level=0x%x,%s",
		    ...
		    obj->typedata.method.serialized ? "Serialized" : "Unserialized"
		    );
	    break;
	case ACPI_OBJ_TYPE_BUFFER:
	    (*printer)("Length=0x%lx",
		    (ul_t)obj->typedata.buffer.len);
	    break;
	case ACPI_OBJ_TYPE_MUTEX:
	    (*printer)("Sync-Level=0x%x",
		    (u_t)obj->typedata.mutex.sync_level);
	    break;
	case ACPI_OBJ_TYPE_PACKAGE:
	    for(size_t i = 0; i < obj->typedata.package.len; i++) {
		struct acpi_obj *inner = obj->typedata.package.objs[i];
	...
	case ACPI_OBJ_TYPE_REFERENCE:
		...
		case ACPI_OBJ_TYPE_NAMED_REFERENCE:
		    ...
		    break;
		default:
	            (*printer)(acpi_obj_type_to_string(obj->typedata.reference.obj->type));
		    break;
	    }
	    break;
	case ACPI_OBJ_TYPE_NAMED_REFERENCE:
		...
		struct acpi_path *path =
		    acpi_path_create_absolute(
			    obj->typedata.named_reference.scope,
