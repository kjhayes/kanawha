...
acpi_node_create(struct acpi_namespace *ns, struct acpi_name *name)
{
...
static inline int
__acpi_node_destroy_lockless(struct acpi_node *node)
{
    int res;
    node->destroyed = 1;
...
acpi_node_destroy(struct acpi_node *node)
{
    int res;
...
static inline void
acpi_node_deallocate(struct acpi_node *node)
{
...
static void
acpi_destroy_namespace(struct acpi_namespace *ns)
{
...
void
acpi_node_get(struct acpi_node *node)
{
    atomic_fetch_inc(&node->refcount);
}

void
acpi_node_put(struct acpi_node *node)
{
...
struct acpi_node *
acpi_node_get_parent(struct acpi_node *node)
{
    struct acpi_node *parent;
...
struct acpi_node *
acpi_namespace_get_root(struct acpi_namespace *ns)
{
...
struct acpi_node *
acpi_node_lookup_name(struct acpi_node *node, struct acpi_name *name)
{
    struct acpi_node *child;
    rlock_read_lock(&node->ns->lock);
...
acpi_node_lookup_ancestor(struct acpi_node *scope,
...
struct acpi_node *
acpi_node_lookup(struct acpi_node *scope, struct acpi_path *path)
{
    return acpi_node_lookup_ancestor(scope, path, 0);
}

static struct acpi_node *
acpi_node_lookup_parent(struct acpi_node *scope, struct acpi_path *path)
{
    return acpi_node_lookup_ancestor(scope, path, 1);
}

int
acpi_node_create_named_object(struct acpi_node *scope,
                              struct acpi_path *path,
                              struct acpi_obj *object)
{
    ...
    // We have a reference to both parent and child, so they can't be destroyed
    // from underneath us.
...
struct acpi_obj *
acpi_node_get_named_object(struct acpi_node *scope, struct acpi_path *path)
{
    int res;
...
static void
__acpi_namespace_dump_node_lockless(struct acpi_node *node,
                                    printk_f *printer,
                                    int depth)
{
...
void
acpi_namespace_dump(printk_f *printer, struct acpi_namespace *ns)
{
...
void
acpi_node_dump_path(printk_f *printer, struct acpi_node *node)
{
    if(node->parent)
    {
...
struct acpi_name
acpi_node_get_name(struct acpi_node *node)
{
    return node->name;
}
...
static int
acpi_namespace_create_predefined_scope(struct acpi_namespace *ns,
                                       const char *name)
{
    int res;
    ...
    res = acpi_node_create_named_object(root, tmp_path, tmp_obj);
    if(res)
    {
...
declare_init_desc(dynamic,
                  acpi_create_default_namespace,
                  "Creating Default ACPI Namespace");
...
//     return 0;
// }
// declare_init_desc(launch, acpi_dump_namespace_on_launch, "Dumping Default
// ACPI Namespace");
