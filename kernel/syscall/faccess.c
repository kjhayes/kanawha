...
#define LOG(fmt, ...) printk("PID(%ld) syscall_faccess: " fmt, process->id, ##__VA_ARGS__)
#else
#define LOG(...)
#endif
...
#define SUPPORTED_MODE_FLAGS \
    ...
    if(mode == FACCESS_MODE_EXACT) {
        file->mode_flags = (file->mode_flags & ~SUPPORTED_MODE_FLAGS) | mode_flags;
