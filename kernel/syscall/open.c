...
int
syscall_open(const char __user *path,
             ...
             fd_t __user *fd)
{
    ...
    if(res)
    {
        LOG("syscall_open: failed to read file path! "
            "process_read_usermem(%p) "
            "-> %s\n",
            path,
            errnostr(res));
        return res;
    ...
    LOG("PID(%ld) syscall_open(path=%s, path_len=0x%llx, access_flags=0x%llx, "
        "mode_flags=0x%llx)\n",
        process->id,
        path_buf,
        path_len,
    ...
    {
        fd_t dir_fd;
        res = process_read_usermem(process, &dir_fd, fd, sizeof(dir_fd));
        if(res)
        {
            return res;
        }
        struct file *dir_file;
        dir_file = file_table_get_file(process->file_table, process, dir_fd);
    ...
    res = process_write_usermem(process, fd, &kernel_fd, sizeof(fd_t));
    if(res)
    {
        file_table_close(process->file_table, process, kernel_fd);
        LOG("PID(%ld) syscall_open: Failed to write FID back to "
            "userspace!\n",
            (sl_t)process->id);
        return res;
    }
    ...
    {
        struct file *file =
            file_table_get_file(process->file_table, process, kernel_fd);
        if(file == NULL)
        {
            LOG("PID(%ld) syscall_open: Failed to open file for "
                "truncation! "
                "(err=%s)\n",
                process->id,
                errnostr(res));
        ...
        res = fs_node_setattr(fs_node, FS_NODE_ATTR_DATA_SIZE, 0);
        if(res)
        {
            LOG("PID(%ld) syscall_open: Failed to truncate file! "
                "(err=%s)\n",
                process->id,
                errnostr(res));
            file_table_put_file(process->file_table, process, file);
