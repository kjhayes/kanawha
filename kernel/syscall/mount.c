...
int
syscall_mount(const char __user *src,
              fd_t dst_dir,
              ...
              unsigned long flags)
{
    ...
    struct fs_type *type = NULL;
    {
        ...
        if(res)
        {
            eprintk("PID(%ld) syscall_mount: Failed to get fs_type "
                    "strlen (err=%s)\n",
                    (sl_t)process->id,
                    errnostr(res));
            return res;
        ...
        {
            kfree(fs_type_buf);
            eprintk("PID(%ld) syscall_mount: Failed to read fs_type "
                    "string (err=%s)\n",
                    (sl_t)process->id,
                    errnostr(res));
            return -ENOMEM;
...

    {
        ...
        if(res)
        {
            eprintk("PID(%ld) syscall_mount: Failed to get source strlen "
                    "(err=%s)\n",
                    (sl_t)process->id,
                    errnostr(res));
            return res;
        ...
        if(res)
        {
            eprintk("PID(%ld) syscall_mount: Failed to read string from "
                    "usermem! "
                    "(err=%s)\n",
                    (sl_t)process->id,
                    errnostr(res));
        ...
        if(flags & MOUNT_SPECIAL)
        {
            ...
            if(res)
            {
                eprintk("PID(%ld) syscall_mount: Failed to mount "
                        "special fs "
                        "mount (id=%s) (err=%s)\n",
                        (sl_t)process->id,
                        src_buf,
        ...
        else
        {
            ...
            if(res)
            {
                eprintk("PID(%ld) syscall_mount: Failed to open file "
                        "\"%s\"\n",
                        (sl_t)process->id,
                        src_buf);
                kfree(src_buf);
            ...
            struct file *src_desc =
                file_table_get_file(process->file_table, process, src_fd);
            if(src_desc == NULL)
            {
                eprintk("PID(%ld) syscall_mount: Failed to get file "
                        "descriptor "
                        "for \"%s\"\n",
                        (sl_t)process->id,
                        src_buf);
                kfree(src_buf);
                file_table_close(process->file_table, process, src_fd);
                return -EINVAL;
            }
            ...
            if(node == NULL)
            {
                file_table_put_file(process->file_table, process, src_desc);
                kfree(src_buf);
                file_table_close(process->file_table, process, src_fd);
                return -EINVAL;
            }

            res = fs_type_mount_file(type, node, &mnt);
            ...
            if(res)
            {
                eprintk("PID(%ld) syscall_mount: fs_type_mount_file "
                        "returned %s!\n",
                        process->id,
                        errnostr(res));
                kfree(src_buf);
    ...
    if(res)
    {
        eprintk("PID(%ld) syscall_mount: Failed to get destination "
                "strlen from "
                "userspace! (err=%s)\n",
                (sl_t)process->id,
                errnostr(res));
    ...
    if(res)
    {
        eprintk("PID(%ld) syscall_mount: Failed to read destination string "
                "from userspace! (err=%s)\n",
                (sl_t)process->id,
                errnostr(res));
        kfree(dst_name_buf);
        return res;
    ...
    struct file *dst_desc =
        file_table_get_file(process->file_table, process, dst_dir);
    if(dst_desc == NULL)
    {
        kfree(dst_name_buf);
        eprintk("PID(%ld) syscall_mount: Could not find destination directory "
                "descriptor!\n",
                (sl_t)process->id);
        return -ENXIO;
    }
    ...
    res = fs_path_mount_dir(dst_desc->path, dst_name_buf, mnt, &mnt_point);
    if(res)
    {
        eprintk("PID(%ld) syscall_mount: Failed to attach mount point to "
                "directory! (err=%s)\n",
                (sl_t)process->id,
                errnostr(res));
        kfree(dst_name_buf);
        file_table_put_file(process->file_table, process, dst_desc);
        return res;
    }
...
int
syscall_unmount(fd_t mntpoint_fd)
{
    struct process *process = current_process();
    return -EUNIMPL;
}
