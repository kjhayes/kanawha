...
static void
perf_counter_display(struct perf_metric *metric, printk_f *printer)
{
...
static void
perf_timer_display(struct perf_metric *metric, printk_f *printer)
{
    struct perf_timer *timer = container_of(metric, struct perf_timer, metric);
...
int
perf_counter_register(struct perf_counter *counter, const char *name)
{
    return register_perf_metric(&counter->metric, name);
}
int
perf_counter_unregister(struct perf_counter *counter)
{
...
int
perf_timer_register(struct perf_timer *timer, const char *name)
{
    return register_perf_metric(&timer->metric, name);
}
int
perf_timer_unregister(struct perf_timer *timer)
{
...
static void
dump_perf_metrics_callback(struct perf_metric *metric, void *_printer)
{
...
{
    for_each_perf_metric(dump_perf_metrics_callback, printer);
}

DEFINE_REGISTRY(perf_metric,
                registry_node,
