...
__fdt_node_find_parent_helper(
        struct fdt *fdt,
        struct fdt_node *parent,
        struct fdt_node *child)
{
    struct fdt_node *iter =
    ...
    while(iter) {
        ...
        struct fdt_node *rec =
            __fdt_node_find_parent_helper(fdt,iter,child);
        if(rec != NULL) {
            return rec;
        }
        ...
        struct fdt_node *node)
{
    ...
    return __fdt_node_find_parent_helper(
            fdt,
            root,
            node);
}
        ...
        struct fdt_node *node)
{
    ...
    size_t len =
        fdt_property_size(
            ...
            prop);
    if(len != 4) {
        wprintk("fdt_node_address_cells: found #address-cells property but len=%lu!\n",
                (ul_t)len);
        return address_cells;
    }
        ...
        struct fdt_node *node)
{
    ...
    size_t len =
        fdt_property_size(
            ...
            prop);
    if(len != 4) {
        wprintk("fdt_node_size_cells: found #size-cells property but len=%lu!\n",
                (ul_t)len);
        return size_cells;
    }
    ...
    for(size_t i = 0; i < buflen; i++) {
        ...
        switch(addr_cells) {
            case 1:
                ...
                break;
            case 2:
                addr_buf[i] = (void __phys *)(uintptr_t)fdttoh64(*(fdt64_t*)addr_ptr);
                break;
            default:
                return -EINVAL;
        }
        ...
        struct fdt *fdt)
{
    ...
    (*printer)("FDT@%p (totalsize=0x%lx) {\n", fdt, fdttoh32(fdt->hdr.totalsize));
