...
int
dt_node_check_id(struct dt_node *node, struct dt_node_id *id)
{
    struct fdt *fdt = devtree_get_fdt(node->dt);

    DEBUG_ASSERT(id->name != NULL || id->type != NULL ||
                 id->compatible != NULL);
    if(id->name != NULL)
    {
...
static int
devtree_try_match(struct dt_driver *driver, struct dt_node *node)
{
    int res;
    int matched_id = 0;
    ...
    res = dt_driver_probe(driver, node);
    if(res)
    {
        return res;
    }

    res = dt_driver_init_node(driver, node);
    if(res)
    {
        return res;
    }
...
static int
__init_dt_driver(struct dt_driver *driver)
{
...
int
register_dt_driver(struct dt_driver *driver)
{
    int res;
    ...
    ilist_for_each(list_node, &devtree_node_list)
    {
        ...
        int res = devtree_try_match(driver, node);
        if(res)
        {
            // We don't actually care if we fail to match with any given
            // node
            continue;
        }
...
int
register_dt_driver_nomatch(struct dt_driver *driver)
{
    int res;
    res = __init_dt_driver(driver);
    if(res)
...
static int
register_dt_node(struct dt_node *node)
{
    int res;
...
static int
register_dt_node_and_children(struct dt_node *node)
{
    int res;
...
int
dt_driver_claim_node(struct dt_driver *driver, struct dt_node *node)
{
    int res;
