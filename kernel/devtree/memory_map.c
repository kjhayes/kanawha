...
fdt_find_memory(void)
{
    ...
    { // Checking Device Type
        struct fdt_property *device_type =
            fdt_find_property_by_name(fdt, memory_node, "device_type");
        if(device_type == NULL)
        {
            wprintk("FDT \"memory\" node is missing \"device_type\" "
                    "property\n");
        }
        else
        {
            ...
            {
                wprintk("FDT \"memory\" node has invalid "
    ...
    res = fdt_node_read_reg(fdt, memory_node, num_regs, addr_buf, size_buf);
    if(res)
    {
        return res;
    }
    ...
    {
        res = mem_flags_set_flags(phys_map,
    ...
    struct fdt_node *resv_memory_node =
        fdt_find_node_by_unitname(fdt, "reserved-memory");
    if(resv_memory_node != NULL)
    {
        struct fdt_node *resv_child =
            fdt_node_first_subnode(fdt, resv_memory_node);
        while(resv_child)
        {
            ...
            if(res)
            {
                wprintk("Failed to read reg property of reserved "
                        "memory region "
                        "in device tree! (err=%s)\n",
                        errnostr(res));
            ...
            struct fdt_property *no_map_prop =
                fdt_find_property_by_name(fdt, resv_child, "nomap");
            if(no_map_prop != NULL)
            {
                wprintk("Device tree reserved-memory node has "
                        "\"nomap\" "
            ...
            for(size_t i = 0; i < num_reg; i++)
            {
                ...
                res = mem_flags_clear_flags(phys_map,
                                            (uintptr_t)bases[i],
                                            sizes[i],
    ...
    while(fdttoh64(entry->size) != 0)
    {
        ...
        res = mem_flags_set_flags(phys_map, addr, size, PHYS_MEM_FLAGS_FW_RESV);
        if(res)
        {
            wprintk("Failed to mark physical memory region [%p - %p) as "
                    "firmware reserved!\n",
                    addr,
                    size);
            entry++;
        ...
        res = mem_flags_clear_flags(phys_map, addr, size, PHYS_MEM_FLAGS_AVAIL);
        if(res)
        {
            wprintk("Failed to mark physical memory region [%p - %p) as "
                    "unavailable!\n",
...
declare_init_desc(mem_flags,
                  fdt_find_memory,
                  "Finding Memory Region(s) in FDT");
