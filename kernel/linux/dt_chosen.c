...
static void
linux_chosen_reserve_mem_flags_initrd(
        struct fdt *fdt,
        struct fdt_node *node)
{
    ...
    uintptr_t start;
    switch(start_size) {
        case 1: start = *(uint8_t*)start_data; break;
        case 2: start = fdttoh16(*(fdt16_t*)start_data); break;
        case 4: start = fdttoh32(*(fdt32_t*)start_data); break;
        case 8: start = fdttoh64(*(fdt64_t*)start_data); break;
        default: return;
    }

    uintptr_t end;
    switch(end_size) {
        case 1: end = *(uint8_t*)end_data; break;
        case 2: end = fdttoh16(*(fdt16_t*)end_data); break;
        case 4: end = fdttoh32(*(fdt32_t*)end_data); break;
        case 8: end = fdttoh64(*(fdt64_t*)end_data); break;
        default: return;
    }
        ...
        struct dt_node *node)
{
    ...
    if(res) {
        eprintk("Device tree initrd either was not properly reserved by the firmware or overlaps the kernel! (ignoring...)\n");
        return;
    }
...
declare_init_desc(mem_flags, linux_dt_chosen_reserve_mem_flags, "Reserving Memory for /chosen Node in Device Tree");
