...
static void
mb2_rsdp_handler(struct mb2_info *info, struct mb2_info_tag *tag, void *state)
{
    int *res = state;
    if(*res)
    {
    ...
    if(tag->hdr.type == MULTIBOOT2_INFO_TAG_RSDP_v1)
    {
        ...
        struct acpi_rsdp *rsdp =
            ((void *)tag) + sizeof(struct mb2_info_tag_header);
...
{
    int res = 0;
    mb2_info_for_each_tag(boot_mb2_info_ptr, mb2_rsdp_handler, &res);
    return res;
}
declare_init_desc(boot,
                  mb2_find_acpi_rsdp,
                  "Search Multiboot2 Info for ACPI RSDP");
