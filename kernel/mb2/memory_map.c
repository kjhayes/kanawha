...
static void
mb2_memory_info_handler(struct mb2_info *info,
...
mb2_handle_mem_flags(struct mb2_info_tag *tag, struct mem_flags *map)
{
    ...
    for(size_t off = 16; off < tag->hdr.size; off += tag->mem_map.entry_size)
    {
        ...
        default: // Ignore this region
            ...
            to_clear = PHYS_MEM_FLAGS_AVAIL;
            dprintk("Found Unknown Region Type in MB2 Memory Info [%p-%p) "
                    "type=%d!\n",
                    entry->base,
...
mb2_handle_basic_info(struct mb2_info_tag *tag, struct mem_flags *map)
{
    ...
    if(res)
    {
        printk("Failed to add Multiboot2 basic_mem_info lower mem [%p - "
               "%p) to "
               "the memory map! (err=%s)\n",
               (uintptr_t)(0x0),
               (uintptr_t)(0x0 + tag->basic_mem_info.mem_lower),
               errnostr(res));
        return res;
    }
    res = mem_flags_set_flags(map,
    ...
    if(res)
    {
        printk("Failed to add Multiboot2 basic_mem_info upper mem [%p - "
               "%p) to "
               "the memory map! (err=%s)\n",
               (uintptr_t)(0x100000),
               (uintptr_t)(0x100000 + tag->basic_mem_info.mem_upper),
               errnostr(res));
        return res;
    }
    return 0;
...
mb2_init_memory_map(struct mb2_info *info, struct mem_flags *map)
{
    ...
    if(mem_info.mem_map_found)
    {
        ...
        if(res)
        {
            printk("Failed to get memory flags info from Multiboot2 "
                   "mem_map "
                   "tag (err=%s)\n",
                   errnostr(res));
        }
...
static void
mb2_module_reserve_handler(struct mb2_info *info,
                           struct mb2_info_tag *tag,
                           void *state)
{
    int *res = state;
    ...
    case MB2_INFO_TAG_TYPE_MODULE:
        *res =
            mem_flags_clear_flags(get_phys_mem_flags(),
...
#ifdef CONFIG_MULTIBOOT2_MODULES
    mb2_info_for_each_tag(boot_mb2_info_ptr, mb2_module_reserve_handler, &res);
#endif
...
declare_init_desc(mem_flags,
                  mb2_mem_flags_init,
                  "Initializing Physical Memory Flags from Multiboot2 Info");
declare_init_desc(post_mem_flags,
                  mb2_reserve_info_struct,
                  "Reserving Multiboot2 Info Structure");
