...
static_bsp_init_builtin_percpu(void)
{
    ...
    // CPU 0 (the BSP) just uses the inplace percpu section for builtin percpu
    // variables during boot (eventually we will need to transfer to a percpu
    // heap to allow allocations)
...
declare_init_desc(static,
                  static_bsp_init_builtin_percpu,
                  "Initializing BSP builtin percpu Variables");
...
bsp_transfer_to_percpu_heap(struct cpu *bsp)
{
    ...
    size_t size =
        (uintptr_t)__builtin_kpercpu_end - (uintptr_t)__builtin_kpercpu_start;
    printk(
        "Copying perpcu data area of BSP [%p - %p) to percpu heap [%p - %p)\n",
        __builtin_kpercpu_start,
        __builtin_kpercpu_end,
        bsp->percpu_data,
        bsp->percpu_data + size);

    memcpy(bsp->percpu_data,
    ...
    if(bsp_id != bsp->id)
    {
        panic("Corrupted percpu data area when transferring BSP to percpu "
              "heap!\n");
    }

    if(current_thread() != cur_thread)
    {
        panic("Corrupted percpu data area when transferring BSP to "
              "percpu heap! "
    ...
    // In-case the architecture doesn't uses a different mechanism for remote
    // CPU's still place this call
...
static int
percpu_heap_page_fault_handler(struct excp_state *state,
...
init_cpu_percpu_data(struct cpu *cpu)
{
    ...
    heap->region =
        vmem_region_create_paged(1ULL << CONFIG_PERCPU_HEAP_SIZE_ORDER,
...
#ifdef CONFIG_PERCPU_DEBUG_ASSERTIONS
        *(uint64_t *)percpu_ptr_specific(percpu_addr(__percpu_assert_checksum),
                                         cpu->id) =
...
/*
 * These are not fast, with the percpu heaps we are more focused on saving
 * space, because the percpu heap(s) are much more limited than the kmalloc
 * heap
 */
...
int
__percpu_grow_heaps(size_t size)
{
...
percpu_alloc(size_t size)
{
    ...
    while(ptr == PERCPU_NULL)
    {
        ...
        ilist_for_each(node, &percpu_heap_free_list)
        {
            ...
            if((uintptr_t)base & ((1ULL << KMALLOC_ALIGN_ORDER) - 1))
            {
                size_t misalign =
                    ...
                    ((uintptr_t)base & ((1ULL << KMALLOC_ALIGN_ORDER) - 1));
        ...
        if(to_use == NULL)
        {
            ...
            int res = __percpu_grow_heaps(size < PERCPU_HEAP_MAX_GROW_STEP
                                              ? size
                                              : PERCPU_HEAP_MAX_GROW_STEP);
            if(res)
            {
                percpu_heap_lock_release();
                return PERCPU_NULL;
        ...
        if(base != to_use_start)
        {
            ...
            if(to_use_end != base + size)
            {
                ...
                struct percpu_heap_free_region *after_region =
                    kmalloc(sizeof(struct percpu_heap_free_region), KM_KERNEL);
                ...
                ilist_push_tail(&percpu_heap_free_list,
                                &after_region->list_node);
            }
        }
...
__percpu_free(void __percpu *ptr, size_t size)
{
    ...
    ilist_for_each(node, &percpu_heap_free_list)
    {
        ...
        if(region->start_offset == end)
        {
            ...
            {
                eprintk("percpu heap corrupted! (multiple free "
                        "regions have same "
                        "start_offset)\n");
                return -EINVAL;
            }
