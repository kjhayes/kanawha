...
int
ipv4_dev_send_icmp(struct ipv4_dev *dev,
                   ...
                   size_t datalen)
{
    ...
    pkt = ipv4_dev_alloc_packet(dev,
                                sizeof(struct ipv4_raw_packet) +
                                    sizeof(struct icmp_pkt_hdr),
                                0);
    if(pkt == NULL)
    {
        return -ENOMEM;
    ...
    ipv4_pkt_hdr_set_total_length(hdr,
                                  sizeof(struct ipv4_raw_packet) +
                                      sizeof(struct icmp_pkt_hdr));
...
int
ipv4_dev_send_icmp_ping(struct ipv4_dev *dev,
                        struct ipv4_addr src,
                        struct ipv4_addr dst)
{
    return ipv4_dev_send_icmp(dev,
                              src,
                              dst,
