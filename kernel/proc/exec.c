...
static void
process_exec_type_probe_callback(struct exec_type *exec_type,
             ...
             unsigned long exec_flags)
{
    ...
    for_each_exec_type(process_exec_type_probe_callback, &probe_state);
    ...
    else if((probe_state.status == EXEC_TYPE_PROBE_MAYBE) &&
            !(exec_flags & EXEC_PERMISSIVE))
    {
        return -EINVAL;
    }
    ...
    if(res)
    {
        wprintk("syscall_exec: Failed to handle CLOSE_ON_EXEC files! "
                "(err=%s)\n",
                errnostr(res));
    }
