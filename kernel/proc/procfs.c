...
init_procfs_mount(void)
{
    ...
    res = sysfs_register_mount(&procfs_mount->fs_mount, "proc");
    if(res)
    {
...
static int
__procfs_read_user_id(unsigned long *out, void *state)
{
...
static int
__procfs_read_group_id(unsigned long *out, void *state)
{
    struct process *proc = state;
...
static int
__procfs_read_parent_pid(unsigned long *out, void *state)
{
    struct process *proc = state;
    pid_t parent_id;
