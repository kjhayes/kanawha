...
sysfs_register_mount(struct fs_mount *mnt, const char *id)
{
    ...
    struct sysfs_mount *sysmnt =
        kzmalloc(sizeof(struct sysfs_mount), KM_KERNEL);
    if(sysmnt == NULL)
    {
        sysfs_mount_lock_release();
        return -ENOMEM;
...
sysfs_unregister_mount(const char *id)
{
    ...
    struct stree_node *rem = stree_remove(&sysfs_mount_tree, id);

    DEBUG_ASSERT(KERNEL_ADDR(rem));

    struct sysfs_mount *mnt = container_of(rem, struct sysfs_mount, tree_node);
...
static int
sysfs_mount_special(struct fs_type *type, const char *id, struct fs_mount **out)
{
    int res;
    sysfs_mount_lock_acquire();
...
static int
sysfs_unmount(struct fs_type *type, struct fs_mount *mnt)
{
    return 0;
}
