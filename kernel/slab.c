...
static int
slab_allocator_add_block(struct slab_allocator *alloc,
                         void *block_base,
                         size_t block_size,
                         unsigned long flags)
{
    ...
    {
        size_t mis_align =
            ((uintptr_t)block % alignof(struct slab_alloc_block));
        size_t patch =
            mis_align == 0 ? 0 : alignof(struct slab_alloc_block) - mis_align;
        overhead += patch;
        useful_mem -= patch;
    ...
    size_t bitmap_size =
        (max_obj / BITS_PER_LONG + ((max_obj % BITS_PER_LONG) != 0)) *
    ...
    // Align the object array
    {
        ...
        size_t patch =
            mis_align == 0 ? 0 : (1ULL << alloc->obj_align) - mis_align;
        overhead += patch;
        useful_mem -= patch;
...
create_static_slab_allocator(void *buffer,
                             size_t buffer_size,
                             size_t obj_size,
                             order_t obj_align)
{
...

    {
        ...
        size_t patch =
            mis_align == 0 ? 0 : alignof(struct slab_allocator) - mis_align;
        useful_mem -= patch;
    ...
    if(useful_mem > 0)
    {
        ...
        int res = slab_allocator_add_block(alloc,
                                           block_base,
                                           useful_mem,
...
create_dynamic_slab_allocator(size_t obj_size, order_t obj_align)
{
    ...
    alloc =
        create_static_slab_allocator(buffer, buffer_size, obj_size, obj_align);
...
recurse:
        ...
        node = ptree_get_next(node))
    {
        ...
        dprintk("slab_alloc: Checking block %p: num_free=0x%llx, "
                "num_slots=0x%llx\n",
                block,
                (ull_t)block->num_free,
                (ull_t)block->num_slots);
        size_t first_free =
            bitmap_find_first_clear(block->bitmap, block->num_slots);
        if(first_free >= block->num_slots)
        {
            eprintk("slab_alloc Found block with num_free != 0 but "
                    "bitmap has no "
                    "free bits! (trying to correct)\n");
            block->num_free = 0;
            continue;
        }
        block->num_free--;
        ...
        if(block->num_free == 0)
        {
            ...
            ptree_insert(&alloc->full_block_tree,
                         &block->pnode,
                         (uintptr_t)block);
        }
...
slab_free(struct slab_allocator *alloc, void *obj)
{
    ...
    do
    { // Try and find the block in the "block_tree"
        struct ptree_node *node =
            ptree_get_max_less(&alloc->block_tree, (uintptr_t)obj);
        ...
        void *objs_end =
            cur_block->objects + (cur_block->num_slots * alloc->obj_size);
    ...
    if(block == NULL)
    { // Look for the block in the "full_block_tree"
        struct ptree_node *node =
            ptree_get_max_less(&alloc->full_block_tree, (uintptr_t)obj);
