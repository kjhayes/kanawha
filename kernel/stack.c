...
static int
thread_stack_page_fault_handler(struct excp_state *state,
                                ...
                                void *priv_state)
{
    ...
    {
        eprintk("Page fault on stack [%p - %p) region [%p - %p) page=%p "
                "without a current thread!\n",
                stack->stack_top,
                stack->stack_base,
                stack->virt_base,
...
thread_stack_init(struct thread_stack *stack, order_t order)
{
    ...
    stack->region =
        vmem_region_create_paged(1ULL << THREAD_STACK_VIRT_SIZE_ORDER,
    ...
    res = vmem_paged_region_map(stack->region,
    ...
    res = vmem_force_mapping(stack->region, (void *)stack->virt_base);
    if(res)
    {
...
err3:
    mem_flags_set_flags(get_virt_mem_flags(),
                        stack->virt_base,
