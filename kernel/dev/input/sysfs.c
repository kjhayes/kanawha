...
static int
input_dev_fs_probe_input_dev(struct input_dev *dev)
{
    return 0;
}

static int
input_dev_fs_receive_input_dev(struct input_dev *dev)
{
    int res;
    ...
    res = vfs_mount_insert_node_and_link_root(input_dev_fs_mount,
                                              &node->vfs_node,
...
static int
input_dev_fs_revoke_input_dev(struct input_dev *dev)
{
    wprintk("input_dev_fs_on_unregister is undefined!\n");
    return -EUNIMPL;
}

static struct input_dev_owner input_dev_fs_owner = {
...
static ssize_t
input_fs_file_read(struct file *file,
                   void *buffer,
                   ssize_t amount,
                   unsigned long flags)
{
    ...
    struct input_dev_fs_node *kbfs =
        container_of(vfs_node, struct input_dev_fs_node, vfs_node);
    ...
    {
        res = input_driver_dequeue_event(input, &event);
        if(res == -EWOULDBLOCK && num_events_written <= 0)
        {
...
static int
input_fs_file_poll(struct file *file, unsigned long in, unsigned long *out)
{
    struct fs_node *fs_node = fs_path_get_fs_node(file->path);
    if(fs_node == NULL)
    {
