...
input_dev_udrv_create(struct udrv_mount *mnt, const char *name)
{
    int res;
...
static int
input_dev_udrv_destroy(struct udrv_mount *mnt, struct udrv_dev *udrv_dev)
{
    int res;

    struct udrv_input_dev *dev =
        container_of(udrv_dev, struct udrv_input_dev, udrv_dev);
...
static int
input_dev_udrv_on_recv(struct udrv_mount *mnt,
                       struct udrv_dev *udrv_dev,
                       struct udrv_pkt *pkt,
                       size_t pktlen)
{
    ...
    case UDRV_INPUT_PKT_PROVIDE_INPUT:
        ...
        {
            wprintk("Userspace provided %s with a PROVIDE_INPUT packet of "
                    "length 0x%lx < 0x%lx!\n",
                    pktlen,
        ...
        res =
            input_driver_enqueue_event(input, (struct input_event *)pkt->data);
        if(res)
        {
            return res;
        }
...
static struct udrv_mount_ops input_dev_udrv_mount_ops = {
    .create = input_dev_udrv_create,
    .destroy = input_dev_udrv_destroy,
    .on_recv = input_dev_udrv_on_recv,
};

static struct udrv_mount input_dev_udrv_mount = {
...
declare_init_desc(fs,
                  register_input_dev_udrv,
                  "Registering input_dev Userspace Driver Framework");
