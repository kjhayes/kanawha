...
term_dev_init(struct term_dev *dev)
{
    ...
    dev->buffer_queue = kmalloc(sizeof(struct term_buffer) * dev->buffer_queue_len, KM_KERNEL);
...
static inline int
term_driver_raw_input_full_unlocked(
	struct term_dev *dev)
{
...
static inline int
term_driver_raw_input_empty_unlocked(
	struct term_dev *dev)
{
...
static inline int
term_driver_buffer_queue_full_unlocked(
	struct term_dev *dev)
{
    return (dev->buffer_queue_head+1 == dev->buffer_queue_tail)
	|| ((dev->buffer_queue_head+1 == dev->buffer_queue_len) && (dev->buffer_queue_tail == 0));
}

static inline int
term_driver_buffer_queue_empty_unlocked(
	struct term_dev *dev)
{
...
static inline int
term_driver_drop_all_buffered_input_unlocked(
	struct term_dev *dev)
{
...
static inline int
term_driver_handle_canonical_input(
	struct term_dev *dev,
	char c)
{
    int res;
	...
	if(dev->canon_input_offset > 0) {
	    dev->canon_input_offset--;
            ...
            irq_lock_release(&dev->buffer_lock);
	    return -ENOMEM; // No "room" for an erase at the start of the line
	}
        irq_lock_release(&dev->buffer_lock);
	return 0;
    }
...
static ssize_t
term_driver_handle_raw_read_nonblocking(
	struct term_dev *dev,
	void *buffer,
	size_t buflen)
{
...
static ssize_t
term_driver_handle_canonical_read_nonblocking(
	struct term_dev *dev,
	void *buffer,
	size_t amount)
{
	...
	if(term_driver_buffer_queue_empty_unlocked(dev)) {
	    ...
	    dev->canon_output_offset = 0;
	    dev->canon_output_buflen = dev->buffer_queue[dev->buffer_queue_tail].len;
	    dev->canon_output_buffer = dev->buffer_queue[dev->buffer_queue_tail].data;
	    dev->buffer_queue_tail++;
	    if(dev->buffer_queue_tail >= dev->buffer_queue_len) {
		dev->buffer_queue_tail = 0;
	    }
    ...
    memcpy(buffer, dev->canon_output_buffer + dev->canon_output_offset, to_copy);
	...
	size_t buflen)
{
    ...
    rlock_read_lock(&dev->mode_lock);
    if(dev->mode.raw) {
	res = term_driver_handle_raw_read_nonblocking(
		dev,
		buffer,
		buflen);
    } else {
	res = term_driver_handle_canonical_read_nonblocking(
		dev,
		buffer,
		buflen);
    }
	...
	size_t amount)
{
	...
	if(res < 0) {
            if(res == -EWOULDBLOCK && written > 0) {
                // We outputted something (return how much we wrote)
                break;
            } else {
                // Some other error or we would block without outputting anything
                return res;
            }
	} else {
	    // We outputted a char
