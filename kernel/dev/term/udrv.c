...
term_dev_udrv_create(struct udrv_mount *mnt, const char *name)
{
    int res;
...
static int
term_dev_udrv_destroy(struct udrv_mount *mnt, struct udrv_dev *udrv_dev)
{
    int res;

    struct udrv_term_dev *dev =
        container_of(udrv_dev, struct udrv_term_dev, udrv_dev);
...
static int
term_dev_udrv_on_recv(struct udrv_mount *mnt,
                      struct udrv_dev *udrv_dev,
                      struct udrv_pkt *pkt,
                      size_t pktlen)
{
    ...
    default:
        printk("udrv: term_dev Received invalid packet from userspace! "
               "(type=%ld)\n",
               (sl_t)pkt->type);
        return -EINVAL;
    }
...
static struct udrv_mount_ops term_dev_udrv_mount_ops = {
    .create = term_dev_udrv_create,
    .destroy = term_dev_udrv_destroy,
    .on_recv = term_dev_udrv_on_recv,
};

static struct udrv_mount term_dev_udrv_mount = {
...
static int
udrv_term_dev_set_baudrate(struct term_dev *term_dev, baud_t baud)
{
    struct udrv_term_dev *dev =
        container_of(term_dev, struct udrv_term_dev, term_dev);
    if(baud != 0)
...
static int
udrv_term_dev_get_baudrate(struct term_dev *term_dev, baud_t *baud)
{
    struct udrv_term_dev *dev =
        container_of(term_dev, struct udrv_term_dev, term_dev);
    *baud = 0;
