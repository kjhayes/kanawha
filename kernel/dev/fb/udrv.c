...
static inline void
udrv_fb_dev_mode_info_get_ref(struct udrv_fb_dev_mode_info *info)
{
...
static inline void
udrv_fb_dev_mode_info_put_ref(struct udrv_fb_dev_mode_info *info)
{
...
static int
__udrv_fb_set_mode_lock_held(struct udrv_fb_dev *dev, size_t mode);

static struct udrv_dev *
fb_dev_udrv_create(struct udrv_mount *mnt, const char *name)
{
    int res;
...
static int
fb_dev_udrv_destroy(struct udrv_mount *mnt, struct udrv_dev *udrv_dev)
{
    int res;

    struct udrv_fb_dev *dev =
        container_of(udrv_dev, struct udrv_fb_dev, udrv_dev);
...
static int
fb_dev_udrv_have_mode_info(struct udrv_fb_dev *dev, unsigned long index)
{
...
static int
fb_dev_udrv_revoke_mode_info(struct udrv_fb_dev *dev, unsigned long index)
{
    irq_lock_acquire(&dev->mode_lock);
...
static int
fb_dev_udrv_add_mode_info(struct udrv_fb_dev *dev,
                          unsigned long index,
                          struct fb_mode_info *mode_info,
                          size_t mode_info_len)
{
    struct udrv_fb_dev_mode_info *info;
    info = kmalloc(sizeof(struct udrv_fb_dev_mode_info) + mode_info_len,
                   KM_KERNEL);
    if(info == NULL)
    {
        return -ENOMEM;
...
// Forward Decl.
static int
udrv_fb_flush_buffer(struct fb_dev *fb_dev);

static int
fb_dev_udrv_on_recv(struct udrv_mount *mnt,
                    struct udrv_dev *udrv_dev,
                    struct udrv_pkt *pkt,
                    size_t pktlen)
{
    ...
    case UDRV_FB_PKT_PROVIDE_MODE_INFO:
    {
        ...
        size_t mode_info_len =
            datalen - (sizeof(struct udrv_fb_pkt_provide_mode_info) -
        ...
        res = fb_dev_udrv_add_mode_info(dev,
                                        index,
                                        &args->mode_info,
                                        mode_info_len);
        if(res)
...
static struct udrv_mount_ops fb_dev_udrv_mount_ops = {
    .create = fb_dev_udrv_create,
    .destroy = fb_dev_udrv_destroy,
    .on_recv = fb_dev_udrv_on_recv,
};

static struct udrv_mount fb_dev_udrv_mount = {
...
declare_init_desc(fs,
                  register_fb_dev_udrv,
                  "Registering fb_dev Userspace Driver Framework");
...
__udrv_fb_set_mode_lock_held(struct udrv_fb_dev *dev, size_t mode)
{
    ...
    struct udrv_pkt *req_pkt = udrv_create_user_pkt(
        sizeof(*req_pkt) + sizeof(struct udrv_fb_pkt_set_mode));
...
static int
udrv_fb_set_mode(struct fb_dev *fb_dev, size_t mode)
{
    int res;
    struct udrv_fb_dev *dev = container_of(fb_dev, struct udrv_fb_dev, fb_dev);
    irq_lock_acquire(&dev->mode_lock);
...
static int
udrv_fb_load_buffer(struct fb_dev *fb_dev, void __phys **buffer)
{
    int res;
...
static int
udrv_fb_unload_buffer(struct fb_dev *fb_dev, void __phys *buffer)
{
    struct udrv_fb_dev *dev = container_of(fb_dev, struct udrv_fb_dev, fb_dev);
    irq_lock_acquire(&dev->mode_lock);
    if(dev->buffer_size <= 0)
...
#define MAX_PACKET_LEN                                                         \
    (DESIRED_MAX_PACKET_LEN > FILE_READ_MAX_BUFSIZE ? FILE_READ_MAX_BUFSIZE    \
                                                    : DESIRED_MAX_PACKET_LEN)
#define MAX_DATA_LEN                                                           \
    (MAX_PACKET_LEN -                                                          \
     (sizeof(struct udrv_pkt) + sizeof(struct udrv_fb_pkt_write_to_buffer)))
    ...
    {
        size_t datalen =
            (amt_to_write > MAX_DATA_LEN) ? MAX_DATA_LEN : amt_to_write;
        struct udrv_pkt *req_pkt = udrv_create_user_pkt(
            sizeof(*req_pkt) + sizeof(struct udrv_fb_pkt_write_to_buffer) +
            datalen);
        if(req_pkt == NULL)
        {
            wprintk("failed to allocated udrv packet during udrv "
                    "fb_dev flush "
                    "operation!\n");
            break;
        }
...
udrv_fb_get_mode_info(struct fb_dev *fb_dev, size_t mode)
{
    struct udrv_fb_dev *dev = container_of(fb_dev, struct udrv_fb_dev, fb_dev);
    irq_lock_acquire(&dev->mode_lock);
    ilist_node_t *iter;
...
static int
udrv_fb_put_mode_info(struct fb_dev *fb_dev, size_t mode)
{
    struct udrv_fb_dev *dev = container_of(fb_dev, struct udrv_fb_dev, fb_dev);
