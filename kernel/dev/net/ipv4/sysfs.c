...
static int
ipv4_dev_fs_probe_ipv4_dev(struct ipv4_dev *dev)
{
    return 0;
}

static int
ipv4_dev_fs_receive_ipv4_dev(struct ipv4_dev *dev)
{
    int res;
    ...
    res = vfs_mount_insert_node_and_link_root(ipv4_dev_fs_mount,
                                              &edfs->vfs_node,
    ...
    res = ipv4_dev_send_icmp_ping(dev,
                                  IPV4_ADDR_LOOPBACK,
                                  IPV4_ADDR_LOCAL_BROADCAST);
    if(res)
    {
...
static int
ipv4_dev_fs_revoke_ipv4_dev(struct ipv4_dev *dev)
{
...
static ssize_t
ipv4_dev_fs_file_write(struct file *file,
                       void *buffer,
                       ssize_t amount,
                       unsigned long flags)
{
    ...
    struct ipv4_dev_fs_node *idfs = container_of(fs_node->backing.priv_state,
                                                 struct ipv4_dev_fs_node,
                                                 vfs_node);
    ...
    struct ipv4_packet *pkt = ipv4_dev_alloc_packet(idfs->dev, amount, 0);
    ...
    res = ipv4_dev_send_packet(idfs->dev, pkt);

    // drop the packet regardless of success or failure to send
    int drop_res = ipv4_dev_drop_packet(idfs->dev, pkt);
    if(drop_res)
    {
        wprintk("Potential Memory Leak: ipv4_dev sysfs failed to drop "
                "allocated packet! (drop-err=%s)\n",
                errnostr(drop_res));
    }
...
static struct ipv4_dev_owner ipv4_dev_fs_owner = {
