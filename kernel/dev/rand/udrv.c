...
rand_dev_udrv_create(struct udrv_mount *mnt, const char *name)
{
    int res;
...
static int
rand_dev_udrv_destroy(struct udrv_mount *mnt, struct udrv_dev *udrv_dev)
{
    int res;

    struct udrv_rand_dev *dev =
        container_of(udrv_dev, struct udrv_rand_dev, udrv_dev);
...
static int
rand_dev_udrv_on_recv(struct udrv_mount *mnt,
                      struct udrv_dev *udrv_dev,
                      struct udrv_pkt *pkt,
                      size_t pktlen)
{
...
static struct udrv_mount_ops rand_dev_udrv_mount_ops = {
    .create = rand_dev_udrv_create,
    .destroy = rand_dev_udrv_destroy,
    .on_recv = rand_dev_udrv_on_recv,
};

static struct udrv_mount rand_dev_udrv_mount = {
...
declare_init_desc(fs,
                  register_rand_dev_udrv,
                  "Registering rand_dev Userspace Driver Framework");

static ssize_t
udrv_rand_dev_read(struct rand_dev *rand_dev, void *buffer, size_t buflen)
{
    ssize_t amt_read;

    struct udrv_rand_dev *dev =
        container_of(rand_dev, struct udrv_rand_dev, rand_dev);
