...
static ssize_t
rand_dev_fs_file_read(struct file *file,
                      void *buf,
                      ssize_t buflen,
                      unsigned long flags)
{
    ...
    struct rand_dev_fs_node *rdfs = container_of(node->backing.priv_state,
                                                 struct rand_dev_fs_node,
                                                 vfs_node);
    ...
    while(1)
    {
        amt_read = rand_dev_read(rdfs->dev, buf, buflen);
        if(amt_read < 0)
        {
...
static int
rand_dev_fs_probe_rand_dev(struct rand_dev *dev)
{
    return 0;
}

static int
rand_dev_fs_receive_rand_dev(struct rand_dev *dev)
{
    int res;
    ...
    res = vfs_mount_insert_node(rand_dev_fs_mount, &rdfs->vfs_node, &inode);
    if(res)
    {
        kfree(rdfs);
        return res;
    ...
    res = vfs_mount_link_root(rand_dev_fs_mount, rand_dev_get_name(dev), inode);
    if(res)
    {
        vfs_mount_remove_node(rand_dev_fs_mount, &rdfs->vfs_node);
        kfree(rdfs);
        return res;
    }
...
static int
rand_dev_fs_revoke_rand_dev(struct rand_dev *dev)
{
    wprintk("Tried to unregister a rand_dev from sysfs! (UNIMPL)\n");
    return -EUNIMPL;
}

static struct rand_dev_owner rand_dev_fs_owner = {
...
rand_dev_init_fs_mount(void)
{
    ...
    res = sysfs_register_mount(&rand_dev_fs_mount->fs_mount, "randdev");
    if(res)
    {
...
declare_init_desc(fs,
                  rand_dev_init_fs_mount,
                  "Registering randdev Sysfs Mount");
