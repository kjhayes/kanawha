...
#define DMA_MIN_REGION_ORDER                                                   \
    (VMEM_MIN_PAGE_ORDER < PAGE_ALLOC_MIN_ORDER ? PAGE_ALLOC_MIN_ORDER         \
                                                : VMEM_MIN_PAGE_ORDER)
...
__add_dma_region(order_t order, unsigned long dma_flags)
{
    int res;
    ...
    res = page_alloc(region->order, &region->phys_page, page_flags);
    if(res)
    {
        kfree(region);
        return NULL;
    ...
    struct dma_free_block *block =
        kmalloc(sizeof(struct dma_free_block), KM_KERNEL);
    if(block == NULL)
    {
...
static int
__free_dma_region(struct dma_region *region)
{
    int res;
...
static int
__dma_region_alloc(struct dma_region *region,
                   size_t size,
                   order_t align,
                   void __phys **out)
{
    ...
    ilist_for_each(free_node, &region->free_list)
    {
        ...
        else
        {
            continue; // Don't try splitting blocks
                      //            dprintk("__dma_region_alloc: Doubly
            //            Mis-Aligned\n"); size_t misalign_below =
            //            (1ULL<<align) - (phys_base & align_mask); size_t
            //            misalign_above = (phys_end - size) & align_mask;
            //            size_t total_size = blk->size;
            //
            //            struct dma_free_block *above =
            //            kmalloc(sizeof(struct dma_free_block),
            //            KM_KERNEL); if(above == NULL) {
            //                // Not enough memory to split the block in
            //                two continue;
            //            }
            //
            //            // Is it better to keep one large block and one
            //            tiny block,
            //            // or try to keep two medium sized blocks? (I'm
            //            not sure, but we'll go with the latter)
            ...
            //            above->size = (uintptr_t)phys_end -
            //            (uintptr_t)above->phys_base;
            //
            //            blk->size = (uintptr_t)(*out) - phys_base;
            //
            //            ilist_push_tail(&region->free_list,
            //            &above->list_node);
        }
...
static int
__dma_region_free(struct dma_region *region, void __phys *base, size_t size)
{
          ...
          dma_addr_t *dma_out)
{
    ...
    ilist_for_each(node, &dma_region_list)
    {
        ...
        void __phys *phys_base;
        res = __dma_region_alloc(region, size, align_order, &phys_base);
        if(res)
        {
            continue;
        }
    ...
    struct dma_region *region = __add_dma_region(region_order, flags);
...
int
dma_free(dma_addr_t addr, size_t size)
{
    ilist_node_t *iter;
