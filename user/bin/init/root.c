...
setup_root_fs(void)
{
...

    {
        res = mkdir("dev", 0);
        if(res) {return res;}
        res = setup_sysfs_mounts(
                "dev",
                (struct sysfs_mnt *)dev_sysfs_mnts,
                sizeof(dev_sysfs_mnts)/sizeof(dev_sysfs_mnts[0])
                );
        if(res) {
            return res;
        }
...

    {
        ...
        res = setup_sysfs_mounts(
                ...
                sizeof(sys_sysfs_mnts)/sizeof(sys_sysfs_mnts[0])
                );
        if(res) {
            return res;
        }
        res = setup_fs_mounts(
                ...
                sizeof(sys_base_fs_mnts)/sizeof(sys_base_fs_mnts[0])
                );
        if(res) {
            return res;
        }
