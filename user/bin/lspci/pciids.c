...
hex_chars(char *hex)
{
    ...
    for(int i = 0; i < 4; i++)
    {
        ...
        {
            fprintf(stderr,
                    "pci.ids: invalid hexadecimal string \"%c%c%c%c\"!\n",
                    hex[0],
                    hex[1],
                    hex[2],
                    hex[3]);
...
{
    if(found_vendor && found_device && found_subsystem && found_class &&
       found_subclass)
    {
    ...
    while(1)
    {
        ...
        if(linebuf[0] == '\t')
        {
            ...
            else
            {
                ...
                {
                    fprintf(stderr,
                            "malformed pci.ids file (found device/subsystem "
                            "before any vendor)!\n");
                    return 0;
                }
                if(linebuf[1] == '\t')
                {
                    ...
                    if(!found_subsystem)
                    {
                        ...
                        {
                            fprintf(stderr,
                                    "malformed pci.ids file (found subsystem "
                                    "before any device)!\n");
                            return 0;
                        }
                        ...
                        {
                            fprintf(stderr,
                                    "malformed pci.ids file (failed to read "
                                    "subsystem hex ids from line: \"%s\"\n",
                                    linebuf);
                            return 0;
                        }
        ...
        else
        {
            ...
            if(!found_vendor)
            {
                ...
                {
                    fprintf(stderr,
                            "malformed pci.ids file (failed to read vendor hex "
                            "id from line: \"%s\"\n",
                            linebuf);
                    return 0;
                }
