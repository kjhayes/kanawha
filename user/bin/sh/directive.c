...
do_setstdout(struct simple_cmd *cmd)
{
    ...
    int res;
    res = kanawha_sys_open(path, FILE_PERM_WRITE, 0, &file);
    if(res)
    {
        return res;
    }
...
#define BUFLEN 0x1000
    ...
    res = kanawha_sys_environ(key, buffer, BUFLEN, ENV_GET);
    if(res)
    {
        return res;
    }
...
do_setenv(struct simple_cmd *cmd)
{
    ...
    res = kanawha_sys_environ(key, (char *)value, strlen(value), ENV_SET);
    if(res)
    {
        return res;
    }
...
int
run_directive(struct simple_cmd *cmd)
{
    if(cmd->command == NULL)
    {
