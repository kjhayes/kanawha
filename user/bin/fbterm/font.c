...
load_font(const char *path)
{
    ...
    {
        fprintf(stderr,
                "failed to load font \"%s\": could not read PSF1 header!\n",
                path);
        fclose(file);
        return NULL;
    }
    ...
    {
        fprintf(stderr,
                "failed to load font \"%s\": header was invalid!\n",
                path);
        fclose(file);
        return NULL;
    }
    ...
    {
        fprintf(stderr,
                "failed to load font \"%s\": failed to allocate data buffer!\n",
                path);
        fclose(file);
        return NULL;
    }
    ...
    {
        fprintf(stderr,
                "failed to load font \"%s\": failed to read font data!\n",
                path);
        free(font_data);
        fclose(file);
        return NULL;
    ...
    if(fdata == NULL)
    {
        free(font_data);
        fprintf(stderr,
                "failed to load font \"%s\": failed to allocate font data!\n",
                path);
        return NULL;
    }
    ...
    if(fdata->glyphs == NULL)
    {
        ...
        free(fdata);
        fprintf(stderr,
                "failed to load font \"%s\": failed to allocate glyph data!\n",
                path);
        return NULL;
    }
    ...
    for(size_t i = 0; i < fdata->num_glyphs; i++)
    {
        ...
        color_t *fg_pixel_data =
            malloc(sizeof(color_t) * fdata->width * fdata->height);
        color_t *bg_pixel_data =
            malloc(sizeof(color_t) * fdata->width * fdata->height);

        if(fg_img == NULL || bg_img == NULL || fg_pixel_data == NULL ||
           bg_pixel_data == NULL)
        {
            ...
            free(bg_pixel_data);
            fprintf(stderr,
                    "failed to load font \"%s\": failed to load glyph!\n",
                    path);
            return NULL;
        }
        ...
        for(size_t fy = 0; fy < fdata->height; fy++)
        {
            ...
            for(int fx = 0; fx < 8; fx++)
            {
                ...
                color_t *fg_pixel =
                    &((color_t *)fg_img->data)[fx + (fy * fdata->width)];
                color_t *bg_pixel =
                    &((color_t *)bg_img->data)[fx + (fy * fdata->width)];
                if(value)
                {
                    fg_pixel->r = 0xFF;
                    fg_pixel->g = 0xFF;
