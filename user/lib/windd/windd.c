...
static int
window_init_common(struct window *win)
{
    win->disconnected = 0;
    ...
    win->input_buflen = 0;
    win->input_ringbuf =
        malloc(sizeof(struct input_event) * DEFAULT_INPUT_EVENT_BUFLEN);
...
static int
window_deinit_common(struct window *win)
{
...
static int
windd_window_on_recv_input_msg(struct window *win, struct input_event *evt);

static int
windd_window_pong(struct window *win, char recv_ping)
{
...
static inline int
windd_server_window_notify_layout(struct window *win)
{
    struct window_msg msg = {
...
static inline int
windd_server_window_notify_position(struct window *win)
{
    struct window_msg msg = {
...
static inline int
windd_window_on_recv_msg(struct window *win, struct window_msg *msg)
{
    int res = 0;
    switch(msg->type)
    {
    ...
    case WINDD_MSG_TYPE_INPUT_EVENT:
        ...
        res = windd_window_on_recv_input_msg(win,
...
windd_window_reload_buffer(struct window *win)
{
    ...
    size_t req_len =
        win->layout.offset +
        (win->layout.width * win->layout.height) * win->layout.stride;
    ...
    // printf("windd_window_reload_buffer: need to remap buffer (req=0x%lx,
    // cur=0x%lx) buffer=%p\n",
...
static int
windd_input_lock_acquire(struct window *win)
{
...
static int
windd_input_lock_release(struct window *win)
{
...
int
windd_window_disconnected(struct window *window)
{
    int res;
