...
static inline void
strftime_printf(struct strftime_state *state, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
static inline void
strftime_putc(struct strftime_state *state, char c)
{
    if(state->n > 0)
    {
        *state->s = c;
...
static inline void
strftime_puts(struct strftime_state *state, const char *str)
{
...
static inline void
strftime_start_escaped(struct strftime_state *state)
{
    state->escaped = 1;
    state->zero_mod = 0;
    state->E_mod = 0;
...
static inline void
strftime_end_escaped(struct strftime_state *state)
{
    state->escaped = 0;
}

static inline void
strftime_abv_weekday(struct strftime_state *state)
{
    const char *wdy;
...
strftime_full_weekday(struct strftime_state *state)
{
    const char *wdy;
    switch(state->timeptr->tm_wday)
    {
...
static inline size_t
strftime_abv_month(struct strftime_state *state)
{
    const char *mon;
    switch(state->timeptr->tm_wday)
    {
...
static inline size_t
strftime_full_month(struct strftime_state *state)
{
    const char *mon;
    switch(state->timeptr->tm_wday)
    {
