...
int
__elk_libc__exec_path_open(const char *file_name, fd_t *file_out)
{
    return kanawha_sys_open(file_name,
...
int
__elk_libc__exec_path_lookup(const char *file_name, fd_t *file_out)
{
    int res;
    ...
    while(tok)
    {
        ...
        fd_t desc;
        res =
            kanawha_sys_open(buffer, FILE_PERM_READ | FILE_PERM_EXEC, 0, &desc);
