...
int
__elk_doopen(const char *pathname, int flags, va_list args, fd_t *dir_fd)
{
    int res;
    ...
    res = kanawha_sys_open(pathname, access_flags, mode_flags, &file_fd);
    if(res == 0)
    {
        return file_fd;
    }
    ...
    if(flags & O_CREAT)
    {
        ...
        fd_t dir_fd;
        res = kanawha_sys_open(directory,
        ...
        res = kanawha_sys_mkfile(dir_fd, new_file_name, mkfile_flags);
        free(pathname_dup);
        if(res)
        {
            errno = res;
