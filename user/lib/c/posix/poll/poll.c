    ...
    for(nfds_t i = 0; i < nfds; i++)
    {
        ...
        else
        {
            ...
            typeof(fds[i].events) read_events =
                ...
                (fds[i].events & POLLRDBAND) || (fds[i].events & POLLPRI);

            typeof(fds[i].events) write_events = (fds[i].events & POLLOUT) ||
            ...
            res = kanawha_sys_poll(fds[i].fd, watching, &triggered);
            ...
            {
                fds[i].revents =
                    ((triggered & POLL_READ_NONBLOCKING) ? read_events : 0) ||
