...
static inline int
__elk_libc_ringbuffer_init(struct __elk_libc_ringbuffer *buf, size_t scale)
{
    buf->buffer = NULL;
    buf->buflen = 0;
    buf->head = 0;
...
static inline int
__elk_libc_ringbuffer_deinit(struct __elk_libc_ringbuffer *buf)
{
    if(buf->buffer)
    {
        free(buf->buffer);
...
static inline int
__elk_libc_ringbuffer_full(struct __elk_libc_ringbuffer *buf)
{
    return ((buf->head + 1) % buf->buflen) == buf->tail;
}

static inline int
__elk_libc_ringbuffer_empty(struct __elk_libc_ringbuffer *buf)
{
    return buf->head == buf->tail;
}

static inline size_t
__elk_libc_ringbuffer_enqueued(struct __elk_libc_ringbuffer *buf)
{
...
static inline int
__elk_libc_ringbuffer_grow(struct __elk_libc_ringbuffer *buf)
{
    size_t new_len;
    if(buf->buflen <= 1)
    {
...
static inline int
__elk_libc_read_into_ringbuffer(int fd,
                                ...
                                size_t maximum_read)
{
    ...
    ssize_t amt_read = kanawha_sys_read(fd, buf->buffer + buf->head, room);
    if(amt_read < 0)
    {
        return amt_read;
    }
...
__elk_libc_write_from_ringbuffer(int fd, struct __elk_libc_ringbuffer *buf)
{
    int res;
    ...
    ssize_t amt_written = kanawha_sys_write(fd, buf->buffer + buf->tail, room);
    if(amt_written < 0)
    {
        return amt_written;
    }
...
static inline int
__elk_libc_ringbuffer_drop_data(struct __elk_libc_ringbuffer *buf)
{
    buf->head = 0;
    buf->tail = 0;
    buf->buflen = 0;
