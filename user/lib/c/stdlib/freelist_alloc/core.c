...
_Static_assert(sizeof(struct free_region) == sizeof(struct allocation),
        "Kanawha LibC freelist allocator depends on sizeof(struct free_region) == sizeof(struct allocation)!");
...
heap_grow_lockless(void)
{
    ...
    init_free->next = NULL;
    init_free->data_size = block->size
        - sizeof(struct free_region)
