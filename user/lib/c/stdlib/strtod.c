...
double
strtod(const char *nptr, char **restrict endptr)
{
    while(isspace(*nptr))
    {
        nptr++;
    ...
    // deal with exponents
    if((*nptr == EXPONENT_0 || *nptr == EXPONENT_1) && isdigit(nptr[1]))
    {
        nptr++; // consume the "e" or "E"
        long long exp = strtoll(nptr, (char **)&nptr, 10);
        if(exp > 1000)
        {
            exp = 1000; // This is more than large enough already to
                        // saturate any reasonable floating point
                        // width...
        }
        if(exp < -1000)
        {
            exp = -1000; // This is more than small enough to round to
                         // zero...
        }
        if(exp > 0)
        {
