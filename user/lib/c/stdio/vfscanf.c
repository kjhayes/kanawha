...
static int
vfscanf_consumestr(size_t len, void *state)
{
...
static const char *
vfscanf_peekstr(size_t min_len, size_t max_len, void *state)
{
    FILE *file = (FILE *)state;
...
{
    return doscan(vfscanf_consumestr, vfscanf_peekstr, stream, format, arg);
}
